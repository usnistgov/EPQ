...
public class FilteredSpectrum
   ...
   static private final boolean TRIM_THEN_FILTER = true;
   ...
   private void compute(ISpectrumData spec, RegionOfInterest roi) {
      ...
      if(TRIM_THEN_FILTER)
         ...
         compute(SpectrumUtils.slice(spec, 0, spec.getChannelCount()), 0, spec.getChannelCount());
         final double[] filter = mFilter.getFilter();
         final int hl = filter.length / 2, ol = filter.length - hl;
         Arrays.fill(mFilteredData, 0, lowCh - hl, 0);
         Arrays.fill(mErrors, 0, lowCh - hl, 0);
         Arrays.fill(mFilteredData, highCh + ol, spec.getChannelCount(), 0);
         Arrays.fill(mErrors, highCh + ol, spec.getChannelCount(), 0);
      }
   }

   private void compute(ISpectrumData spec) {
      final int lld = SpectrumUtils.getZeroStrobeDiscriminatorChannel(spec);
      compute(SpectrumUtils.slice(spec, lld, spec.getChannelCount() - lld), lld, spec.getChannelCount() - lld);
   }
   ...
   private void compute(double[] roiData, int lowCh, int chCount) {
      ...
      Arrays.fill(err, Double.MAX_VALUE);
    ...
    * @param ff FittingFilter
    */
   public FilteredSpectrum(ISpectrumData src, FittingFilter ff)
         throws EPQException {
