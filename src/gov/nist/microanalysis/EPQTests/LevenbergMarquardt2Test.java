 ...
 * @version 1.0
 */
public class LevenbergMarquardt2Test
   extends TestCase {
   private static final int NUM_ITERATIONS = 10000;
   private static final int NUM_GAUSSIANS = 2;

   private class TestFunction
      extends LevenbergMarquardt2.AutoPartialsFitFunction {
      ...
      public Matrix partials(Matrix params) {
         if(false) {
            final Matrix res = new Matrix(mNumChannels, params.getRowDimension());
            int idx = 0;
            final double s = params.get(idx++, 0);
            final double[] center = new double[NUM_GAUSSIANS], amplitude = new double[NUM_GAUSSIANS];
            for(int i = 0; i < NUM_GAUSSIANS; ++i) {
               center[i] = params.get(idx++, 0);
               amplitude[i] = params.get(idx++, 0);
            }
            assert idx == params.getRowDimension();
            for(int ch = 0; ch < mNumChannels; ++ch) {
               double tmp = 0.0;
               for(int i = 0; i < NUM_GAUSSIANS; ++i)
                  tmp += amplitude[i] * Math.exp(-0.5 * Math2.sqr((center[i] - ch) / s)) * ((center[i] - ch) / (s * s * s));
               res.set(ch, 0, tmp);
            }
            idx = 1;
            for(int i = 0; i < NUM_GAUSSIANS; ++i) {
               double tmpA = 0.0, tmpC = 0.0;
               for(int ch = 0; ch < mNumChannels; ++ch) {
                  final double de = center[i] - ch;
                  final double g = Math.exp(-0.5 * Math2.sqr(de / s));
                  tmpC -= amplitude[i] * g * (de / (s * s));
                  tmpA += g;
                  res.set(ch, idx, tmpC);
                  res.set(ch, idx + 1, tmpA);
               }
               idx += 2;
            }
            assert idx == params.getRowDimension();
    ...
    * @throws EPQException
    */
   public void testComputeFitFunctionMatrixMatrixMatrix()
         throws EPQException {
