...
public class MaterialFactory {
   ...
   private static final String[] PBaseMaterials = {K3189, RynasAlTiAlloy, Mylar, VanadiumPentoxide, SiliconDioxide, Ice, CaCO3, Al2O3, SS316,
         UraniumOxide, K227, K309, K411, K412, K961, K1080, K2450, K2451, K2466, K2469, K2472, K2496, ParaleneC, MagnesiumOxide, Chloroapatite,
         CalciumFluoride};

   public static List<String> BaseMaterials = Collections.unmodifiableList(Arrays.asList(PBaseMaterials));

   public static final double[] mElementalDensities = {0.0, 0.0, 0.534, 1.85, 2.53, 2.25, 0.0, 0.0, 0.0, 0.0, // H
                                                                                                              // to
         // Ne
         0.97, // Na
         ...
         if (c == '(') {
            ...
            for (int j = i + 1; j < str.length(); ++j) {
               ...
               if (pcx == 0) {
                  ...
                  if (tmp.size() == 0)
                     throw new EPQException("There must be something between the (). " + str.substring(i + 1, j - 1));
   ...
   /**
    * Create a Composition by parsing a string containing a compound in
    * simplified chemical notation. Examples: "Al2O3", "Ca5(PO3)4Cl", "SiO2"
    * 
    * @param str
    * @return Composition
    * @throws EPQException
   ...
   /**
    * createMaterial - Creates the material based on the type identified as one
    * of the integer constants listed above.
    * 
    * @param name
    *           String - One of the predefined named constants
    * @param density
    *           double in SI
    * @return Material
    */
   ...
   public static Composition createMaterial(String name) {
      ...
      try {
         if (name.equals(K3189)) {
            ...
            tmp.defineByMaterialFraction(new Composition[]{MaterialFactory.createCompound("SiO2"), MaterialFactory.createCompound("Al2O3"),
                  MaterialFactory.createCompound("CaO"), MaterialFactory.createCompound("MgO"), MaterialFactory.createCompound("TiO2"),
                  MaterialFactory.createCompound("Fe2O3")}, new double[]{0.400000, 0.140000, 0.140000, 0.100000, 0.020000, 0.200000});
            mat = new Material(tmp, ToSI.gPerCC(3.23));
            mat.setName(K3189);
         } else if (name.equals(RynasAlTiAlloy))
            return new Material(new Element[]{Element.Ti, Element.Al, Element.Nb, Element.W}, new double[]{0.54, 0.31, 0.11, 0.04}, ToSI.gPerCC(8.57),
                  RynasAlTiAlloy);
            ...
            comp.defineByMaterialFraction(new Composition[]{MaterialFactory.createMaterial("SiO2"), MaterialFactory.createMaterial("PbO")},
                  new double[]{0.20000, 0.80000,});
            comp.setName(K227);
         } else if (name.equals(K309)) {
            comp = new Composition();
            comp.defineByMaterialFraction(
                  new Composition[]{MaterialFactory.createCompound("Al2O3"), MaterialFactory.createCompound("SiO2"),
                        MaterialFactory.createCompound("CaO"), MaterialFactory.createCompound("Fe2O3"), MaterialFactory.createCompound("BaO")},
            ...
            mat.defineByMaterialFraction(
                  new Composition[]{MaterialFactory.createCompound("MgO"), MaterialFactory.createCompound("SiO2"),
            ...
            comp = new Composition();
            comp.defineByMaterialFraction(
                  new Composition[]{MaterialFactory.createCompound("MgO"), MaterialFactory.createCompound("Al2O3"),
                        MaterialFactory.createCompound("SiO2"), MaterialFactory.createCompound("CaO"), MaterialFactory.createCompound("FeO")},
                  new double[]{0.193300, 0.092700, 0.453500, 0.152500, 0.099600});
            comp.setName(K412);
         } else if (name.equals(K961))
            mat = new Material(
                  new Element[]{Element.Na, Element.Mg, Element.Al, Element.Si, Element.P, Element.K, Element.Ca, Element.Ti, Element.Mn, Element.Fe,
                        Element.O},
                  new double[]{0.029674, 0.030154, 0.058215, 0.299178, 0.002182, 0.024904, 0.035735, 0.011990, 0.003160, 0.034972, 0.469837},
                  ToSI.gPerCC(6.0), K961);
         else if (name.equals(K1080))
            mat = new Material(
                  new Element[]{Element.Li, Element.B, Element.Mg, Element.Al, Element.Si, Element.Ca, Element.Ti, Element.Sr, Element.Zr, Element.Lu,
                        Element.O},
                  new double[]{0.027871, 0.006215, 0.008634, 0.079384, 0.186986, 0.107204, 0.011990, 0.126838, 0.007403, 0.017588, 0.416459},
                  ToSI.gPerCC(6.0), K1080);
         else if (name.equals(K2450)) {
            comp = new Composition();
            comp.defineByMaterialFraction(
                  new Composition[]{MaterialFactory.createCompound("SiO2"), MaterialFactory.createCompound("Al2O3"),
                        MaterialFactory.createCompound("CaO"), MaterialFactory.createCompound("TiO2")},
            ...
            comp.defineByMaterialFraction(
                  new Composition[]{MaterialFactory.createCompound("SiO2"), MaterialFactory.createCompound("Al2O3"),
                        MaterialFactory.createCompound("CaO"), MaterialFactory.createCompound("V2O5")},
                  new double[]{0.300000, 0.300000, 0.300000, 0.100000});
            comp.setName(name);
         } else if (name.equals(K2466)) {
            comp = new Composition();
            comp.defineByMaterialFraction(new Composition[]{MaterialFactory.createCompound("SiO2"), MaterialFactory.createCompound("BaO"),
                  MaterialFactory.createCompound("TiO2")}, new double[]{0.44, 0.48, 0.08});
            comp.setName(name);
         } else if (name.equals(K2469)) {
            comp = new Composition();
            comp.defineByMaterialFraction(new Composition[]{MaterialFactory.createCompound("SiO2"), MaterialFactory.createCompound("BaO"),
                  MaterialFactory.createCompound("TiO2")}, new double[]{0.36, 0.48, 0.16});
            comp.setName(name);
         } else if (name.equals(K2472)) {
            comp = new Composition();
            comp.defineByMaterialFraction(new Composition[]{MaterialFactory.createCompound("SiO2"), MaterialFactory.createCompound("BaO"),
                  MaterialFactory.createCompound("TiO2"), MaterialFactory.createCompound("V2O5")}, new double[]{0.36, 0.48, 0.10, 0.06});
            comp.setName(name);
         } else if (name.equals(K2496)) {
            comp = new Composition();
            comp.defineByMaterialFraction(new Composition[]{MaterialFactory.createCompound("SiO2"), MaterialFactory.createCompound("BaO"),
                  MaterialFactory.createCompound("TiO2")}, new double[]{0.49, 0.48, 0.03});
            comp.setName(name);
   ...
   /**
    * MaterialFactory - Create a material factory from the XML library specified
    * by the Reader r.
    * 
    * @param r
    *           Reader
   ...
   /**
    * append - Append the materials in the XML library specified in the Reader R
   ...
   /**
    * estimatedDensity - Compute an estimated density for a mixture of the
    * element specified by the Material. The assumption is that the density of
    * the Material is the mass percent weighted average of the densities of the
    * constituent elements. (Ok! So its just an estimate.)
    * 
    * @param mat
    *           Material
    * @return double
   ...
   /**
    * Returns the nominal default density (in SI units) for a common solid form
    * of the specified element as a pure material.
    * 
    * @param elm
    * @return double
    */
