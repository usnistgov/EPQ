...
public class ThresholdChargeConductionModel
   ...
   /*
    * oneStep() algorithm summary: The corrected electric field in this region
    * is the field with any components in forbidden directions (e.g., into
    * neighboring vacuum) subtracted. Find the corrected electric field in this
    * region and compare it to the threshold for this region. If it is less,
    * then the electron doesn't move. (We return the start region.) If it is
    * more, we flow the electron across one of the startRegion's faces. (We
    * return the region on the other side of the face.) Which face? We find the
    * faces with negative currents, meaning electrons flow out through them. The
    * current through a face is proportional to the dot product of the corrected
    * E field and its area * outward normal vector. If there's only one of
    * these, that's the one. If there are more than one, we choose one with
   ...
   private MeshElementRegion oneStep(MeshElementRegion startRegion, Mesh mesh) {
      ...
      /*
       * For charge to flow through a face we require two conditions be
       * satisfied: (1) The current through that face must be negative. (2) The
       * potential energy must decrease. The latter condition guarantees we do
       * not have closed loops that repeat ad infinitum. Faces that do not meet
       * these conditions have face current set to 0.
       */
      ...
      /*
       * Because we average the field across faces it is possible to have inflow
    ...
    * supplied tetrahedron, E2[i] is the corrected field in the tet through the
    * ith face, and S[i] is a vector with magnitude equal to the area of the ith
    * face and direction given by the face's outward normal. If a face has no
    * neighbor or if its neighbor is of a different material, the corresponding
    * value is 0. For finite conductivity, this quantity is proportional to the
    * current through the faces.
    *
    * @param tet
    * @param eField
    * @return
   ...
   private double[] faceCurrents(Tetrahedron tet, double[] eField) {
      ...
      for(int faceIndex = 0; faceIndex < 4; faceIndex++) {
         ...
         /*
          * Current through each face is proportional to area*eField.n. eField
          * is discontinuous across boundaries between tets. The average of the
          * two is a consistent estimate. (It will be the same if tet and
          * nextTet are reversed.) There's no need to actually divide by 2,
          * since we're only comparing faces to each other.
          */
