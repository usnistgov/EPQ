...
public class MaterialTest extends TestCase {
   ...
   public void testOne() throws EPQException {
      final double eps = 1.0e-8;
      {
         ...
         {
            final Element[] elms = {Element.Ti, Element.O};
            final double[] wgts = {fac * wTi, fac * wO};
      ...
      { // K3189 - NIST SRM glass
         final Element[] elms = {Element.Si, Element.O, Element.Al, Element.Ca, Element.Mg, Element.Ti, Element.Fe};
         final double[] mF = {0.151971599, 0.608811816, 0.062690151, 0.05699065, 0.056638401, 0.005716531, 0.057180851};
         final double[] wF = {0.186973968, 0.426700667, 0.074093109, 0.100056707, 0.06030359, 0.011986858, 0.139885101};
         ...
         { // Try an alternative method to define a K3189
            ...
            mats[0].defineByMoleFraction(new Element[]{Element.Si, Element.O}, new double[]{1.0, 2.0});
            mats[1] = new Material(1.0);
            mats[1].defineByMoleFraction(new Element[]{Element.Al, Element.O}, new double[]{2.0, 3.0});
            mats[2] = new Material(1.0);
            mats[2].defineByMoleFraction(new Element[]{Element.Ca, Element.O}, new double[]{1.0, 1.0});
            mats[3] = new Material(1.0);
            mats[3].defineByMoleFraction(new Element[]{Element.Mg, Element.O}, new double[]{1.0, 1.0});
            mats[4] = new Material(1.0);
            mats[4].defineByMoleFraction(new Element[]{Element.Ti, Element.O}, new double[]{1.0, 2.0});
            mats[5] = new Material(1.0);
            mats[5].defineByMoleFraction(new Element[]{Element.Fe, Element.O}, new double[]{2.0, 3.0});
            mat.defineByMaterialFraction(mats, new double[]{0.40, 0.14, 0.14, 0.10, 0.02, 0.20});
            for (int i = 0; i < elms.length; ++i)
               assertEquals(mat.weightFraction(elms[i], true), wF[i], 1.0e-5);
      ...
      {
         final String[] mats = {MaterialFactory.K2450, MaterialFactory.Ice, MaterialFactory.Al2O3};
