...
final public class SpectrumUtils {
    ...
    * 
    * @param sd ISpectrumData
    * @param applyLLD true-&gt; Does not include channels less than or equal to
    *           the LLD
    * @return double
    */
   ...
   static public BufferedImage toThumbnail(ISpectrumData spec, int maxCh, int width, int height) {
      ...
      if(false) {
         int mean = 0;
         for(int i = 0; i < width; ++i) {
            final int minI = (height - 2) - (int) Math.round((min[i] / allMax) * (height - 2));
            final int maxI = (height - 2) - (int) Math.round((max[i] / allMax) * (height - 2));
            final int newMean = (minI + maxI) / 2;
            g.drawLine(i - 1, mean, i, newMean);
            g.drawLine(i, minI, i, maxI);
            mean = newMean;
         }
      } else
         for(int i = 0; i < width; ++i) {
    ...
    *            liveTime too large.
    */
   public static EditableSpectrum subSampleSpectrum(ISpectrumData sd, double liveTime, long seed)
         throws EPQException {
      ...
      }
      {
         ...
         res.getProperties().setTextProperty(SpectrumProperties.SpectrumDisplayName, "Subsampled[" + sd.toString() + ","
               + nf.format(liveTime) + " s]");
    ...
    *            liveTime too large.
    */
   public static EditableSpectrum subSampleSpectrum(ISpectrumData sd, double liveTime)
         throws EPQException {
   ...
   public static EditableSpectrum addNoiseToSpectrum(ISpectrumData src, double k) {
      ...
      if(sp.isDefined(SpectrumProperties.LiveTime))
         ...
         if(sp.isDefined(SpectrumProperties.FaradayBegin))
            sp.setNumericProperty(SpectrumProperties.FaradayBegin, k
                  * sp.getNumericWithDefault(SpectrumProperties.FaradayBegin, 0.0));
         if(sp.isDefined(SpectrumProperties.FaradayEnd))
            sp.setNumericProperty(SpectrumProperties.FaradayEnd, k
                  * sp.getNumericWithDefault(SpectrumProperties.FaradayEnd, 0.0));
      }
   ...
   static public double[] estimateLowBackground(ISpectrumData spec, int min) {
      if(false) {
         final int MAX_BINS = 100, MIN_BINS = 3;
         final DescriptiveStatistics ds = new DescriptiveStatistics();
         for(int ch = min; (ch > (min - MIN_BINS)) && (ch >= 0); --ch)
            ds.add(spec.getCounts(ch));
         // Check to ensure that the best case error statistics follow
         // count statistics, the worst case can be dictated by some
         // screwball structure in the spectrum.
         double count = ds.sum() > 0.0 ? (Math.sqrt(ds.sum()) / ds.sum()) * ds.average() : 1.0;
         double altErr = ds.standardDeviation() / Math.sqrt(MIN_BINS);
         double bestErr = altErr > count ? altErr : count;
         double bestAvg = ds.average();
         final int end = SpectrumUtils.bound(spec, min - MAX_BINS);
         for(int ch = min - MIN_BINS; ch > end; --ch) {
            ds.add(spec.getCounts(ch));
            count = ds.sum() > 0.0 ? (Math.sqrt(ds.sum()) / ds.sum()) * ds.average() : 1.0;
            altErr = ds.standardDeviation() / Math.sqrt((min - ch) + 1);
            final double err = altErr > count ? altErr : count;
            if(err < bestErr) {
               bestErr = err;
               bestAvg = ds.average();
            }
         ...
         return new double[] {
            bestRes,
            Math.max(1.0, Math.sqrt(bestRes) * bestErr),
            (min - bestCh) + 1
         };
      }
   }
   ...
   static public double[] estimateHighBackground(ISpectrumData spec, int max) {
      if(false) {
         final int MAX_BINS = 100, MIN_BINS = 3;
         final DescriptiveStatistics ds = new DescriptiveStatistics();
         for(int ch = max; (ch < (max + MIN_BINS)) && (ch < spec.getChannelCount()); ++ch)
            ds.add(spec.getCounts(ch));
         double count = ds.sum() > 0.0 ? (Math.sqrt(ds.sum()) / ds.sum()) * ds.average() : 1.0;
         double altErr = ds.standardDeviation() / Math.sqrt(MIN_BINS);
         double bestErr = altErr > count ? altErr : count;
         double bestAvg = ds.average();
         for(int ch = max + MIN_BINS; (ch < (max + MAX_BINS)) && (ch < spec.getChannelCount()); ++ch) {
            ds.add(spec.getCounts(ch));
            count = ds.sum() > 0.0 ? (Math.sqrt(ds.sum()) / ds.sum()) * ds.average() : 1.0;
            altErr = ds.standardDeviation() / Math.sqrt((ch - max) + 1);
            final double err = altErr > count ? altErr : count;
            if(err < bestErr) {
               bestErr = err;
               bestAvg = ds.average();
    ...
    */
   static public double[] backgroundCorrectedIntegral(ISpectrumData spec, double ePeakLow, double ePeakHigh, double eLowBkgLow, double eLowBkgHigh, double eHighBkgLow, double eHighBkgHigh) {
      return backgroundCorrectedIntegral(spec, SpectrumUtils.channelForEnergy(spec, ePeakLow), SpectrumUtils.channelForEnergy(spec, ePeakHigh), SpectrumUtils.channelForEnergy(spec, eLowBkgLow), SpectrumUtils.channelForEnergy(spec, eLowBkgHigh), SpectrumUtils.channelForEnergy(spec, eHighBkgLow), SpectrumUtils.channelForEnergy(spec, eHighBkgHigh));
   }
    ...
    *         integral, double[3] -&gt; background integral
    */
   static public double[] backgroundCorrectedIntegral(ISpectrumData spec, int chPeakLow, int chPeakHigh, int chLowBkgLow, int chLowBkgHigh, int chHighBkgLow, int chHighBkgHigh) {
      ...
      final UncertainValue2 a = UncertainValue2.multiply(((chPeakHigh - chPeakLow)
            + 1), UncertainValue2.add(UncertainValue2.multiply((cPeak - cLow)
                  / (cHigh - cLow), UncertainValue2.subtract(bHigh, bLow)), bLow));
   ...
   /**
    * Turns a spectrum into a ZombieSpectrum which is more friendly way to store
    * a spectrum in low memory circumstances.
    * 
    * @param spec
    * @return A ZombieSpectrum or the original ISpectrumData if a zombie can't
    *         be created.
    */
   ...
   static public double getExitAngle(SpectrumProperties props)
      ...
      if(props.isDefined(SpectrumProperties.DetectorPosition)
         ...
         assert (Math2.distance(getSurfaceNormal(props), Math2.MINUS_Z_AXIS) > 0.01) || (Math.abs(res
               - Math.toRadians(props.getNumericWithDefault(SpectrumProperties.Elevation, Math.toDegrees(res)))) < Math.toRadians(5.0)) : "The computed TOA and default TOA differ by "
    ...
    * @throws EPQException
    */
   static public double getDose(SpectrumProperties props)
         throws EPQException {
    ...
    * @throws EPQException
    */
   static public Map<XRayTransition, Double> roughEnergyCalibration(Collection<Element> elms, Collection<Double> peakPositions, double e0, double eLow, double tol)
         throws EPQException {
    ...
    * @throws EPQException
    */
   static public LinearRegression linearEnergyCalibration(Collection<Element> elms, Collection<Double> peakPositions, double e0, double eLow, double tol)
         throws EPQException {
   ...
   static public UncertainValue2 resolutionU(UncertainValue2 fano, UncertainValue2 noise, double eV) {
      return UncertainValue2.multiply(ENERGY_PER_EH_PAIR, UncertainValue2.add(UncertainValue2.multiply(noise, noise), UncertainValue2.multiply(eV
            / ENERGY_PER_EH_PAIR, fano)).sqrt());
   }
   ...
   static public UncertainValue2 noiseFromResolution(UncertainValue2 fano, UncertainValue2 gRes, double eV) {
      return UncertainValue2.sqrt(UncertainValue2.subtract(UncertainValue2.sqr(UncertainValue2.multiply(1.0
            / ENERGY_PER_EH_PAIR, gRes)), UncertainValue2.multiply(eV / ENERGY_PER_EH_PAIR, fano)));
   }
   ...
   static public ISpectrumData remap(ISpectrumData spec, double zero, double chWidth, int nChannels) {
      class RemapSpectrum
         ...
         RemapSpectrum(double zeroOff, double chWidth, int nCh, ISpectrumData spec) {
            ...
            SpectrumUtils.rename(this, "Remap[" + spec.toString() + "," + df4.format(chWidth) + " eV/ch," + df1.format(zeroOff)
                  + "eV," + Integer.toString(nCh) + "]");
         }
