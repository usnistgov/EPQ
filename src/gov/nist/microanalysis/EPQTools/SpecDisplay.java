...
public class SpecDisplay extends JComponent {
	...
	private static final String COUNTS_SQUARE_ROOT_AXIS_LABEL = "Counts (Square Root)";
	private static final String COUNTS_LOG_AXIS_LABEL = "Counts (Log)";
	private static final String COUNTS_AXIS_LABEL = "Counts";
	 ...
	 * [990,1000), [1000,1010). This suprises users who expect the last channel to
	 * not be there. By subtracting off ENERGY_EPS we eliminate this issue with the
	 * rare possibility of not including E+ENERGY_EPS/2 correctly.
	 */
		...
		try {
			if (colors.isFile()) {
				...
				for (final double[] row : csv) {
					if (row.length >= 3) {
						final Color c = new Color((int) Math.round(row[0]), (int) Math.round(row[1]),
								(int) Math.round(row[2]));
						tmp.add(c);
					}
				}
				if (tmp.size() > 4)
	...
	private static Color[] getDefaultColors() {
		return new Color[] { new Color(255, 66, 14), new Color(0, 69, 134), new Color(87, 157, 28),
				new Color(126, 0, 33), new Color(131, 202, 255), new Color(49, 64, 4), new Color(174, 207, 0),
				new Color(75, 31, 111), new Color(255, 149, 14), new Color(197, 0, 11), new Color(0, 132, 209),
	...
	public enum SCALING_MODE {
		NONE("Same scale"), INTEGRAL("Scaled to equal integral"), MAX_PEAK("Scaled to highest peak"),
		REGION_INTEGRAL("Scaled to region integral"), CONSTANT_FLUX("Scaled to 60 nA\u00B7second"),
		PER_NA_EV("Scaled to 1 eV\u00B7second");
	...
	private SCALING_MODE mCurrentScalingMode = SCALING_MODE.MAX_PEAK;
	...
	/**
	 * Combines KLM labels into a minimal set of compressed labels For example "Fe
	 * Ka, Fe Kb, Ze La, Ze Lg" becomes "Fe K, Ze La+g"
	 * 
	 * @param label
	 * @return String
	 */
	...
	protected void drawKLMLine(Graphics2D gr, int xPos, int hgt, KLMLine.KLMLineType type, String str) {
		...
		if (str.length() > 0) {
			...
			try {
				final Rectangle2D rect = gr.getFontMetrics().getStringBounds(str, gr);
	...
	static private class EffectiveLine {
		...
		/**
		 * Is this KLM of the same type, element and within the energy tolerance of the
		 * KLM lines in this EffectiveLine collection?
		 * 
		 * @param klm
		 * @return
		 */
			...
			try {
				...
				while (effs.size() > 0) {
					...
					EffectiveLine maxLine = (xeffCenter >= mPlotRect.x)
							&& (xeffCenter < mMaxHeight.length + mPlotRect.x) ? eff : null;
					...
					if (maxLine != null) {
						...
						for (final EffectiveLine feff : fam) {
							...
							final double hh = (mPlotRect.y + mPlotRect.height)
									- ((mPlotRect.height * f * scale * feff.mAmplitude) / maxLine.mAmplitude);
	...
	/**
	 * Set the temporary KLM lines to the specified sorted list of KLMLine objects.
	 * 
	 * @param lines Collection&lt;KLMLine&gt;
	 */
	...
	protected void paintComponent(Graphics g, boolean forSvg) {
		...
		if ((mCurrentScalingMode != SCALING_MODE.PER_NA_EV) && (mCurrentScalingMode != SCALING_MODE.CONSTANT_FLUX)
		...
		t0 = 2;
		if (mShowHAxisLabels)
			h0 = (canvasHeight - (t0 + 4)) > (2 * fm.getHeight()) ? canvasHeight - (t0 + (2 * fm.getHeight()))
					: fm.getHeight();
		else
			h0 = canvasHeight - 5;
		...
		{
			if (mShowHAxisLabels) {
				final double step = optimalStepSize(mEMin, mEMax, fm.stringWidth("XXX"), mPlotRect.width)
						/ MIN_GRID_SCALE;
				final double min = ((int) ((mEMin / step) + 0.999)) * step;
				final double max = ((int) (mEMax / step)) * step;
				final NumberFormat fmt = step <= MIN_GRID_SCALE * 4.0 ? new HalfUpFormat("0.00")
				...
				dup.drawString(ENERGY_AXIS_LABEL_KEV,
						(int) (mPlotRect.x + (0.5 * (mPlotRect.width - fm.stringWidth(ENERGY_AXIS_LABEL_KEV)))),
		...
		case LINEAR: { // Draw the labels
			...
			if (showVAxis) {
				...
				dup.drawString(COUNTS_AXIS_LABEL, x, y);
		...
		case LOG: {
			...
			if (showVAxis) {
				final int x = fm.getHeight(),
						y = (int) (mPlotRect.y + (0.5 * (mPlotRect.height + fm.stringWidth(COUNTS_LOG_AXIS_LABEL))));
				final AffineTransform oldAf = dup.getTransform();
				dup.rotate(-0.5 * Math.PI, x, y);
				dup.drawString(COUNTS_LOG_AXIS_LABEL, x, y);
				dup.setTransform(oldAf);
			}
		}
			break;
		case SQRT: {
			...
			while (v < vMax) {
				if ((v > vMin) && (v > (vMax / 32.0))) {
					final int a = (mPlotRect.y + mPlotRect.height)
							- (int) (((v - vMin) * mPlotRect.height) / (vMax - vMin));
					if ((a > t0) && (a < (t0 + h0)))
						dup.drawLine(l0, a, l0 + w0, a);
			...
			if (showVAxis) {
				final int x = fm.getHeight(), y = (int) (mPlotRect.y
						+ (0.5 * (mPlotRect.height + fm.stringWidth(COUNTS_SQUARE_ROOT_AXIS_LABEL))));
				final AffineTransform oldAf = dup.getTransform();
				dup.rotate(-0.5 * Math.PI, x, y);
				dup.drawString(COUNTS_SQUARE_ROOT_AXIS_LABEL, x, y);
				dup.setTransform(oldAf);
			}
		}
			break;
					...
					if ((low < vMax) && (high > vMin)) {
						...
						gr.drawLine(i + mPlotRect.x + off,
								(int) ((mPlotRect.y + mPlotRect.height)
										- (((low - vMin) * mPlotRect.height) / (vMax - vMin))) - off,
	...
	/**
	 * autoScaleV - Autoscale the vertical axis to maxPercent of the range above the
	 * zeroOffset.
	 * 
	 * @param maxPercent double
	 */
	...
	/**
	 * autoScaleH - Autoscale the horizontal axis to minPercent of the range above
	 * the zeroOffset and maxPercent of the range above the zeroOffset.
	 * 
	 * @param minPercent double
	 * @param maxPercent double
	 */
	...
	private void endDrag(int x) {
		...
		if ((mDragStart != Integer.MIN_VALUE) && (Math.abs(mDragStart - x) >= MIN_DRAG_DELTA)) {
			if (mDragEnd != Integer.MIN_VALUE) {
				...
				gr.setXORMode(Color.green);
				gr.fillRect(mDragStart < mDragEnd ? mDragStart : mDragEnd, mPlotRect.y, Math.abs(mDragEnd - mDragStart),
						mPlotRect.height);
			...
			}
			{
				...
				String status = "E[low]  = " + nf.format(min) + " eV\nE[high] = " + nf.format(max) + " eV\nDelta   = "
						+ nf.format(max - min) + " eV";
				final List<ISpectrumData> data = getData();
				for (final ISpectrumData spec : data) {
					final long cx = Math.round(SpectrumUtils.sumCounts(spec, SpectrumUtils.channelForEnergy(spec, min),
	 ...
	 * 
	 * @param min Energy in eV
	 * @param max Energy in eV
	 */
	...
	/**
	 * zoomInBy - Zoom the vertical axis by the specified factor. d&gt;1.0 is zoom
	 * in and d&lt;1.0 is zoom out.
	 * 
	 * @param d double
	 */
	...
	static private String getZoomMenuItemText(double factor) {
		if (factor == 0.0)
			...
			return factor > 1.0 ? "Zoom in " + nf.format(factor) + " \u00D7"
	...
	public class JMenuItem_Zoom extends JMenuItem {
		 ...
		 * 
		 * @param factor 0.0 is zoom to region, 1.0 is zoom to all, 2.0 is zoom in by 2,
		 *               0.5 is zoom out by 2.0 etc.
		 */
			...
			addActionListener(new AbstractAction() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					Preferences.userNodeForPackage(SpecDisplay.class).put("Scaling",
							mMode != SCALING_MODE.REGION_INTEGRAL ? mMode.name() : SCALING_MODE.NONE.name());
				}
			});
		}
	}
	...
	/**
	 * getDefaultMenu - Get a default implementation of the spectrum display popup
	 * menu.
	 * 
	 * @return JPopupMenu
	 */
	public JPopupMenu getDefaultMenu() {
				...
				public void actionPerformed(ActionEvent e) {
					try {
						...
						final JFileChooser jfc = new JFileChooser(
								userPref.get(kBitmapDir, System.getProperty("user.home")));
						...
						jfc.addChoosableFileFilter(
								new SimpleFileFilter(new String[] { "tif", }, "Tagged Image File Format"));
						final SimpleFileFilter png = new SimpleFileFilter(new String[] { "png", },
					...
					public void actionPerformed(ActionEvent e) {
						try {
							...
							final JFileChooser jfc = new JFileChooser(
									userPref.get(kBitmapDir, System.getProperty("user.home")));
							jfc.addChoosableFileFilter(
									new SimpleFileFilter(new String[] { "eps", }, "Encapsulated Postscript"));
							jfc.setAcceptAllFileFilterUsed(false);
							final int option = jfc.showSaveDialog(SpecDisplay.this);
							if (option == JFileChooser.APPROVE_OPTION) {
								final File f = jfc.getSelectedFile();
								...
								final String yy = df
										.format((6.0 * SpecDisplay.this.getHeight()) / SpecDisplay.this.getWidth());
								gpf.generatePostscript(f,
										"eps enhanced color solid lw 2 size 6.0in, " + yy + "in \"Times-Roman\" 16");
								userPref.put(kBitmapDir, f.getParent());
							}
						} catch (final Exception e1) {
							ErrorDialog.createErrorMessage(SpecDisplay.this, "Error saving file", e1);
		...
		compareMenu.add(new JMenuItem_Compare(jButtonGroup_Scaling, SCALING_MODE.CONSTANT_FLUX));
		...
		compareMenu.add(new JMenuItem_Compare(jButtonGroup_Scaling, SCALING_MODE.PER_NA_EV));
		compareMenu.add(new JMenuItem_Compare(jButtonGroup_Scaling, SCALING_MODE.NONE));
	...
	public void backgroundCorrectedPeakIntegration() {
		...
		for (final Region r : mRegions.mList) {
			...
			final List<ISpectrumData> data = getData();
			for (final ISpectrumData sd : data) {
				final double[] bci = SpectrumUtils.backgroundCorrectedIntegral(sd, r.getLowEnergy(),
						r.getHighEnergy() - ENERGY_EPS);
				final double err = bci[1];
				final double sum = bci[2];
				final double bkg = bci[3];
				res += "\n" + nf.format(sum - bkg) + "\t" + nf.format(err) + "\t" + nf.format((sum - bkg) / err) + "\t"
						+ sd.toString();
			}
		}
	...
	public void peakIntegration() {
		...
		for (final Region r : mRegions.mList) {
			...
			for (final ISpectrumData sd : data) {
				...
				res += "\n" + nf.format(sum) + "\t" + nf.format(Math.sqrt(sum)) + "\t" + Integer.toString(maxCh - minCh)
						+ "\t" + sd.toString();
			}
		}
		setTemporaryAnnotation(res);
	...
	public void threeROIpeakIntegration() {
		if (mRegions.size() == 3) {
			...
			for (final ISpectrumData sd : getData()) {
				final double[] tmp = SpectrumUtils.backgroundCorrectedIntegral(sd, onPeak.getLowEnergy(),
						onPeak.getHighEnergy(), low.getLowEnergy(), low.getHighEnergy(), high.getLowEnergy(),
						high.getHighEnergy());
				res += "\n" + nf.format(tmp[0]) + "\t" + nf.format(tmp[1]) + "\t" + nf.format(tmp[0] / tmp[1]) + "\t"
						+ sd.toString();
			}
			setTemporaryAnnotation(res);
		} else
	...
	public Set<SumPeak> idSumPeak() {
		final int[] lines = new int[] { XRayTransition.KA1, XRayTransition.KB1, XRayTransition.LA1,
				XRayTransition.MA1 };
		...
		for (final ISpectrumData sd : data) {
			...
			for (final Region r : mRegions.mList) {
				...
				final int max = SpectrumUtils.bound(sd,
						SpectrumUtils.channelForEnergy(sd, r.getHighEnergy() - ENERGY_EPS));
				// for each data point in the region, write out the energy and
				// the count at that specific point
				for (int i = min; i < max; i++)
					IndividualSpectrumData.append(SpectrumUtils.minEnergyForChannel(sd, i) + '\t'
							+ SpectrumUtils.maxEnergyForChannel(sd, i) + '\t' + sd.getCounts(i) + '\n');
			}
	...
	public void writeDataInMultipleColumns(StringBuffer AllSpectrumData) {
		...
		for (final Region r : mRegions.mList) {
			...
			// Finds the end (right side) of the highlighted region
			final int max = SpectrumUtils.bound(sd0,
					SpectrumUtils.channelForEnergy(sd0, r.getHighEnergy() - ENERGY_EPS));
			// For each channel
			for (int i = min; i < max; i++) {
	...
	/**
	 * areAllVisableSpectrumTheSameWidth - This procedure checks if all the visible
	 * spectra are the same width. It returns true if they are all the same width,
	 * false if at least one is of a different width
	 * 
	 * @return boolean
	 */
	...
	public SpecDisplay() {
		...
		setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 10));
		{
			...
			try {
				setSpectrumScalingMode(SCALING_MODE.valueOf(userPref.get("Scaling", SCALING_MODE.MAX_PEAK.name())));
			} catch (final IllegalArgumentException e1) {
				setSpectrumScalingMode(SCALING_MODE.NONE);
			}
			try {
	 ...
	 * 
	 * @param sd ISpectrumData - The spectrum to add to the display list.
	 */
	...
	/**
	 * removeSpectrum - Remove the spectrum defined by the specified ISpectrumData
	 * from the display list.
	 * 
	 * @param sd ISpectrumData - The spectrum to remove.
	 */
	...
	/**
	 * Returns the number of spectra currently attached to this SpecDisplay object.
	 * Some may be hidden.
	 * 
	 * @return int
	 */
	...
	/**
	 * SetHRange - Set the horizontal range that is displayed within the plot area
	 * of the control.
	 * 
	 * @param min double - The minimum value in eV.
	 * @param max double - The maximum value in eV.
	 */
	...
	/**
	 * setSpectrumScalingMode - Sets the mode by which spectra are scaled relative
	 * to one another. (Valid modes are listed in ScalingModes)
	 * 
	 * @param mode SCALING_MODE
	 */
		...
		if ((mCurrentScalingMode != mode) || (mode == SCALING_MODE.REGION_INTEGRAL)) {
			...
			if (mCurrentScalingMode == SCALING_MODE.REGION_INTEGRAL) {
				...
				for (final Region r : mScalingRegions.mList) {
					sb.append(first ? "Scale to equal integral on " : ", ");
	...
	private double computeScale(ISpectrumData sd) {
		...
		double scale = 1.0;
		if (mCurrentScalingMode == SCALING_MODE.PER_NA_EV) {
			final SpectrumProperties sp = sd.getProperties();
			final double tmp = sd.getChannelWidth() * sp.getNumericWithDefault(SpectrumProperties.LiveTime, 60.0);
			scale = 1.0 / tmp;
		} else if (mCurrentScalingMode == SCALING_MODE.CONSTANT_FLUX) {
			final SpectrumProperties sp = sd.getProperties();
			...
			if (data.size() > 0) {
				final ISpectrumData sd0 = data.get(0);
				if (sd != sd0)
					if (mCurrentScalingMode == SCALING_MODE.INTEGRAL) {
						final double tc0 = SpectrumUtils.totalCounts(sd0, true) * sd0.getChannelWidth();
						if (tc0 > 0.0) {
							final double tc = SpectrumUtils.totalCounts(sd, true) * sd.getChannelWidth();
							scale = tc > 0.0 ? tc0 / tc : 1.0;
						}
					} else if (mCurrentScalingMode == SCALING_MODE.MAX_PEAK) {
						final int MIN_CHANNEL = 20;
						final double c = sd.getCounts(SpectrumUtils.maxChannel(sd, MIN_CHANNEL, sd.getChannelCount()));
						scale = c > 0.0
								? sd0.getCounts(SpectrumUtils.maxChannel(sd0, MIN_CHANNEL, sd0.getChannelCount())) / c
								: 1.0;
					} else if (mCurrentScalingMode == SCALING_MODE.REGION_INTEGRAL)
						if (mScalingRegions != null)
							for (final Region r : mScalingRegions.mList) {
								final double c0 = SpectrumUtils.integrate(sd0, r.getLowEnergy(),
										r.getHighEnergy() - ENERGY_EPS) * sd0.getChannelWidth();
								if (c0 > 0.0) {
									final double c = SpectrumUtils.integrate(sd, r.getLowEnergy(),
											r.getHighEnergy() - ENERGY_EPS) * sd.getChannelWidth();
									scale = c > 0.0 ? c0 / c : 1.0;
								}
							}
			}
		}
	...
	/**
	 * setAxisScalingMode - set the vertical axis scaling mode to one of the values
	 * in AxisModes.
	 * 
	 * @param mode AXIS_MODES
	 */
	 ...
	 * 
	 * @param dragEnabled The value to which to set dragEnabled.
	 */
	 ...
	 * 
	 * @param showImage true to show an image
	 */
	 ...
	 * 
	 * @param staggerOffset The value to which to set staggerOffset.
	 */
	...
	/**
	 * Specify the type of label to use on KLM lines (One of LONG_LABEL, SHORT_LABEL
	 * or LARGE_LABEL)
	 * 
	 * @param lt
	 */
