...
public class ExpQMBarrierSM implements BarrierScatterMechanism {
	 ...
	 * PI*lambda*Sqrt(m/2.)/hbar that multiplies the square roots of energies. Once
	 * lambda is given, this factor is precomputed and stored here.
	 */
	 ...
	 * of a wide barrier (i.e., a classical barrier scatter mechanism).
	 *
	 * @param mat - The material for which to construct it. If the provided argument
	 *            is an SEmaterial, the potential energy of an electron in this
	 *            material will be determined from the material properties.
	...
	public ExpQMBarrierSM(Material mat, double lambda) {
		...
		classical = false;
		this.lambda = lambda;
		lambdaFactor = (Math.PI * lambda * Math.sqrt(PhysicalConstants.ElectronMass / 2.))
				/ PhysicalConstants.PlanckReduced;
	}

	/**
	 * sharpBarrierT -- a private utility routine. It computes the transmission
	 * probability for a sharp (lambda = 0) barrier as a function of a dimensionless
	 * barrier height, reducedU. reducedU = deltaU/(kE*cos(theta)^2) where deltaU is
	 * the barrier height, kE is the kinetic energy of the electron, and theta is
	 * the angle its trajectory makes with the barrier. The more general formula
	 * (below) attempts to divide by 0 if lambda is 0, but its limit is well
	 * defined. This routine implements that limit. The formula is Prob =
	...
	public Electron barrierScatter(Electron pe, RegionBase nextRegion) {
		/*
		 * FIND THE STEP HEIGHT OF THE BARRIER. To do this correctly both materials need
		 * to be SEmaterials so we can get the relevant parameters. The most common
		 * non-SE material that we will encounter is vacuum because MonteCarloSS assigns
		 * this to the space outside the sample. For vacuum the potential energy step by
		 * definition is simply the -potentialU of the current material, so that's what
		 * we do. Other non-SE materials are treated the same.
		 */
		final Material nextmaterial = nextRegion.getMaterial();

		/*
		 * We must discover which shape we have intersected. If nextRegion is not a
		 * subregion of the current one, then our intersection is with a boundary of the
		 * current region. If it IS a subregion, then our intersection is with it or
		 * with one of its containing regions.
		 */
		...
		if (currentRegion.isContainingRegion(nextRegion)) {
			...
			/*
			 * Sometimes we cross multiple boundaries at once. The while loop checks and
			 * corrects for this.
			 */
		...
		/*
		 * If the intersected shape is not a NormalShape, we still haven't initialized
		 * nb. We have no data in this case, so we must make do with an arbitrary
		 * assignment: Let nb be the same as the electron direction. This choice gives
		 * maximum transmission probability and no deflection of the electron's path.
		 */
		if (nb == null)
			nb = n0;

		/*
		 * Let the angle of incidence be called alpha. Cos(alpha) is given by the dot
		 * product
		 */
		...
		if (cosalpha <= 0.) {
			/*
			 * This case corresponds to the electron "hitting" the barrier while moving away
			 * from it. I.e., it didn't really hit the barrier. This can happen, e.g., if
			 * electric field alters the electron's direction of motion. We give it a nudge
			 * away from the barrier towards the inside
			 */
			final double[] pos0 = pe.getPosition();
			pe.setPosition(new double[] { pos0[0] - (MonteCarloSS.SMALL_DISP * nb[0]),
					pos0[1] - (MonteCarloSS.SMALL_DISP * nb[1]), pos0[2] - (MonteCarloSS.SMALL_DISP * nb[2]) });
		...
		if (deltaU == 0.) {
			/*
			 * This corresponds to no barrier. This is usually due to a mathematical
			 * boundary with the same material on both sides. It transmits, so we give it a
			 * nudge off of the barrier toward the outside, update the electron's region,
			 * and return.
			 */
			final double[] pos0 = pe.getPosition();
			pe.setPosition(new double[] { pos0[0] + (MonteCarloSS.SMALL_DISP * nb[0]),
		...
		if ((perpE == 0.) || (perpE <= deltaU))
			/*
			 * Even if deltaU<0 (the electron is stepping downhill) the quantum mechanical
			 * formula gives transmission = 0 when perpE = 0.
			 */
			transmits = false;
		else {
		...
		/*
		 * COMPUTE FINAL DIRECTION AND ENERGY FOR EACH OF THE CASES: TRANSMISSION OR
		 * REFLECTION
		 */
		...
		if (transmits) { // Transmission
			...
			/*
			 * Following is the numerator part of the 3 components of the new direction
			 * vector. All components would need to be divided by the same factor, for
			 * normalization, but we don't bother because we don't need it, except for the z
			 * component.
			 */
			for (int i = 0; i < 3; i++)
			 ...
			 * directions nearly parallel to the interface, have positions on the wrong side
			 * of their boundary. To prevent this we displace the electron position 1 pm
			...
			pe.setPosition(new double[] { pos0[0] + (MonteCarloSS.SMALL_DISP * nb[0]),
					pos0[1] + (MonteCarloSS.SMALL_DISP * nb[1]), pos0[2] + (MonteCarloSS.SMALL_DISP * nb[2]) });
			/*
			 * TODO On rare occasions, such as when our electron is within SMALL_DISP of a
			 * corner, the above displacement can move the electron outside its current
			 * region.
			 */
	 ...
	 *         material and lambda are the parameters supplied to the constuctor and
	 *         u0 is the barrier height (to vacuum).
	 */
	@Override
