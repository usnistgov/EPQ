...
public class EMSAFile extends BaseSpectrum {
	 ...
	 * file @throws IOException throws an IOException if there is an error reading
	 * the file
	 */
	...
	private boolean storeData(String prefix, String value) {
		...
		try {
			...
			// not necessary
				...
				if (val > 0.0) {
					mProperties.setNumericProperty(SpectrumProperties.FaradayBegin, val);
					mProperties.setNumericProperty(SpectrumProperties.FaradayEnd, val);
				}
	...
	private void updateSampleOrientation() {
		final double[] normal = new double[] { Math.sin(mXTilt), Math.cos(mXTilt) * Math.sin(mYTilt),
