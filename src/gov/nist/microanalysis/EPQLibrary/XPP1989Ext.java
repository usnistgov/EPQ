 ...
 * @version 1.0
 */
public class XPP1989Ext extends XPP1991 {

	static private double MIN_TILT = Math.toRadians(1.0);

	static private final BookChapter PAP_IXCOM12 = new BookChapter(
			new Book("Proceedings of the 12th International Congress on X-Ray Optics and Microanalysis", "", 1991,
					new Author[] {}),
			"p52-59", new Author[] { LitReference.JPouchou, LitReference.FPichoir,
	 ...
	 */
	static public class XPPTiltBackscatterFactor extends BackscatterFactor {
	 ...
	 * algorithm with the extensions necessary to handle samples oriented at a tilt
	...
	public boolean initialize(Composition comp, AtomicShell shell, SpectrumProperties props) throws EPQException {
		...
		if (res) {
			...
			if (beta > MIN_TILT) {
				final ProportionalIonizationCrossSection icx = (ProportionalIonizationCrossSection) getAlgorithm(
						ProportionalIonizationCrossSection.class);
				...
				mF = (cosBeta * TiltedBackscatterFactor.compute(mComposition, mShell, mBeamEnergy)
				...
				final double meanZb = PAP1991.papMeanAtomicNumber(mComposition);
				{
					...
					rBar = mF / (1.0
							+ ((x * Math.log(1.0 + (y * (1.0 - (1.0 / Math.pow(u0, 0.42)))))) / Math.log(1.0 + y)));
				...
				{
					final double g = 0.22 * Math.log(4.0 * meanZb)
							* (1.0 - (2.0 * Math.exp((-meanZb * (u0 - 1.0)) / 15.0)));
					...
					mBigB = ((mLittleB * mLittleB * mF * (1.0 + mEps)) - pTilt - (mPhi0 * mLittleB * (2.0 + mEps)))
							/ mEps;
