...
public class GmshMesh
   ...
   public GmshMesh(String meshFileName)
         throws FileNotFoundException {
      ...
      try {
         /*
          * I've chosen to make this import very strict. Any deviation from
          * expectations generates a fatal error. TODO It may be too strict,
          * since there is supposed to be an ability to insert comments by
          * having unrecognized sections, e.g., $Comments...$EndComments, but
          * this reader will throw an error if you try.
          */
         ...
         for(int i = 1; i <= numElements; i++) {
            ...
            /*
             * Format 2.1 has a last unused tag that we'll use later. 2.2 does
             * not, so we add a tag to our internal format for that one.
             */
            ...
            switch(type) {
               ...
               default:
                  if((type >= 1) && (type <= 31))
                     throw new EPQFatalException("Element " + Integer.toString(i) + " is unimplemented type "
         ...
         /*
          * Move the lists of node adjacent volumes to arrays to save space, and
          * sort them.
          */
         for(int i = 1; i <= numNodes; i++) {
   ...
   /**
    * Returns an extended list of volumes adjacent to Tetrahedron #tetIndex
    * through the indexed face. Extended means volumes that share only an edge
    * or a node of this face are returned in addition to the volume that shares
    * the entire face.
    */
   private ArrayList<FrequencyCounter> extendedAdjacentVolumeList(int tetIndex, int faceIndex) {

      /*
       * The 3 nodes of this face have 3 corresponding lists of adjacent
       * volumes. Get arrays for these and take the first volume off of each
       * list.
       */
      /* 3 indices of the 3 nodes of this face */
      final int[] facenodeindices = tetFaceNodeIndices(tetIndex, faceIndex);

      final int[][] nodeAV = new int[][] {
         nodeAdjacentVolumes[facenodeindices[0]],
         nodeAdjacentVolumes[facenodeindices[1]],
         nodeAdjacentVolumes[facenodeindices[2]]
      };

      final int[] currentValue = new int[3];
      final boolean[] validValue = {
         false,
         false,
         false
      };
      ...
      while(numValidValues > 0) {
         ...
         if(index < nodeAV[smallestIndex].length) {
            ...
            /*
             * If no valid value, mark this list finished and decrement the
      ...
      /*
       * Sort our list. The natural order for FrequencyCounter objects is to put
       * high frequency objects first. This puts the most likely adjacent
       * elements at the head of our list.
       */
    ...
    * tetrahedron, there is at most one other tetrahedron in the mesh that
    * shares this face. The index of that tetrahedron is found, if it exists,
    * stored in the indicated array location, and true is returned. If the
    * identified face does not have a neighboring tetrahedron (i.e., because it
    * is on the boundary of the mesh), false is returned.
    *
    * @param tetIndex - Index of a tetrahedron
    * @param faceIndex - Index (0-3) of one of the faces of this tetrahedron
    * @return - true if the adjacent volume exists, false if it doesn't (i.e.,
    *         if this face is on the boundary of the mesh).
    */
   ...
   public int[] tetFaceNodeIndices(int tetIndex, int faceIndex) {
      switch(faceIndex) {
         ...
         case 1:
            return new int[] {
               elementsNodeIndices[tetIndex][0],
               elementsNodeIndices[tetIndex][3],
               elementsNodeIndices[tetIndex][2]
            };
         case 2:
   ...
   /**
    * A utility to compute the tetrahedron volume from the positions of its
    * nodes. It uses the formula, V = a . (b x c)/6, where a, b, and c are
    * vectors representing the 3 edges emerging from any vertex.
    *
    * @param nodeIndices
    * @return
    */
