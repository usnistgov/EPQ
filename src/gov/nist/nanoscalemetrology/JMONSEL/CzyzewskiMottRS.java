...
public class CzyzewskiMottRS extends RandomizedScatter {
	...
	/**
	 * MottScatteringAngle - Creates a MottScatteringAngle object for the specified
	 * element.
	 * 
	 * @param el Element
	 */
	...
	/**
	 * scatteringAngle - Given a random number, rand on the interval [0,1), and an
	 * energy within the tabulated interval (20 eV to 30 keV but expressed in
	 ...
	 * totalCrossSection(MIN_CZYZEWSKI); } else if (energy <= MAX_CZYZEWSKI) { int e
	 * = CzyzewskiMottCrossSection.getEnergyIndex(energy); if(e == 0) e = 1; final
	 * double e0 = CzyzewskiMottCrossSection.getSpecialEnergy(e - 1); final double
	 * e1 = CzyzewskiMottCrossSection.getSpecialEnergy(e); assert (energy >= e0);
	 * assert (energy <= e1); return mMeanFreePath[e - 1] + ((mMeanFreePath[e] -
	 * mMeanFreePath[e - 1]) * ((energy - e0) / (e1 - e0))); } else { if(mRutherford
	 * == null) mRutherford = new ScreenedRutherfordScatteringAngle(mElement);
	 * return
	 * meanFreePath(MAX_CZYZEWSKI)*mRutherford.totalCrossSection(energy)/mRutherford
	 * .totalCrossSection(MAX_CZYZEWSKI); } }
	 */
