...
/**
 * Reads the Bruker file format associated with hand-held XRF units. Reverse
 * engineered by inspection of the binary file format.
 * 
 * @author nritchie
 *
 */
public class BrukerPDZ extends BaseSpectrum {
   ...
   private final short readShort(DataInputStream dis) throws IOException {
      return ByteBuffer.allocate(Short.BYTES).order(ByteOrder.BIG_ENDIAN).putShort(dis.readShort()).order(ByteOrder.LITTLE_ENDIAN).getShort(0);
   }

   private final int readInt(DataInputStream dis) throws IOException {
      return ByteBuffer.allocate(Integer.BYTES).order(ByteOrder.BIG_ENDIAN).putInt(dis.readInt()).order(ByteOrder.LITTLE_ENDIAN).getInt(0);
   }

   private final double readDouble(DataInputStream dis) throws IOException {
      return ByteBuffer.allocate(Double.BYTES).order(ByteOrder.BIG_ENDIAN).putDouble(dis.readDouble()).order(ByteOrder.LITTLE_ENDIAN).getDouble(0);
   }

   private final double readFloat(DataInputStream dis) throws IOException {
      return ByteBuffer.allocate(Float.BYTES).order(ByteOrder.BIG_ENDIAN).putFloat(dis.readFloat()).order(ByteOrder.LITTLE_ENDIAN).getFloat(0);
   }
