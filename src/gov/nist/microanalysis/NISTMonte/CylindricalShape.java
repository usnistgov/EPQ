...
public class CylindricalShape implements MonteCarloSS.Shape, ITransform, TrajectoryVRML.IRender, Cloneable {
	...
	final private double closestPointOnAxis(double[] p) {
		return ((mDelta[0] * (p[0] - mEnd0[0])) + (mDelta[1] * (p[1] - mEnd0[1])) + (mDelta[2] * (p[2] - mEnd0[2])))
				/ mLen2;
	}

	/**
	 * distanceSqr - Distance (squared) from the parameterized point on the cylinder
	 * axis to the specified point.
	 * 
	...
	public double[] getEnd1() {
		return new double[] { mEnd0[0] + mDelta[0], mEnd0[1] + mDelta[1], mEnd0[2] + mDelta[2] };
	}
	...
	public void render(TrajectoryVRML.RenderContext vra, Writer wr) throws IOException {
		...
		// r is the cross product (1,0,0) x norm(mDelta)
		{
			...
			assert (dm > 0.0);
			final double[] r = { mDelta[2] / dm, 0.0, -mDelta[0] / dm, };
