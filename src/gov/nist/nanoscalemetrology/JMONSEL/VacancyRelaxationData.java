...
public class VacancyRelaxationData {
    ...
    * from atomconf.tab (from inner-shell calculations of Bote &amp; Salvat
    * 2008).
    * 
    * @param z0
    *           Atomic number.
   ...
   public void readIonizationEnergiesFromDatabase(int z0) {
      ...
      /*
       * I maintain the subshell labeling of the database (starting at 1): K
       * (1), L1 (2), L2 (3), L3 (4), M1 (5), ...
       * 
      ...
      try {
         try (InputStream is = this.getClass().getResourceAsStream("atomconf.tab")) {
            try (BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(is))) {
               while ((line = bufferedReader.readLine().trim()) != null) {
                  if (line.charAt(0) != 'C') // Ignore header lines commented
                                             // with 'C'.
                  {
                     String[] words = line.split("\\s+");
                     if (Integer.parseInt(words[0]) > z0) // No need to scan
                                                          // file past the
                                                          // requested z0.
                        break;
                     else if (Integer.parseInt(words[0]) == z0) {
                        double ebindEV = Double.parseDouble(words[8]); // DHFS
                                                                       // (6),
                                                                       // CARLSON
                                                                       // (7),
                                                                       // WILLIAMS
         ...
         maxSubshell = bindingEnergies.size() - 1; // Keep track of the highest
                                                   // subshell in the database.
    ...
    * method calculates the probability that the vacancy decays via a radiative
    * process (emission of fluorescence photons). We currently do not follow
    * fluorescence processes in this class, only their frequency of occurrence
    * so as not to mistakenly assume that all inner-shell ionizations decay via
    * the emission of an Auger electron (this would yield an excess of Augers).
    * </p>
    * <p>
   ...
   public String determineRadiativeProbability(int z0) {
      ...
      /*
       * In order to maintain the subshell indexing of the Penelope files, K
       * (1), L1 (2), L2 (3), L3 (4), M1 (5), ..., I add a fake 0-th element
      ...
      for (int i = 1; i <= maxSubshell; i++) {
         ...
         try {
            /*
             * Read pdrelax.p11 line by line and accumulate all
             * radiative-process probabilities for the atomic number z0 and the
             * shell index i.
             */
            try (InputStream is = this.getClass().getResourceAsStream("pdrelax.p11")) {
               try (BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(is))) {
                  ...
                  while ((line = bufferedReader.readLine().trim()) != null) {
                     ...
                     if (zz > z0)
                        break;
                     else if (zz == z0) { // Consider only the requested atomic
                                          // number.
                        int s0 = Integer.parseInt(words[1]);
                        int s3 = Integer.parseInt(words[3]);
                        if (s0 == i && s3 == 0) {
                           // If the vacancy subshell is right and the line
                           // corresponds
                           // to a radiative transition, accumulate the
                           // probability.
   ...
   public void readAugerProbabilities(int z0) {
      ...
      for (int i = 1; i <= maxSubshell; i++) {
            ...
            try (InputStream is = this.getClass().getResourceAsStream("pdrelax.p11")) {
               try (BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(is))) {
                  ...
                  while ((line = bufferedReader.readLine().trim()) != null) {
                     String[] words = line.split("\\s+");
                     zz = Integer.parseInt(words[0]);
                     if (zz > z0) // No need to scan the file past the requested
                                  // z0.
                        break;
                     else if (zz == z0) {
                        int s0 = Integer.parseInt(words[1]);
                        int s3 = Integer.parseInt(words[3]);
                        if (s0 == i && s3 != 0) {
                           // Save the energy and probability if the line
                           // corresponds to an Auger
         ...
         /*
          * Calculate cumulative probabilities to ease the sampling of the
          * transition. Here I do keep the indexing starting from zero.
          */
         double sumProb = 0.0;
         for (int j = 0; j < augersFromSubshell.probability.size(); j++) {
            ...
            if (j > 0)
               augersFromSubshell.probability.set(j, augersFromSubshell.probability.get(j - 1) + augersFromSubshell.probability.get(j));
         }
   ...
   /**
    * Given an energy loss and a binding energy, this method determines which
    * shell the binding energy corresponds to (e.g. L) and which of the
    * subshells are ionizable given the energy loss (e.g. L2 and L3). Then, it
    * decides whether the vacancy decayed via the emission of a fluorescence
    * photon (returning a negative value) or an Auger electron (returning the
   ...
   public double augerEnergy(double eBind, double eLoss) {
      ...
      /*
       * Determine subshell with binding energy closest to the passed value. In
       * future stages (user indicating which subshells are accessible instead
       * of binding energies) this may be redundant.
       */
      ...
      // Consider other subshells in the same shell that could have been
      // ionized.
       ...
       * System.out.println("Possible vacancies:"+possibleSubshellVacancies.
       * toString() );
       */

      // Decide the ionized subshell (so far homogeneously, later using
      // inner-shell
      // ionization cross sections).
      /*
       * There may be some ionization energies in atomconf.tab for which we have
       * no relaxation data in pderelax.p11. I therefore chose to loop until
       * valid data are found.
       */
      int nsteps = 0;
      int ntrans0 = 0;
      int vacancySubshell;
