 ...
 * @version 1.0
 */
public class Armstrong1982ParticleCorrection extends Armstrong1982Base {
	...
	public interface Shape {
		 ...
		 * @throws EPQException
		 */
		double getThickness() throws EPQException;
	 ...
	 */
	public final class BulkShape implements Shape {
		public BulkShape() {
		}

		@Override
		public double getThickness() throws EPQException {
	...
	public final class RightRectangularPrism implements Shape {
		 ...
		 * 
		 * @see gov.nist.microanalysis.EPQLibrary.Armstrong1982ParticleCorrection.Shape
		 * #getDiameter()
		 */
		@Override
		public double getDiameter() {
		...
		public double computeP(double rhoZ) {
			...
			if (mSimpleH) {
				...
				if (rhoZ <= (0.5 * mRhoD * Math.tan(mTakeOffAngle))) // ok
					p = (1.0 / mRhoD) * ((((0.75 * mRhoD) - (0.5 * beta) - ((0.5 * beta * beta) / mRhoD)
							- (beta / (mAlphaPC * mRhoD))) * expNegChi) + ((0.5 * (1.0 - expNegChi)) / mAlphaPC)
							+ ((1.0 - expNegChi) * invAArD));
				else if (rhoZ <= (mRhoD * Math.tan(mTakeOffAngle)))
					p = (1.0 / mRhoD)
							* (((((mRhoD - (1.5 * beta)) + ((0.5 * beta * beta) / mRhoD) + (beta / (mAlphaPC * mRhoD)))
									- (1.0 / mAlphaPC)) * expNegChi) + (0.5 * ((1.0 - expNegChi) / mAlphaPC))
									+ (((1.0 - expNegChi) * invAArD)
											- (2.0 * Math.exp(-0.5 * mAlphaPC * mRhoD) * invAArD)));
				else
					p = (1.0 / mRhoD) * (((0.5 * (1.0 - Math.exp(-mAlphaPC * mRhoD))) / mAlphaPC) + (invAArD
							* ((1.0 + Math.exp(-mAlphaPC * mRhoD)) - (2.0 * Math.exp(-0.5 * mAlphaPC * mRhoD)))));
			}
			return p;
		}
	}
	...
	public final class TetragonalPrism implements Shape {
		...
		public String toString() {
			final NumberFormat df = new HalfUpFormat("0.00 \u00B5m");
			return "TetragonalPrism[Thickness=" + df.format(mThickness * 1.0e6) + ", Diameter="
					+ df.format(mDiameter * 1.0e6) + "]";
		}
		...
		public double computeP(double rhoZ) {
			...
			if (rhoZ <= (mRhoD * Math.tan(mTakeOffAngle)))
				p = (4.0 / (mRhoD * mRhoD)) * (((0.5 * (expNegChi - 1.0)) / (mAlphaPC * mAlphaPC))
						+ ((0.5 * beta) / mAlphaPC) + (Math2.sqr(0.5 * lambda) * expNegChi)
						+ (((0.5 * lambda) / mAlphaPC) * (1.0 - expNegChi)));
			else
	...
	public final class TriangularPrism implements Shape {
		...
		public String toString() {
			final NumberFormat df = new HalfUpFormat("0.00 \u00B5m");
			return "TriangularPrism[Thickness=" + df.format(getThickness() * 1.0e6) + ", Diameter="
					+ df.format(mDiameter * 1.0e6) + "]";
		}
		...
		public double computeP(double rhoZ) {
			...
			if (GREEN_BOOK) {
				...
				p = (((0.5 * (egrz - Math.exp(-mGammaPC * (mRhoD - rhoZ)))) / mGammaPC)
						+ (((0.5 * mRhoD) - rhoZ) * egrz)) / mRhoD;
			}
			return p;
		}
	}
	...
	public final class SquarePyramid implements Shape {
		...
		public String toString() {
			final NumberFormat df = new HalfUpFormat("0.00 \u00B5m");
			return "SquarePyramid[Thickness=" + df.format(getThickness() * 1.0e6) + ", Diameter="
					+ df.format(mDiameter * 1.0e6) + "]";
		}
		...
		public double computeP(double rhoZ) {
			...
			// Checked 4-Mar-2009
			p = (1.0 / (mRhoD * mRhoD)) * ((((zeta / mGammaPC) + (0.5 * zeta * zeta)) * Math.exp(-mGammaPC * rhoZ))
					+ ((Math.exp(-xi) - Math.exp(-mGammaPC * rhoZ)) / (mGammaPC * mGammaPC)));
			return p;
		}
	}

	// Armstrong's model #5
	public final class VerticalCylinder implements Shape {
		...
		public String toString() {
			final NumberFormat df = new HalfUpFormat("0.00 \u00B5m");
			return "VerticalCylinder[Thickness=" + df.format(mThickness * 1.0e6) + ", Diameter="
					+ df.format(mDiameter * 1.0e6) + "]";
		}
		...
		public double computeP(double rhoZ) {
			...
			final Integrator i = new Integrator(TOLERANCE) {
				...
				public double getValue(double ry) {
					assert ((mRhoD * mRhoD) - (4.0 * ry * ry)) >= 0.0
							: Double.toString((mRhoD * mRhoD) - (4.0 * ry * ry));
			...
			if (rhoZ <= (mRhoD * Math.tan(mTakeOffAngle))) {
				...
				p = (4.0 / (Math.PI * mRhoD * mRhoD))
						* ((AA + (2.0 * expNegChi * BB)) - (2.0 * i.integrate(sbl, 0.5 * mRhoD)));
			} else
	...
	// Armstrong's model #6
	public final class HorizontalCylinder implements Shape {
		...
		public double computeP(final double rz) {
			...
			final Integrator i = new Integrator(TOLERANCE) {
				...
				public double getValue(double rx) {
					final double r = (((rz * (mRhoD - (sinPsi * ((2.0 * cosPsi * rx) + (mRhoD * sinPsi)))))
							- Math2.sqr(cosPsi * rz) - Math2.sqr(sinPsi * rx)) + (mRhoD * cosPsi * sinPsi * rx))
				...
				public double getValue(final double rhoy) {
					final Integrator inner = new Integrator(TOLERANCE) {
						...
						public double getValue(double rhox) {
							...
							final double rhow = Math.sqrt((0.25 * mRhoD * mRhoD) - (rhox * rhox) - (rhoy * rhoy))
									- rhoz;
							final double rhow2 = Math.sqrt((0.25 * mRhoD * mRhoD) - (rhox * rhox) - (rhoy * rhoy))
									- rhoz - (rhox * tanGamma);
	...
	public class Sphere implements Shape {
		...
		/**
		 * Generic constructor to implement a sphere, hemisphere or any vertical slice
		 * of a sphere. ie. The top <code>frac</code> of a sphere.
		 * 
		 * @param dia
		 * @param frac
		 */
						...
						final Integrator inner = new Integrator(TOLERANCE) {
							...
							public double getValue(double rx) {
								final double rad = (((0.25 * mRhoD * mRhoD) - (rx * rx)) * Math2.sqr(sinPsi))
										+ ((rz * (mRhoD - rz) * Math2.sqr(cosPsi))
												+ ((rx * ((2.0 * rz) - mRhoD) * sinPsi * cosPsi) - (ry * ry)));
	...
	// Armstrong models #7 & #8
	public class Sphere3 implements Shape {

		private final class SphereXIntegrate extends Integrator {
			...
			public double getValue(double rx) {
				...
				assert ((rx * rx) + (mRy * mRy) + Math2.sqr(rzp - (0.5 * mRhoD))) <= ((0.25 * mRhoD * mRhoD) + 1.0e-14)
						: Double.toString(rx) + ", " + Double.toString(mRy) + ", " + Double.toString(rzp) + ", "
		...
		private final class SphereYIntegral extends Integrator {
		...
		public double computeP(final double rz) {
			...
			if (alpha2 > 0.0) {
				...
				final double res = (new SphereYIntegral(rz)).integrate(-alpha, alpha)
						/ (0.25 * Math.PI * mRhoD * mRhoD);
				if (Double.isNaN(res))
	...
	/**
	 * Demonstrates that it is possible to perform the direct numerical integration
	 * and get the same result.
	 * 
	 * @author nritchie
	 */
	public class IntegratedRRP implements Shape {
		...
		public String toString() {
			final NumberFormat df = new HalfUpFormat("0.00 \u00B5m");
			return "IntegratedRRP[Thickness=" + df.format(mThickness * 1.0e6) + ", Diameter="
					+ df.format(mDiameter * 1.0e6) + "]";
		}
	};

	// Armstrong's model #5 (Using a numerical integral)
	public final class IntegratedCylinder implements Shape {
	...
	/**
	 * The algorithm for computing bulk corrections if USE_ANALYTICAL_BULK is true.
	 */
		...
		if ((mShape != null) && Double.isNaN(rho))
			if (mShape instanceof BulkShape)
				...
				throw new EPQException(
						"Please specify a specimen density.  The particle correction algorithm requires a density.");
		mRhoPC = FromSI.gPerCC(rho);
		return res;
	}

	public class ParticleCorrection extends Integrator {

		private final Shape mShape;

		public ParticleCorrection(Shape s) throws EPQException {
			super(TOLERANCE);
			mShape = s;
		}
	 ...
	 * 
	 * @param ss
	 * @return true if the specified SampleShape devived class is supported; false
	 *         otherwise
	 */
	@Override
	public boolean supports(Class<? extends SampleShape> ss) {
		return (ss == SampleShape.Bulk.class) || (ss == SampleShape.SquarePyramid.class)
				|| (ss == SampleShape.Sphere.class) || (ss == SampleShape.Cylinder.class)
				|| (ss == SampleShape.Fiber.class) || (ss == SampleShape.Hemisphere.class)
	...
	public double particleAbsorptionCorrection(XRayTransition xrt) throws EPQException {
		...
		final double maxThickness = FromSI.cm(
				(1.5 * ElectronRange.KanayaAndOkayama1972.compute(mComposition, mBeamEnergy)) / ToSI.gPerCC(mRhoPC));
		// See p 273 (29)
