...
public class PAP1991 extends CorrectionAlgorithm.PhiRhoZAlgorithm {
    ...
    * 
    * @param shell
    *           The AtomicShell for which to return the exponent
    * @return double
    */
   @Override
      ...
      if (res) {
         ...
         final double u0 = mBeamEnergy / mShell.getEdgeEnergy(); // units ok
         {
            ...
            final double zBar = mComposition.weightAvgAtomicNumber();
            {
               ...
               final double q0 = 1.0 - (0.535 * Math.exp(-Math.pow(21.0 / zBarN, 1.2))) - (2.5e-4 * Math.pow(zBarN / 20.0, 3.5));
   ...
   public double computeZACorrection(XRayTransition xrt) throws EPQException {
      ...
      final double f1 = (mA1 / chi) * ((((((mRc - mRm) * (mRx - mRc - (2.0 / chi))) - (2.0 / chi2)) * Math.exp(-chi * mRc)) - ((mRc - mRm) * mRx))
            + (mRm * (mRc - (2.0 / chi))) + (2.0 / chi2));
      final double f2 = (mA2 / chi)
            * (((((mRx - mRc) * (mRx - mRc - (2.0 / chi))) + (2.0 / chi2)) * Math.exp(-chi * mRc)) - ((2.0 / chi2) * Math.exp(-chi * mRx)));
      return toSI(f1 + f2);
   }
