 ...
 * @version 1.0
 */
public class Armstrong1982Base
   extends ZAFCorrectionAlgorithm {

   static private final LitReference RefBrownJTA = new LitReference.BookChapter(LitReference.MicrobeamAnalysis, "175-180", new Author[] {
   ...
   public boolean initialize(Composition comp, AtomicShell shell, SpectrumProperties props)
      ...
      if(res) {
         ...
         mAlpha = ((2.97e5 * Math.pow(z_bar, 1.05)) / (a_bar * Math.pow(e0keV, 1.25)))
               * Math.sqrt((Math.log((1.166 * e0keV) / w) / (e0keV - eCkeV)));
   ...
   public double generated(XRayTransition xrt) {
      ...
      final double generated = (Math.sqrt(Math.PI) * mGamma0 * 0.5 * (1.0 - (Math.exp(xx * xx) * mQ * Math2.erfc(xx))))
            / mAlpha;
