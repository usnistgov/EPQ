...
public class Math2 {
    ...
    * 
    * @param seed
    *           Seed for {@link Random} constructor
    */
   ...
   public static double chiSquaredConfidenceLevel(double confidence, int degreesOfFreedom) {
      ...
      fr.initialize(new double[]{degreesOfFreedom, confidence});
   ...
   public static final double gammaln(double xx) {
      ...
      final double[] coeff = {76.18009172947146, -86.50532032941677, 24.01409824083091, -1.231739572450155, 0.1208650973866179e-2,
            -0.5395239384953e-5};
      double y = xx;
      double tmp = xx + 5.5;
   ...
   public static double[] cubicSolver(double a, double b, double c, double d) {
      ...
      if ((f == 0.0) && (g == 0.0) && (h == 0.0)) {
         ...
         return new double[]{(2 * j * m) + p, (-j * (m + n)) + p, (-j * (m - n)) + p};
      } else {
   ...
   static final public double[] solvePoly(double[] coeff, double y) throws EPQException {
    ...
    * @param x
    * @param lowerInc
    *           where lowerInc &lt; upperExc
    * @param upperExc
    *           where upperExc &gt; lowerInc
    ...
    */
   public static double[] solveQuadratic(double a, double b, double c) throws EPQException {
      return solvePoly(new double[]{c, b, a});
   }
   ...
   public static double[] solveCubic(double a, double b, double c) {
      ...
      if ((r * r) < (q * q * q)) {
         ...
         return new double[]{(-2.0 * q * Math.cos(th / 3.0)) - (a / 3.0), (-2.0 * q * Math.cos((th + (2.0 * Math.PI)) / 3.0)) - (a / 3.0),
   ...
   static public double findRoot(double[] coeffs, double x1, double x2, double xacc) throws EPQException {
      final int MAXIT = 100;
   ...
   static public double[] y3(double y) {
      return new double[]{0.0, y, 0.0};
   }

   static public double[] z3(double z) {
      return new double[]{0.0, 0.0, z};
   }
