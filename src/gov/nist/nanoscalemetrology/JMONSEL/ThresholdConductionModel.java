...
public class ThresholdConductionModel implements IConductionModel {

   private double frac;
   private final int maxIterations = 40;
   private int feaInterval = 1; /*
                                 * Every feaInterval iterations, potentials
                                 * update is via FEA. Otherwise it is via a
                                 * faster more approximate method.
                                 */
    ...
    *           to flow
    */
   public ThresholdConductionModel(double frac) {
      this.frac = frac;
   }

   /**
    * Flows charges within the supplied MeshedRegion until the effective
    * electric fields become equal or smaller in magnitude to this model's
    * threshold value (supplied by the user at construction or via setFrac).
    * "Effective" electric field means corrections to include internal repulsion
    * of charges within a region but exclude the self-energy of the moving
    * charge are made. Charge flow for fields above the threshold is assumed to
    * be so fast that equilibrium is reached. Therefore the second argument
   ...
   public boolean conductCharges(MeshedRegion thisMeshedRegion, double deltat, IFEArunner feaRunner) {
      ...
      /*
       * TODO The following is taking a lot of iterations. The number of
       * iterations is smaller when p=1 than when p=2 (p the exponent of E field
       * in the conductivity equation. A possible faster algorithm would use the
       * following logic: Inside the below iterNum loop, I could flow charges,
       * then begin a loop that estimates (without FEA) the potentials due to
       * the flow and then flows charges again. This loop would repeat nFast
       * times. Upon exit, an FEA is done (as now) and then we return to the top
       * of the existing loop. Since nFast loops don't require FEA they are fast
       * compared to the existing loops. In this way I could move nFast times as
       * much charge per FEA, but with less accuracy. I'd have to pick nFast
       * based on a speed/accuracy compromise. It's a sort of algorithm tuning
       * parameter. To do this I should put the flow charge and estimate new
       * potentials codes in separate methods. I'd need to think about what
       * tuning data I need to record to assess the accuracy.
       */
      ...
      for (int iterNum = 0; iterNum < maxIterations; iterNum++) {
         ...
         if (currentSolnIsFEA) {
            ...
            for (final Map.Entry<TetFace, Integer> flow : flowSet) {
               ...
               /*
                * Add these, correcting the current charge count for the charge
                * move we just made, in order to record the correct initial
                * charge.
                */
   ...
   /* End debug */
   /**
    * Flows at most 1 charge per region. Returns true is any charges were moved,
    * false if not.
    *
    * @return
    */
   private boolean flowChargesOnce(MeshedRegion thisMeshedRegion, HashMap<TetFace, Integer> flowMap, ChargeFlowDebugInfo cfDebug) {
      ...
      /*
       * If current flow is a fraction of one charge, this algorithm uses a
       * random number between 0 and 1 to flow or not flow one charge with an
       * appropriate probability. To avoid wasting time on calculations when
       * charge is too unlikely to flow, the algorithm ignores flows smaller
       * than MINFRAC of the maximum flow. MINFRAC is set on the following line.
       */
            ...
            for (int i = 0; i <= 3; i++) {
               ...
               if (adjacentIndex > 0) { // Face is in the interior
                  ...
                  if (!facesCurrentPairs.containsKey(new TetFace(adjacentIndex, elemNum))) {
                     fc = faceCurrent(thisMeshedRegion, elemNum, i);
                     facesCurrentPairs.put(new TetFace(elemNum, adjacentIndex), fc);
                     current += fc;
                  }
               } // End face is in the interior
               /*
                * TODO -- Insert an else block here to deal with the case that
                * this face is a boundary of the mesh. Charge might flow if the
                * material on the other side of the boundary is a conductor.
                */
            }

            /*
             * If the current is high enough, make a note to come back to this
             * one.
             */
      ...
      /*
       * Loop through the noted tets in order of current from largest to MINFRAC
      ...
      /*
       * Keep a set of tets in which we've changed the charge. We'll check this
       * list to avoid altering charges more than once per cycle. TODO Change to
       * input Map
       */
   ...
   /*
    * quickUpdate is used in lieu of FEA to update the potentials of nodes of
    * the mesh. It is a poorer approximation, using only distance from center of
    * charge to adjust existing potentials, but it is much faster, so may be
    * used along with occasional full FEA to get back on track. The
    * approximation adjusts node potentials by adding e/(4*PI*epsilon*r) -
    * e/(4*PI*epsilon*CUTOFFRADIUS) for r<CUTOFFRADIUS and 0 for
    * r>=CUTOFFRADIUS. The second term makes sure the two expressions agree at
    * the boundary. The value of CUTOFFRADIUS is a parameter of the
    * approximation. It can be set by editing the next line.
    */
   ...
   private void quickUpdate(MeshedRegion thisMeshedRegion, HashMap<TetFace, Integer> flowMap) {
      ...
      for (final Map.Entry<TetFace, Integer> fc : flowSet) {
         ...
         int destination;
         /*
          * Take account of the sign of current by switching source and
         ...
         for (final Entry<Integer, Double> nodeEntry : nodes) {
            ...
            final double deltaPhi = phiPointCharge(xdest, epsDest, x, eps) - phiPointCharge(xsource, epsSource, x, eps);
    ...
    * dielectric constants at xSource and x are epsSource and eps respectively.
    * The formula here is crude. It returns 0 for x at distance greater than a
    * cutoff. At distances less than the cutoff it uses the point charge formula
    * modified to use the mean dielectric constant (if it differs at xSource and
    * x) and shifted to be 0 at the cutoff distance. In this way potential
    * differences at distances within cutoff are as given by the point charge
    * formula while those outside are 0. The mean dielectric constant is the
    * right formula if xSource and x are in each in an infinite half space
    * (separated by a plane) of materials with a different dielectric constant.
    * Thus, this formula does not take into account boundary conditions, shapes
   ...
   /**
    * Starting from regions with numbers given by source and destination, find
    * all nodes "close" to the point r0. Close is defined by JMONSEL for the
    * purpose of this routine to be 10 nm. Return a map of nodes and
    ...
    * @return
    */
   private HashMap<Integer, Double> getNearbyNodes(MeshedRegion thisMeshedRegion, int source, int destination, double[] r0) {
      final Mesh mesh = thisMeshedRegion.getMesh();
      ...
      /*
       * Find the next layer of near neighbors. Add each element's nodes to our
   ...
   /*
    * Conductivity is proportional to electric field to a power, p. The value of
    * p may be material and field dependent. If the conduction mechanism is
    ...
    * though still favoring places with higher fields. Smaller values of p
    * should therefore be better for speed of the algorithm. I think it likely
    * that if the calculation proceeds until current no longer flows, the value
    * of p will not strongly effect the final result. The reason is that flows
    * are from one tet to its neighbor, generally a very small distance. The
    * difference in fields is a dipole, therefore mainly significant only in the
    * neighborhood of the moved charge, and this in turn means that charges at
    * some distance can be safely independently moved.
    */
   private double currentFromField(double eFieldMag) {
      // return eFieldMag; // If I want p = 1
      return eFieldMag * eFieldMag; // If I want p = 2
      /*
       * To use a higher power than p = 2, set the desired value of p and
       * replace the above line with some similar to below.
       */
      // final double p = 3.;
   ...
   /**
    * Returns a double proportional to the signed current across the tet face
    * specified by tet1ID and faceIndex. Returns 0 if there is no current.
    *
    * @param tet1ID
    *           integer index of the source tet
    * @param faceIndex
    *           integer index of the face through which current may flow
    * @return
    */
   private double faceCurrent(MeshedRegion thisMeshedRegion, int tet1ID, int faceIndex) {
      /*
       * Many or most tet pairs can have no charge flow because they fail an
       * easy-to-check condition. We go through these first to save time. I
       * don't check that region 1 is non-vacuum because I'm assuming this
       * routine is only called when n1 != 0.
       */
      ...
      final int tet2ID = tet1.adjacentTetIndex(faceIndex);
      ...
      /*
       * These tets are useful for geometrical information, but should not be
       * used for electrical info (e.g., potentials) until I call
       * updatePotentials. In the meantime, it is more efficient to directly
       * compute the potential at the center by averaging that at the nodes.
       */
      final double pot1 = meanPotential(tet1);
      final double pot2 = meanPotential(tet2);
      final double deltaV = pot2 - pot1;
      final int n2 = mesh.getChargeNumber(tet2ID);

      /*
       * If we pass all the previous hurdles, estimate the energy difference if
       * we flow 1 charge.
       */
      ...
      /*
       * The "current" computed below is only proportional to the current.
       * currentFromField() field^p where p is the appropriate power. The result
       * is proportional to current density (current per unit area). This is
       * multiplied by the area of the face through which the current is flowing
       * and corrected by a cosine of the angle between the face and direction
       * of current flow.
       */
      if (effDeltaV < -thresholdEnergy) {
         ...
         final double[] currentDirection = Math2.normalize(Math2.minus(tet2.getCenter(), tet1.getCenter()));
         final double[] n = tet1.faceNormal(faceIndex);
         final double area = tet1.faceArea(faceIndex);
         current *= area * Math2.dot(currentDirection, n);
   ...
   /**
    * Returns the average of the potentials on the nodes of the supplied
    * tetrahedron.
    *
    * @param tet
    * @return
    */
   private double meanPotential(Tetrahedron tet) {
      final int[] nodes = tet.getNodeIndices();
      final Mesh mesh = tet.getMesh();
      double pot = 0.;
      for (final int node : nodes)
         pot += mesh.getNodePotential(node);
      return pot / 4.;
   }

   /**
    * This function implements the expression for effective potential difference
    * in my ConductionModelNotes.nb notebook, in the section on "Formulas useful
    * for implementation." The effective difference is the potential that is
    * relevant for determining if a charge in one of two adjacent regions will
    * transfer to the other region. It is equal to the potential difference of
    * the last FEA (input as deltaV, the last argument) with corrections. The
    * corrections account for 1) the fact that the FEA potential (erroneously
    * for this purpose) includes the field of the charge we are considering
    * transferring, 2) the mutual repulsion of like charges within the same
    * region, which is excluded wholly or in part by FEA, and 3) the interaction
    * between the charge we are considering moving and the charges in the
    * neighboring region. The result is an effective potential: the energy
    * difference between the two states (charge in region 1 vs. charge in region
    * 2) divided by e. It is positive if its tendency is to cause positive
    * current out of region 1 (transfer of +e from 1->2 or -e from 2->1) and
   ...
   private double effectivePotentialDifference(int n1, int n2, double r1, double r2, double eps1, double eps2, double deltaV) {
       ...
       * ConductionModelNotes.nb) indicates there is a term that is either 6/r1
       * or 6/r2 in the potential difference expression. Either r1 or r2 is
       * chosen depending upon which region supplies the carrier of the charge
       * that moves. Within the present model I am assuming carriers are scarce,
       * i.e., that the small regions (the only ones for which this is likely to
       * make much difference) are unlikely to contain trapped charges of either
       * sign apart from those excess charges that we know ended up in them as a
       * result of the scattering processes we track in JMONSEL. Hence the
       * requirement that the carrier from a region must be of the same sign as
       * excess charges in the region. In cases when current of the correct sign
       * can only be generated by a carrier from one region, we use that
       * region's radius. If the carrier can come from either region (because
       * the regions have excess charge of opposite sign) then I assume it flows
       * from the larger region, since this choice is the one that minimizes
       * energy. To change the model to assume plentiful carriers, i.e., that
       * all regions contain trapped charges of both signs and therefore that
   ...
   /**
    * If we're not going to do charge flows into low density regions we must
    * retain only the component of electric field parallel to the boundary
    * (i.e., perpendicular to the normal vector of the boundary). This method
    * returns the component of a tet's electric field that satisfies this
    * condition.
    *
    * @param tet
    * @return
   ...
   /**
    * <p>
    * Private utility to determine whether electrons are permitted to traverse
    * the indexed face of a given Tetrahedron. The criterion is that the
    * material on the other side of the face must have density >
    * DENSITY_THRESHOLD, but the way to determine this varies depending upon
    * whether the neighboring region is part of the mesh or not.
    * </p>
    * <p>
    * tetRegion and tet are redundant in the sense that the calling routine is
    * responsible to make sure tet = tetRegion.getShape(). (The caller has
    * already done this, so it saves time to pass both.)
    * </p>
    *
    * @param tetRegion
    * @param tet
   ...
   private boolean currentAllowed(MeshElementRegion tetRegion, Tetrahedron tet, int faceIndex) {
      ...
      if (nextTet == null) {
          ...
          * methods calls needed for the other cases.
          */
         ...
         /*
          * None of the above, so the adjacent region is a mesh element with
          * different material, and the different material is too low density to
          * support a current.
          */
         return false;
   }
   ...
   /**
    * Returns an array of 4 values equal to (E0+E[i]).S[i], where E0 and E[i]
    * are the corrected electric fields (see notes at the correctedEField
    * method) in the supplied tetrahedron and in the neighboring tetrahedron
    * through the ith face and S[i] is a vector with magnitude equal to the area
    * of the ith face and direction given by the face's outward normal. If a
    * face's neighboring region has density too low to support a charge (e.g.,
    * it's vacuum), the corresponding value is 0. For finite conductivity, this
    * quantity is proportional to the current through the faces.
    *
    * @param tetRegion
    * @param eField0
    * @return
    ...
    * neighbor through the face indexed by faceIndex. S is a vector with
    * magnitude equal to the area of the indexed face and direction given by the
    * face's outward normal. If a face's neighboring region has density too low
    * to support a charge (e.g., it's vacuum), the corresponding value is 0. For
    * finite conductivity, this quantity is proportional to the current through
    * the face.
    *
    * @param tetRegion
    * @param eField0
   ...
   private double faceCurrent(MeshElementRegion tetRegion, double[] eField0, int faceIndex) {
      ...
      /*
       * We have to do the "if" on the next line even though E.n ought to be 0
       * for forbidden faces because round-off error can make it not so. Even
       * though the error is tiny, it is significant if it becomes the only face
      ...
      if (currentAllowed(tetRegion, tet, faceIndex)) {
         ...
         if (nextTet == null)
             ...
             * away. I.e., we'll treat it as though the electric field on the
             * other side is equal to the one in this tet.
             */
    ...
    * positiveCharge==true or electrons if false. The corrected electric field
    * in this region is the field with any components in forbidden directions
    * (e.g., into neighboring vacuum) subtracted. Find the corrected electric
    * field in this region and compare it to the threshold for this region. If
    * it is less, then the charge doesn't move. (We return the start region.) If
    * it is more, we flow the charge across one of the startRegion's faces. (We
    * return the region on the other side of the face.) Which face? We find the
    * faces with currents of the correct sign (negative for electrons, positive
    * for holes). The current through a face is proportional to the dot product
    * of the corrected E field and its area * outward normal vector. If there's
    * only one of these, that's the one. If there are more than one, we choose
   ...
   private MeshElementRegion oneStep(MeshElementRegion startRegion, Mesh mesh, boolean positiveCharge) {
      ...
      /*
       * For our charge to flow through a face we require two conditions be
       * satisfied: (1) The current through that face must have the same sign as
       * the charge. (2) The potential energy must decrease. The latter
       * condition guarantees we do not have closed loops that repeat ad
       * infinitum. Faces that do not meet these conditions have face current
       * set to 0.
       */
      ...
      /*
       * Because we average the field across faces and elements have finite
       * size, it is possible to have no face with current of the correct sign.
   ...
   /**
    * Computes v - (v.n)n where v and n are vectors and "." is the dot product.
    ...
    * frac*(dielectric breakdown field), where frac is supplied as the argument
   ...
   /**
    * During charge flow iterations a new finite element analysis is performed
    * to update fields at intervals of 1 or more iterations, as set by this
    * method's argument. When FEA is not performed a faster but more approximate
    * method is used.
    *
    * @param feaInterval
    */
    ...
    * representing a tetrahedron (via its integer ID) and the total current into
    * or out of that tetrahedron. The ordering is in terms of absolute value of
    * the current.
    * </p>
    *
    * @author John Villarrubia
   ...
   /**
    * <p>
    * A class for designating faces of a tet and specifying its hash. The face
    * is specified by a pair of numbers, left and right, representing the ID
    * numbers of the Tets that share the face.
    * </p>
    *
    * @author John Villarrubia
    * @version 1.0
   ...
   private class TetFace {
      ...
      private ThresholdConductionModel getOuterType() {
         return ThresholdConductionModel.this;
      }
   ...
   private class ChargeFlowDebugInfo {
      ...
      /**
       * Takes a snapshot of tracked nodes and elements. A snapshot means the
       * current potential is appended to the list of potentials (initial and
       * previous) for each node. The current charge is likewise appended to the
       * list of charges for each element.
       */
