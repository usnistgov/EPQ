...
public class VectorSet {
   ...
   public Map<Element, Double> getNormalizedKRatios(final ISpectrumData spec) {
      ...
      if(false) {
         ...
         double sum = 0.0;
         for(final Vector vec : mVectors) {
            final double val = Math.max(0.0, vec.apply(specData));
            res.put(vec.getElement(), val);
            sum += val;
         }
         for(final Element elm : new TreeSet<Element>(res.keySet()))
            res.put(elm, res.get(elm).doubleValue() / sum);
      } else {
    ...
    * @throws IOException
    */
   public void writeXML(final OutputStream os)
         throws IOException {
