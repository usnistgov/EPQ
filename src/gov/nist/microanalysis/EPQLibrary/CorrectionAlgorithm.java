...
abstract public class CorrectionAlgorithm extends AlgorithmClass implements Comparable<AlgorithmClass> {
   ...
   public interface IAbsorptionSensitivity {
      /**
       * Computes the sensitivity coefficients for the quantitative correction
       * due to uncertainty in the mass absorption coefficient.
       * 
       * @param std
       ...
       * @param props
       *           The properties (must include beam energy)
   ...
   /**
    * The angle at which the detector is located. Measured up from the x-y
    * plane.
    */
   protected double mTakeOffAngle;
   /**
    * The angle at which the beam exits the sample. On a sample which is normal
    * to the beam, the exit angle is the same as the take-off angle. When the
    * sample is flat but tilted, the exit angle determines how much extra path
    * length the beam must traverse to exit the sample.
    */
    ...
    * Each item is an implements the CorrectionAlgorithm class.
    * 
    * @return List
    */
   @Override
   ...
   /**
    * initialize - Initialize the basic input parameters. Override this in
    * derived classes to preform any precomputations based on the Material,
    * AtomicShell, beam energy or take-off angle.
    * 
    * @param comp
    *           The material's Composition (as best known)
    * @param shell
    *           The AtomicShell for which to compute the correction
    * @param props
    *           A SpectrumProperties object containing at least
   ...
   public boolean initialize(Composition comp, AtomicShell shell, SpectrumProperties props) throws EPQException {
      ...
      if (res) {
         ...
         if (!comp.containsElement(mShell.getElement()))
            throw new IllegalArgumentException("The matrix material " + comp.descriptiveString(true) + " does not contain "
                  + mShell.getElement().toAbbrev() + " represented by " + shell.toString());
         if (shell.getEdgeEnergy() > e0)
            throw new IllegalArgumentException("The beam energy is less than the shell excitation energy for" + shell.toString() + ".");
      }
      return res;
   }

   /**
    * computeTilt - Computes the sample tilt relative to the electron beam axis.
    * If the SampleSurfaceNormal property is available it is used otherwise,
    * XTilt and YTilt properties are used. Returns 0.0 if none of these are
    * available.
    * 
    * @param props
    * @return double
   ...
   /**
    * Computes the ZAF correction relative to a pure element standard. This
    * method is not very computationally efficient. Usually this is not a
    * problem except with MCCorrectionAlgorithm.
    * 
    * @param comp
    ...
    *           - Spectrum properties
    * @param standard
    *           - The standard's Composition
    * @return A double[4] containing the z, a, f and zaf correction factors
    *         respectively
    * @throws EPQException
    */
   public double[] relativeZAF(Composition comp, XRayTransition xrt, SpectrumProperties props, Composition standard) throws EPQException {
      if (comp.containsElement(xrt.getElement())) {
         ...
         return new double[]{zComp / zStd, (zaComp * zStd) / (zaStd * zComp), fComp / fStd, (zaComp * fComp) / (zaStd * fStd)};
      } else
    ...
    * @throws EPQException
    */
   public double[] relativeZAF(Composition comp, XRayTransitionSet xrts, SpectrumProperties props, Composition standard) throws EPQException {
    ...
    * not very computationally efficient. Usually this is not a problem except
   ...
   public double relativeA(Composition comp, XRayTransition xrt, SpectrumProperties props) throws EPQException {
      final Material mat = new Material(new Element[]{xrt.getElement()}, new double[]{1.0}, ToSI.gPerCC(5.0), xrt.getElement().toAbbrev());
    ...
    *           - Spectrum properties
    * @param standard
    *           - The material used as a standard (often pure element.)
    * @return double
    * @throws EPQException
    */
   public double relativeA(Composition comp, XRayTransition xrt, SpectrumProperties props, Composition standard) throws EPQException {
      initialize(comp, xrt.getDestination(), props);
   ...
   /**
    * Compute only the atomic number term (Z) of the ZAF correction. This method
    * is not very computationally efficient. Usually this is not a problem
    * except with MCCorrectionAlgorithm.
    * 
    * @param comp
    *           - The Composition of the material
    * @param xrt
    ...
    * @throws EPQException
    */
   public double relativeZ(Composition comp, XRayTransition xrt, SpectrumProperties props, Composition standard) throws EPQException {
      initialize(comp, xrt.getDestination(), props);
   ...
   /**
    * <p>
    * Computes the ZA or Phi(rho z) part of the ZAF correction. You must must
    * call initialize(...) before calling computeZACorrection. The number
    * returned by computeZACorrection is not necessarily meaningful
    * in-and-of-itself. It may need to be ratioed with a standard to produce a
    * correction factor.
    * </p>
    * 
    * @param xrt
    ...
    * @throws EPQException
    */
   public double computeKRatio(Composition unk, Composition std, XRayTransition xrt, SpectrumProperties sp) throws EPQException {
      ...
      if (sp.isDefined(SpectrumProperties.SampleShape)) {
         ...
         stdSp.removeAll(Arrays.asList(new SpectrumProperties.PropertyId[]{SpectrumProperties.SampleShape, SpectrumProperties.SpecimenDensity,
               SpectrumProperties.MassThickness}));
      }
    ...
    * @throws EPQException
    */
   public double computeKRatio(Composition comp, Composition std, XRayTransitionSet xrts, SpectrumProperties sp) throws EPQException {
   ...
   /**
    * Computes the quantity of x-rays generated - nominally per incident
    * electron. Often emitted and generated share extensive similar
    * calculations. It is best to implement these in initialize(...) and then
    * call initialize(...) from both emitted and generated.
    * 
    * @param xrt
    * @return double
    */
   ...
   /**
    * caveat - Create a String describing the limitations of this algorithm for
    * the specified composition, shell and spectrum properties.
    * 
    * @param comp
    * @param shell
    * @param props
   ...
   /**
    * chi - Computes the mass absorption coefficient times a factor to account
    * for the non-normal take-off angle.
    * 
    * @param xrt
    *           XRayTransition - The specific x-ray transition
    * @return double - in (m^2)/kg
    * @throws EPQException
    *            - If the xrt is not a well known transition or take-off angle
    *            out of range.
    */
   ...
   static public class ZAFCorrectionAlgorithm extends CorrectionAlgorithm {
      ...
      public boolean initialize(Composition comp, AtomicShell shell, SpectrumProperties prop) throws EPQException {
         ...
         if (res) {
            ...
            mPureZ = bs.compute(mPureElement, mShell, mBeamEnergy) * sp.computeInv(mPureElement, mShell, mBeamEnergy);
         }
         return res;
      }
      ...
      public double computeZACorrection(XRayTransition xrt) throws EPQException {
         ...
         return (ac.compute(mComposition, xrt, mBeamEnergy, mExitAngle) * bs.compute(mComposition, xrt.getDestination(), mBeamEnergy)
      ...
      public double computeAbsorptionUncertainty(XRayTransition xrt) {
         ...
         if (ac instanceof PhilibertHeinrichAbsorption) {
            ...
            try {
               return (ph.uncertainty(mComposition, xrt, mBeamEnergy, mExitAngle) * bs.compute(mComposition, xrt.getDestination(), mBeamEnergy)
                     * sp.computeInv(mComposition, xrt.getDestination(), mBeamEnergy)) / mPureZ;
   ...
   public static class NullCorrectionAlgorithm extends CorrectionAlgorithm {
      ...
      /**
       * generated - Computes the quantity of x-rays generated. Often emitted
       * and generated share extensive similar calculations. It is best to
       * implement these in initialize(...) and then call initialize(...) from
       * both emitted and generated.
       * 
       * @return double
       */
      @Override
   ...
   /**
    * Pouchou &amp; Pichoir's XPP algorithm extended for non-normal incident
    * beams as implemented in the class XPP1989Ext.
    */
   ...
   /**
    * <p>
    * An abstract class that extends CorrectionAlgorithm for implementing
    * methods specific to phi-rho-z models of quantitiative x-ray microanalysis
    * correction.
    * </p>
    * <p>
    * Copyright: Pursuant to title 17 Section 105 of the United States Code this
   ...
   abstract static public class PhiRhoZAlgorithm extends CorrectionAlgorithm {
       ...
       * Initialize the Composition, AtomicShell and SpectrumProperties using
      ...
      /**
       * Computes the &phi;(&rho;&middot;z) curve with absorption for the
   ...
   static private final AlgorithmClass[] mAllImplementations = {NullCorrection, PouchouAndPichoir, XPP, XPPExtended, ZAFCorrection, Armstrong1982,
         Armstrong1982Particle};
}