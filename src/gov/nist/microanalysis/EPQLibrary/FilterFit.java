...
public class FilterFit extends LinearSpectrumFit {
	...
	public interface CullingStrategy {
		 ...
		 *
		 * @param fitParams UncertainValue[]
		 * @return Set&lt;Element&gt;
		 */
	...
	private static class FilteredPacket {
		...
		/**
		 * Returns the number of x-ray events in the peak corresponding to this filtered
		 * spectrum
		 *
		 * @return double
		 */
		private double getCounts() {
			if (Double.isNaN(mIntegral)) {
				...
				mIntegral = SpectrumUtils.backgroundCorrectedIntegral(mFiltered.getBaseSpectrum(),
						FromSI.eV(roi.lowEnergy()), FromSI.eV(roi.highEnergy()))[0];
			}
			return mIntegral;
		}
	}
	 ...
	 * perform culling after the fit to remove elements for which the spectrum does
	...
	private ArrayList<FilteredSpectrum> filterReference(final XRayTransitionSet xrts, final ISpectrumData ref)
			throws EPQException {
	...
	private ArrayList<FilteredSpectrum> filterReference(final ISpectrumData ref, final RegionOfInterestSet rois,
			final Element elm) throws EPQException {
		...
		for (final RegionOfInterestSet.RegionOfInterest roi : rois) {
			final double[] bci = SpectrumUtils.backgroundCorrectedIntegral(ref, FromSI.eV(roi.lowEnergy()),
	 ...
	 *
	 * @param ref A reference spectrum
	 * @throws EPQException - If the reference is not compatible with the unknown
	 */
	...
	/**
	 * Add a references spectrum associated with the specified RegionOfInterest. The
	 * RegionOfInterest must be associated with one and only one Element ie.
	...
	/**
	 * Returns 0.0 for perfect fit, 1.0 for terrible fit. Based on the fraction of
	 * peak counts that are actually fit by the element set.
	 *
	 * @return double
	 * @throws EPQException
	 */
	...
	/**
	 * Returns a set containing the filtered spectra associated with the specified
	 * element.
	 *
	 * @param elm
	...
	public ISpectrumData getResidualSpectrum(final ISpectrumData unk) throws EPQException {
		...
		for (final FilteredPacket raf : mFilteredPackets) {
			...
			final double norm = Math.max(0.0,
					raf.mKRatio.doubleValue() * (fs.getNormalization() / mFilteredUnknown.getNormalization()));
			if (norm > 0.0) {
		...
		if (unkProps.isDefined(SpectrumProperties.LiveTime))
			props.setNumericProperty(SpectrumProperties.LiveTime,
					unkProps.getNumericWithDefault(SpectrumProperties.LiveTime, Double.NaN));
	...
	/**
	 * getResidualSpectrum - Returns a ISpectrumData object containing the
	 * difference between the unknown and the sum of the reference spectra for the
	 * elements specified.
	 *
	 * @param unk
	 * @param elms
	 ...
	 * references times the associated k-ratio. It should be relatively easy to tell
	 * from this spectrum whether any major elements have been omitted from the fit.
	 *
	 * @return ISpectrumData
	 * @throws EPQException
	 */
	 ...
	 * spectrum for peaks and identifies which elements those peaks could correspond
	 * to. Any elements which are not in this list are removed from the fit. This
	 * optimization can speed the fit process by a factor of 10 or more. On the down
	 * side, it can remove elements which are actually present in very small
	 * quantities. Use with care....
	 *
	 ...
	 * quantities. Use with care....
	 *
	 * @param stripUnlikely true to preform this quick prefit test
	 */
	 ...
	 *
	 * @param residualModelThreshold The value to which to set
	 *                               residualModelThreshold.
	 */
	 ...
	 *
	 * @param cullingStrategy The value to which to set cullingStrategy.
	 */
	...
	public static class DontCull implements CullingStrategy {
		 ...
		 *
		 * @param base base {@link CullingStrategy} to build on
		 * @param dont The set of transitions not to cull if positive
		 */
	 ...
	 * chi-squared statistic. The optimal threshold depends upon count statistics.
	 * For spectra with good count statistics, a 1% increase (1.01) represents a
	 * good threshold. For spectra with less good statistics, 10% or larger might be
	 * appropriate. This necessity to tune the strategy makes it in general a poor
	 * choice.
	 * </p>
	 * <p>
	 * Copyright: Pursuant to title 17 Section 105 of the United States Code this
	...
	/**
	 * <p>
	 * This class makes certain that if a minor line is present that the associated
	 * major line is also present. For example, if MG is present then MA1 must also
	 * be present.
	 * </p>
	 * <p>
	 * Copyright: Pursuant to title 17 Section 105 of the United States Code this
	...
	public static class CullWithinFamily implements CullingStrategy {
		...
		public Set<Element> compute(final FilterFit ff, final UncertainValue2[] fitParams) {
			...
			for (final Element elm : elms) {
				...
				if (present.contains(new XRayTransition(elm, XRayTransition.KB1))
						&& (!present.contains(new XRayTransition(elm, XRayTransition.KA1))))
					removeThese.add(elm);
	 ...
	 * example if both K and L should be visible but only one of K or L is visible
	 * the element should probably not be included in the fit.
	 * </p>
	 * <p>
	 * Copyright: Pursuant to title 17 Section 105 of the United States Code this
	 * software is not subject to copyright protection and is in the public domain
	...
	public static class CullByFamilies implements CullingStrategy {
		...
		public Set<Element> compute(final FilterFit ff, final UncertainValue2[] fitParams) {
			final TreeSet<Element> removeThese = new TreeSet<Element>();
			/*
			 * Do inter-family checks... If the lower energy family is visible and the
			 * higher energy family is visible in the reference then the higher energy
			 * family should be visible in the unknown.
			 */
			for (final Element elm : ff.getElements()) {
				...
				if ((fpLa1 != null) && (lS > mThreshold) && (fpKa1 != null) && (fpKa1.getCounts() > fpLa1.getCounts())
						&& (kS < mThreshold))
					removeThese.add(elm);
				// If m visible but intense l not visible
				if ((fpLa1 != null) && (mS > mThreshold) && (fpMa1 != null) && (fpLa1.getCounts() > fpMa1.getCounts())
						&& (lS < mThreshold))
					removeThese.add(elm);
			}
	...
	/**
	 * <p>
	 * Check the most intense line (according to the standard) for each element and
	 * remove the element if this line is not clearly present.
	 * </p>
	 * <p>
	...
	public static class CullByBrightest implements CullingStrategy {
		...
		public Set<Element> compute(final FilterFit ff, final UncertainValue2[] fitParams) {
			...
			try {
				/*
				 * Find the most intense line family in the reference and make certain that it
				 * is visible
				 */
				for (final Element elm : ff.getElements()) {
					...
					for (final FilteredPacket fp : ff.getFilteredPackets())
						if (fp.getElement().equals(elm))
							if ((bestFp == null)
									|| ((fp.mKRatio.uncertainty() != 0.0) && (fp.getCounts() > bestFp.getCounts())))
								bestFp = fp;
					if ((bestFp != null)
							&& ((bestFp.mKRatio.doubleValue() / bestFp.mKRatio.uncertainty()) < mThreshold))
						removeThese.add(elm);
				}
	...
	/**
	 * <p>
	 * Remove one element based on the presence of another. ie. F in the presence of
	 * Fe.
	 * </p>
	 * <p>
	...
	public static class SpecialCulling implements CullingStrategy {
		...
		/**
		 * Remove <code>removeMe</code> when the element <code>whenPresent</code> is
		 * present with better statistics.
		 *
		 * @param removeMe
		 * @param whenPresent
		 */
		...
		/**
		 * Returns the element which triggers the removal of the specified element.
		 *
		 * @param removeMe
		 * @return Element
		 */
	...
	/**
	 * <p>
	 * Allows multiple CullingStrategy objects to be strung together sequentially.
	 * The resulting list of Elements consists of elements removed by any of the
	 * constituent CullingStrategy objects.
	 * </p>
	 * <p>
	 * Copyright: Pursuant to title 17 Section 105 of the United States Code this
	 * software is not subject to copyright protection and is in the public domain
	...
	public static class CompoundCullingStrategy implements CullingStrategy {
		...
		/**
		 * Applys all the CullingStrategy objects in the order in which they are entered
		 * into the list and returns the intersection of the resulting element sets.
		 *
		 * @param ff
		 * @param fitParams
		 * @return Set&lt;Element&gt;
