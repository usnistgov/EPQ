...
public class NISTMottRS extends RandomizedScatter {
	...
	public static class NISTMottRSFactory extends RandomizedScatterFactory {
		...
		/**
		 * extrapMethod is an integer = 1 or 2. Since the NISTMott tables are valid only
		 * for energies in the interval [50 eV, 20 keV], cross sections for energies <
		 * 50 eV can't be determined directly by table lookup. Optionally users may
		 * specify a higher minimum for use of the tabulated values. Outside of the
		 * permitted and availabe range, we use one of 2 methods: (1) extrapolation with
		 ...
		 * using the tablulated values by using Browning's power law or 2 if we will use
		...
		/**
		 * By default, we interpolate the NIST Mott tables for all energy values that
		 * fall within the range of tabulated values, but the lower limit can be
		 * increased from the default value with this setter.
		 * 
	...
	/**
	 * Returns a NISTMottRSFactory that uses minEforTable = 50 eV and extrapMethod =
	 * 1 (Browning).
	 */
	public static final RandomizedScatterFactory Factory = new NISTMottRSFactory();
	/**
	 * Returns a NISTMottRSFactory that uses minEforTable = 100 eV and extrapMethod
	 * = 1 (Browning).
	 */
	 ...
	 * computing random scattering angles using Mott cross sections. The constructor
	 * loads a table of numbers which are used to quickly compute cross sections for
	 * any energy within the tabulated range, 50 eV to 20,000 eV.
	 * </p>
	 * <p>
	 * For energies above the tabulated range, computations use
	...
	public NISTMottRS(Element elm) {
		...
		try {
			...
			/*
			 * If the above was successful mSpwem is a 1-d table of total cross sections,
			 * uniform in Log(E) from Log(50.) to Log(20000). mX1[j][i] is a 2-d table of q
			 * vs Log(E) and random #. j indexes log(E). i indexes r. q is related to
			 * cos(theta) by 1-2*q*q=cos(theta).
			 */
	 ...
	 * energies, namely from 8.01e-18 J to 3.20e-15 J (50 eV to 20 keV). The lower
