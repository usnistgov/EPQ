...
public class MaterialFactory {
   ...
   private static final String[] PBaseMaterials = {
      K3189,
      RynasAlTiAlloy,
      Mylar,
      VanadiumPentoxide,
   ...
   private static TreeMap<Element, Integer> parseCompound(String str)
         throws EPQException {
      final TreeMap<Element, Integer> elMap = new TreeMap<Element, Integer>();
      str = str.trim();
      final StringBuffer elStr = new StringBuffer();
      char c = (str.length() > 0 ? str.charAt(0) : Character.MIN_VALUE);
      for(int i = 0; c != Character.MIN_VALUE; ++i) {
         c = (i < str.length() ? str.charAt(i) : Character.MIN_VALUE);
         if(Character.isUpperCase(c) || Character.isDigit(c) || (c == '(') || (c == Character.MIN_VALUE)) {
            if(elStr.length() > 0) {
               final Element el = Element.byName(elStr.toString());
               if(el.isValid()) {
                  int count = 1;
                  elStr.setLength(0);
                  if(Character.isDigit(c)) {
                     ...
                     count = Integer.parseInt(elStr.toString());
                  }
                  final Integer ii = elMap.get(el);
                  elMap.put(el, Integer.valueOf((ii != null ? ii.intValue() : 0) + count));
               }
               elStr.setLength(0);
            }
            while(c == '(') {
               ...
               for(++i; br > 0; ++i) {
                  ...
                  if(c == Character.MIN_VALUE)
                     throw new EPQException("Unmatched bracket in " + str);
                  if(c == ')')
                     --br;
                  if(c == '(')
                     ++br;
               ...
               final TreeMap<Element, Integer> subComp = parseCompound(elStr.toString());
               elStr.setLength(0);
               c = (i < str.length() ? str.charAt(i) : Character.MIN_VALUE);
               int count = 1;
               ...
               for(final Entry<Element, Integer> e : subComp.entrySet()) {
                  final Element elm = e.getKey();
                  final Integer ic = elMap.get(elm);
                  elMap.put(elm, (ic != null ? ic : 0) + (count * e.getValue()));
               }
            }
            elStr.append(c);
         } else
    ...
    * @throws EPQException
    */
   public static Composition createCompound1(String str)
         throws EPQException {
   ...
   public static Composition createCompound(String str)
         throws EPQException {
      {
    ...
    * @throws EPQException
    */
   public static Material createCompound(String str, double density)
         throws EPQException {
   ...
   public static Composition createMaterial(String name) {
      ...
      try {
         if(name.equals(K3189)) {
            ...
            tmp.defineByMaterialFraction(new Composition[] {
               MaterialFactory.createCompound("SiO2"),
               MaterialFactory.createCompound("Al2O3"),
               MaterialFactory.createCompound("CaO"),
               MaterialFactory.createCompound("MgO"),
            ...
            return new Material(new Element[] {
               Element.Ti,
               Element.Al,
               Element.Nb,
               Element.W
               ...
               0.11,
               0.04
            }, ToSI.gPerCC(8.57), RynasAlTiAlloy);
            ...
            comp.defineByMaterialFraction(new Composition[] {
               MaterialFactory.createMaterial("SiO2"),
               MaterialFactory.createMaterial("PbO")
            }, new double[] {
               0.20000,
               0.80000,
            });
            ...
            comp.defineByMaterialFraction(new Composition[] {
               ...
               MaterialFactory.createCompound("CaO"),
               MaterialFactory.createCompound("Fe2O3"),
               MaterialFactory.createCompound("BaO")
            }, new double[] {
               0.15000,
               0.40000,
               0.15000,
            ...
            mat.defineByMaterialFraction(new Composition[] {
               MaterialFactory.createCompound("MgO"),
               MaterialFactory.createCompound("SiO2"),
               MaterialFactory.createCompound("CaO"),
