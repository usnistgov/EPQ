...
public class ControlChart extends BufferedImage {
   ...
   public void setRange(Date minDate, Date maxDate, double minY, double maxY) {
      ...
      final long msMaxDate = (TO_DAY * ((maxDate.getTime() + ((2 * (maxDate.getTime() - minDate.getTime())) / 100)) / TO_DAY)) + TO_DAY;
      ...
      if (mDrawStats) {
         ...
         if (!Double.isNaN(avg)) {
            ...
            try {
               ...
               gr.setStroke(new BasicStroke(1.0f, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_MITER, 10.0f, new float[]{12.0f, 4.0f}, 0.0f));
               gr.drawLine(canvas.x, pt.y, canvas.x + canvas.width, pt.y);
               final double std = mStats.standardDeviation();
               if (!Double.isNaN(std)) {
                  gr.setStroke(new BasicStroke(1.0f, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_MITER, 10.0f, new float[]{2.0f, 2.0f}, 0.0f));
                  for (int i = -1; i <= 1; i += 2) {
