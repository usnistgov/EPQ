...
public class NShapes {
	...
	private static void addOffsetPlane(NormalMultiPlaneShape shape, double[] normal, double[] pt, double dist) {
		...
		shape.addPlane(normal,
				new double[] { pt[0] + (normal[0] * dist), pt[1] + (normal[1] * dist), pt[2] + (normal[2] * dist) });
	}
	 ...
	 * with its top face at a specified z value. Its bottom width, length, sidewall
	 * angles, and top corner radii are all independently specified via the input
	 * parameters. The line can be subsequently rotated and translated if desired by
	 * using the rotate and translate functions. This routine uses an algorithm that
	 * forms the line as the intersection of three pieces, a right side (one or two
	 * planes + possible corner rounding), a similar left side, and an enclosure
	 * (top, bottom, and end caps on the line).
	 *
	 ...
	 * @param thetal double -- angle of the left sidewall in radians, expressed as a
	...
	public static NormalShape createLine(double topz, // z of the top face
		...
		/*
		 * The line will be the intersection of 3 pieces, a right side, a left side, and
		 * an "enclosure". The right side is a multiplane shape with 2 planes, one
		 * representing the sidewall and the other a plane that joins the top of the
		 * line to the place where the cylinder touches the sidewall. The left side does
		 * the same for the other side. The enclosure is a NormalMultiPlaneShape shape
		 * consisting of the top, bottom, and endcaps. I replaced the previous
		 * createLine algorithm with this one on 2/14/2013. The previous one formed the
		 * union of two cylinders, representing the corner rounding, with one multiplane
		 * shape (8 planes) to form the line. This worked fine for lines that were wide
		 * enough, but when the corner rounding becomes large enough (e.g., in the
		 * extreme when the cylinder diameter is greater than the linewidth) the
		 * cylinder that rounds the right corner can protrude through the left sidewall.
		 * This new algorithm eliminates that issue at the cost of being slightly (less
		 * than 5%) slower. This is a price worth paying now that narrow lines are much
		...
		if (radr > 0) {
			...
			rightNMPS.addPlane(new double[] { rad / root2, 0., (signz * costhetar) / root2 / rad }, new double[] {
					((width / 2.) - (radr / costhetar)) + (((radr - absz) * sinthetar) / costhetar), 0., topz });
			...
			final NormalCylindricalShape rcylinder = new NormalCylindricalShape(new double[] { xc, -length / 2., zc },
					new double[] { xc, length / 2., zc }, radr);
		...
		if (radl > 0.) {
			...
			leftNMPS.addPlane(new double[] { -rad / root2, 0., (signz * costhetal) / root2 / rad }, new double[] {
					((-width / 2.) + (radl / costhetal)) - (((radl - absz) * sinthetal) / costhetal), 0., topz });
			...
			final NormalCylindricalShape lcylinder = new NormalCylindricalShape(new double[] { -xc, -length / 2., zc },
					new double[] { -xc, length / 2., zc }, radl);
