...
abstract public class AbsorptionCorrection extends AlgorithmClass {
   ...
   private static final AlgorithmClass[] mAllImplementations = {AbsorptionCorrection.Philibert, AbsorptionCorrection.PhilibertHeinrich,
   ...
   abstract public double compute(Composition comp, XRayTransition xrt, double e0, double takeOffAngle) throws EPQException;
   ...
   public static class PhilibertAbsorption extends AbsorptionCorrection {
      ...
      @Override
      public double compute(Composition comp, XRayTransition xrt, double e0, double takeOffAngle) throws EPQException {
         ...
         // return 1.0 / ((1.0 + (chi / sigma)) * (1.0 + ((h * (chi / sigma)) /
         // (1.0 +
         // h))));
      }
   }
   ...
   static public class PhilibertHeinrichAbsorption extends AbsorptionCorrection {
       ...
       * @throws EPQException
       */
      public double uncertainty(Composition comp, XRayTransition xrt, double e0, double takeOffAngle) throws EPQException {
         final MassAbsorptionCoefficient mac = (MassAbsorptionCoefficient) getAlgorithm(MassAbsorptionCoefficient.class);
         final double chi = MassAbsorptionCoefficient.toCmSqrPerGram(computeChi(xrt, comp, takeOffAngle));
   ...
   public static class PhilibertHeinrichFullAbsorption extends AbsorptionCorrection {
      ...
      public double compute(Composition comp, XRayTransition xrt, double e0, double takeOffAngle) throws EPQException {
         ...
         return (1.0 + ((phi0 * h * chi) / ((4.0 + (phi0 * h)) * sigma))) / ((1.0 + (chi / sigma)) * (1.0 + ((h * (chi / sigma)) / (1.0 + h))));
      }
