 ...
 * @version 1.0
 */
public class CompositionFromKRatios
   extends
   AlgorithmClass {
   ...
   static public class ElementByDifference
      extends
    ...
    * @version 1.0
    */
   static public class ElementByFiat
      extends
      UnmeasuredElementRule {
      ...
      @Override
      public String toHTML() {
         return "<b>Element-by-fiat:</b> " + getElement().toAbbrev() + " at " + mQuantity.format(new DecimalFormat("0.000"))
               + " mass fraction.";
      }
   }
    ...
    * @author nicholas
    */
   static public class OxygenByStoichiometry
      extends
      UnmeasuredElementRule {
    ...
    * @author nicholas
    */
   public static class WatersOfCrystallization
      extends
      OxygenByStoichiometry {
      ...
      public Composition compute(final Composition comp) {
         ...
         final UncertainValue2 Oexcess = UncertainValue2.subtract(comp.weightFractionU(Element.O, false).reduced("M[O]"), oByStoic.weightFractionU(Element.O, false).reduced("S[O]"));
         if(Oexcess.doubleValue() > 0) {
            final UncertainValue2 massFracH = UncertainValue2.multiply((2.0 * Element.H.getAtomicWeight())
                  / Element.O.getAtomicWeight(), Oexcess);
   ...
   private void throwConfigurationError(final KRatioSet krs)
         throws EPQException {
   ...
   private Composition estimateInitialComposition(final KRatioSet krs) {
      ...
      for(final XRayTransitionSet xrts : krs.getTransitions()) {
         final Composition ref = mStandardData.get(xrts).mComposition;
         final UncertainValue2 c = UncertainValue2.multiply(ref.weightFraction(xrts.getElement(), true), krs.getKRatioU(xrts));
            ...
            if(sxrts.size() > 1) {
               ...
               sorted.sort(new Comparator<XRayTransitionSet>() {
                  ...
                  public int compare(final XRayTransitionSet o1, final XRayTransitionSet o2) {
                     ...
                     return Double.compare(1.0 / measured.getKRatioU(o1).fractionalUncertainty(), 1.0
                           / measured.getKRatioU(o2).fractionalUncertainty());
                  }
               });
               best = null;
    ...
    * @throws EPQException
    */
   public Composition compute(final KRatioSet measured, final SpectrumProperties unkProps)
         throws EPQException {
      if(!isReady(measured))
      ...
      for(final XRayTransitionSet xrts : mBestKRS.getTransitions())
         result.addElement(xrts.getElement(), UncertainValue2.multiply(massFractionU(xrts, best, unkProps), mBestKRS.getKRatioU(xrts).fractionalUncertaintyU()));
   ...
   public UncertainValue2 massFractionU(final XRayTransitionSet xrts, final Composition unkComp, final SpectrumProperties unkProps) {
      ...
      if(cUnk.doubleValue() > 0.0) {
         ...
         for(final XRayTransition xrt : xrts) {
            ...
            if((w / norm) >= mMinWeight)
               try {
                  sum = UncertainValue2.add(sum, UncertainValue2.multiply(w, CorrectionAlgorithm.XPP.kratio(stdData.mComposition, unkComp, xrt, unkProps)));
                  sumW += w;
               }
               catch(final EPQException e) {
                  e.printStackTrace();
