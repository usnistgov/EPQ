...
abstract public class TransitionEnergy extends AlgorithmClass {
	 ...
	 * @throws EPQException
	 */
	abstract public double compute(XRayTransition xrt) throws EPQException;
	...
	/**
	 * <p>
	 * Deslattes2005TransitionEnergy is based on the tabulation of x-ray transition
	 * energies from
	 ...
	 * @version 1.0
	 */
	public static class Deslattes2005TransitionEnergy extends TransitionEnergy {
		...
		private TreeMap<XRayTransition, Double> loadEnergies() {
			...
			try {
				try (final Reader isr = new InputStreamReader(getClass().getResourceAsStream("deslattes.csv"),
						"US-ASCII")) {
		...
		/**
		 * R.D. Deslattes, E.G. Kessler Jr., P. Indelicato, L. de Billy, E. Lindroth, J.
		 * Anton, J.S. Coursey, D.J. Schwab, C. Chang, R. Sukumar, K. Olsen, and R.A.
		 * Dragoset (2005), X-ray Transition Energies (version 1.2). [Online] Available:
		 * http://physics.nist.gov/XrayTrans [2009, May 22]. National Institute of
	 ...
	 * http://physics.nist.gov/ffast [10-Mar-2005]. National Institute of Standards
	 * and Technology, Gaithersburg, MD. Originally published as Chantler, C.T., J.
	 * Phys. Chem. Ref. Data 29(4), 597-1048 (2000); and Chantler, C.T., J. Phys.
	 * Chem. Ref. Data 24, 71-643 (1995)." Supports elements H to U and transitions
	 * to and from shells K to O5, P1 to P3.
	 */
	public static class Chantler2005TransitionEnergy extends TransitionEnergy {
		...
		public double compute(XRayTransition xrt) throws EPQException {
			...
			final AtomicShell src = xrt.getSource();
	...
	/**
	 * I modeled this on Chantler2005TransitionEnergy but it doesn't work as well.
	 * It seems that the edge energies in Chantler aren't consistent with other tabulations but
	 * if you use them to calculate trnsition energies, the transition energies are quite
	 * accurate.  On the other hand, the William's edge energies are more consistent with
	 * other tabulations but the transition energies calculated using them are not consistent
	...
	public static class Williams2011TransitionEnergy extends TransitionEnergy {
		public Williams2011TransitionEnergy() {
			super("Williams-2011",
					"Derived from edge energies taken from Williams 2011 in CRC Physics & Chemistry Handbook");
		}
		...
		@Override
		public boolean isSupported(XRayTransition xrt) {
			return EdgeEnergy.Williams2011.isSupported(xrt.getSource())
					&& EdgeEnergy.Williams2011.isSupported(xrt.getDestination());
		}
	}
	
	
	public static final TransitionEnergy Williams2011 = new Williams2011TransitionEnergy();
	/**
	 * DTSA - Extracts the transition energy from the DTSA database.
	 */
	public static class DTSATransitionEnergy extends TransitionEnergy {
		...
		public boolean isSupported(XRayTransition xrt) {
			...
			return XRayTransition.isWellKnown(xrt.getTransitionIndex()) && (z >= Element.elmH)
					&& ((z - 1) < mEnergy.length) && (mEnergy[z - 1] != null);
		}

		private synchronized void loadEnergyTable() {
			try {
				if (mEnergy == null) {
					mEnergy = (new CSVReader.ResourceReader("LineEnergies.csv", true))
							.getResource(TransitionEnergy.class);
	...
	public static class SuperSetTransitionEnergy extends TransitionEnergy {
	...
	public static final TransitionEnergy SuperSet = new SuperSetTransitionEnergy(Chantler2005, Williams2011);
	public static final TransitionEnergy DeslattesSuperSet = new SuperSetTransitionEnergy(DESLATTES, DTSA);
	public static final TransitionEnergy Default = TransitionEnergy.SuperSet;

	static private final AlgorithmClass[] mAllImplementations = { TransitionEnergy.Williams2011, TransitionEnergy.Chantler2005, TransitionEnergy.DTSA,
			TransitionEnergy.SuperSet, TransitionEnergy.DeslattesSuperSet };
}
