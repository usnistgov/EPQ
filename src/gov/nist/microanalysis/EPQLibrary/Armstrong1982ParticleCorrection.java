...
public class Armstrong1982ParticleCorrection extends Armstrong1982Base {
   ...
   public final class RightRectangularPrism implements Shape {
      ...
      public String toString() {
         ...
         return "Right-rectangular-prism[Thickness=" + df.format(this.getThickness() * 1.0e6) + ", Diameter=" + df.format(this.getDiameter() * 1.0e6)
               + "]";
      }
      ...
      public double computeP(double rhoZ) {
         ...
         if (mSimpleH) {
            ...
            if (rhoZ <= (0.5 * mRhoD * Math.tan(mTakeOffAngle))) // ok
               p = (1.0 / mRhoD) * ((((0.75 * mRhoD) - (0.5 * beta) - ((0.5 * beta * beta) / mRhoD) - (beta / (mAlphaPC * mRhoD))) * expNegChi)
               ...
               p = (1.0 / mRhoD)
                     * (((((mRhoD - (1.5 * beta)) + ((0.5 * beta * beta) / mRhoD) + (beta / (mAlphaPC * mRhoD))) - (1.0 / mAlphaPC)) * expNegChi)
                           + (0.5 * ((1.0 - expNegChi) / mAlphaPC))
                           + (((1.0 - expNegChi) * invAArD) - (2.0 * Math.exp(-0.5 * mAlphaPC * mRhoD) * invAArD)));
            else
               p = (1.0 / mRhoD) * (((0.5 * (1.0 - Math.exp(-mAlphaPC * mRhoD))) / mAlphaPC)
                     + (invAArD * ((1.0 + Math.exp(-mAlphaPC * mRhoD)) - (2.0 * Math.exp(-0.5 * mAlphaPC * mRhoD)))));
         }
         return p;
      }
   }
   ...
   public final class TetragonalPrism implements Shape {
      ...
      public double computeP(double rhoZ) {
         ...
         if (rhoZ <= (mRhoD * Math.tan(mTakeOffAngle)))
            p = (4.0 / (mRhoD * mRhoD)) * (((0.5 * (expNegChi - 1.0)) / (mAlphaPC * mAlphaPC)) + ((0.5 * beta) / mAlphaPC)
                  + (Math2.sqr(0.5 * lambda) * expNegChi) + (((0.5 * lambda) / mAlphaPC) * (1.0 - expNegChi)));
         else
            p = (4.0 / Math2.sqr(mRhoPC)) * (((Math.exp(-mAlphaPC * mRhoD) - 1.0) / Math2.sqr(mAlphaPC)) + ((0.5 * mRhoD) / mAlphaPC));
         return p;
      }
   }
   ...
   public final class VerticalCylinder implements Shape {
      ...
      public double computeP(double rhoZ) {
         ...
         if (rhoZ <= (mRhoD * Math.tan(mTakeOffAngle))) {
            ...
            assert Math.abs(((2.0 * sbl) / mRhoD) - (beta * sbl)) <= 1.0
      ...
      public double computeP(final double rz) {
         ...
         final Integrator i = new Integrator(TOLERANCE) {
            ...
            public double getValue(double rx) {
               final double r = (((rz * (mRhoD - (sinPsi * ((2.0 * cosPsi * rx) + (mRhoD * sinPsi))))) - Math2.sqr(cosPsi * rz)
                     - Math2.sqr(sinPsi * rx)) + (mRhoD * cosPsi * sinPsi * rx)) + Math2.sqr(0.5 * mRhoD * sinPsi);
            ...
            public double getValue(final double rhoy) {
               final Integrator inner = new Integrator(TOLERANCE) {
                  ...
                  public double getValue(double rhox) {
                     ...
                     final double rhow2 = Math.sqrt((0.25 * mRhoD * mRhoD) - (rhox * rhox) - (rhoy * rhoy)) - rhoz - (rhox * tanGamma);
   ...
   public class Sphere3 implements Shape {
      ...
      private final class SphereXIntegrate extends Integrator {
         ...
         public double getValue(double rx) {
            ...
            final double rzp = ((0.5 * mRhoD) + mRz) - Math.sqrt(Math.max(0.0, (0.25 * mRhoD * mRhoD) - (rx * rx) - (mRy * mRy)));
      ...
      public double computeP(final double rz) {
         ...
         if (alpha2 > 0.0) {
            ...
            if (Double.isNaN(res))
               assert !Double.isNaN(res) : Double.toString(alpha) + ", " + Double.toString((new SphereYIntegral(rz)).integrate(-alpha, alpha));
            return res;
         } else
            return 0.0;
      }
   ...
   public double particleAbsorptionCorrection(XRayTransition xrt) throws EPQException {
      ...
      final double maxThickness = FromSI.cm((1.5 * ElectronRange.KanayaAndOkayama1972.compute(mComposition, mBeamEnergy)) / ToSI.gPerCC(mRhoPC));
      // See p 273 (29)
