...
public class Mesh implements ITransform {
	 ...
	 * typically have efficient methods of determining which elements are adjacent
	 * to each other.
	 * </p>
	 * <p>
	 * Copyright: Pursuant to title 17 Section 105 of the United States Code this
	...
	public interface ConnectedShape extends NormalShape {
		 ...
		 * contains(pos0,pos1) last returned true. This can be used, for example, after
		 * a call to MeshShape.contains(pos0,pos1) returns true, to ascertain which
		 * Tetrahedron within the MeshShape contains pos0.
		 *
		 ...
		 * intersection, or null if the trajectory was leaving the mesh. This can be
		 * used, for example, to determine the next shape that an electron enters when
		 * it leaves the current one.
		 *
		...
		/**
		 * Forces update of cached values in this ConnectedShape. This is used if the
		 * underlying mesh is transformed.
		 */
		void updateGeom();
	}
	...
	static abstract public class Element {
		...
		/**
		 * Updates cached values that depend on the node positions. This routine should
		 * be called after any transformation (e.g., rotate or translate) that alters
		 * the node positions.
		 */
	...
	static public class Line extends Element {
		...
		public void updateGeom() {
			/*
			 * There's no cached geometry info in this class, so nothing to do here.
			 */
		}
	}
	...
	/**
	 * <p>
	 * MeshShape is a ConnectedShape that describes the boundary of a mesh. It is a
	 * closed surface consisting of triangular facets that are faces of tetrahedral
	...
	static public class MeshShape
			...
			implements ConnectedShape, ITransform {
		...
		/*
		 * Saved data from previous contains() call: The Tetrahedron that contains the
		 * point.
		 */
		private Tetrahedron containingShape = null;

		/*
		 * Our O(N^1/3) contains() method requires us to have a starting point known to
		 * be inside the shape. We store it and its containing tetrahedron here.
		 */
		...
		public MeshShape(Mesh mesh) {
			this.mesh = mesh;
			/*
			 * Pick an element near the middle. If it's not a tetrahedron Scan up (and if
			 * necessary down) to find the next tetrahedron.
			 */
		 ...
		 * returns true and moves pos a small distance (normally 1.e-15 m) off the face
		 * towards the center of the containing mesh element.
		 *
		 * @param pos
		 * @return
		...
		/**
		 * Returns true if this shape contains pos0. If pos0 is on the boundary, the
		 * point is deemed inside if the the path from pos0 to pos1 points to the inside
		 * of the shape.
		 *
		 * @param pos0
		 * @param pos1
		 ...
		 * method. These are all variants on the basic functionality: return true if
		 * pos0 is within the shape, false if it is not. Supplying pos1 produces the
		 * same result when pos0 is in the interior of the shape, but if pos0 is on the
		 * surface it breaks the tie using pos1, returning true if the path from pos0 to
		 * pos1 is entering the shape, false if it is leaving it. The remaining variants
		 * provide the same result, but differ in speed. The algorithm works by tracing
		 * a path from a starting position known to be inside a particular Tetrahedron
		 * member of the mesh. The closer the starting position is to pos0, the faster
		 * the routine. Thus, the quickest version is to call contains with 4 arguments,
		 * where the last two arguments are a Tetrahedron that is likely to be close to
		 * pos0 and a position within that Tetrahedron. The next fastest is to provide 3
		 * arguments, leaving out the position. In this case the insidePoint will be
		 * calculated as the center of the supplied Tetrahedron. Finally, if there is no
		 * reason to prefer one Tetrahedron over another, the two-argument version
		 * (omitting both the Tetrahedron and insidePoint) uses values for these
		 * predetermined and stored by the constructor.
		 * </p>
		 *
		 * @param pos0
		 * @param pos1
		...
		public boolean contains(double[] pos0, double[] pos1, Tetrahedron insideTet, double[] insidePoint) {
			 ...
			 * constituent tetrahedra. A brute force search (checking all tetrahedra until a
			 * match is found) is O(N), and N may be several million, so we employ a more
			 * efficient algorithm. The idea is this: Imagine a line segment joining
			 * insidePoint, known to be inside the mesh, to pos0. Starting with insidePoint
			 * and insideFlag=true, step from tetrahedron to tetrahedron along this path. At
			 * each boundary where we step out of or into the whole mesh (i.e., at the mesh
			 * surface, not the internal boundaries between elements) toggle the flag from
			 * true to false or back again. When we reach pos0, the value of the flag tells
			 * us whether we're inside or out. This algorithm should be O(N^1/3).
			 */

			/*
			 * A note for future consideration. The following algorithm should be O(N^1/3)
			 * as just described, but the proportionality constant seems high, probably
			 * because of the need to instantiate each Tetrahedron as the trajectory enters
			 * it. An alternative would be to use the getFirstIntersection() method
			 * implemented below for the surface of the MeshShape. This is O(N^2/3), but the
			 * triangles are all already instantiated. Tests indicate that for a large mesh
			 * (2.5 million tets), getFirstNormal requires 340 us (but a contains() based on
			 * it would likely have to call it at least twice) while the contains method
			 * below requires 940 us.
			 */
			...
			while (u < 1.) {
				/*
				 * The following conditionals check for backtracking. This can happen as
				 * follows: If pos0 is on the boundary of the current tet then u=1. However,
				 * round-off error can make u<1. In this case currentpos switches to the center
				 * of the tet on the other side of the boundary, from which position u is again
				 * = 1 but may again round off to less than 1. We would end up in an infinite
				 * loop. Instead, we detect this situation and decide which of the two shapes
				 * contains pos0.
				 */
				nextShape = currentShape.nextTet();
				if ((nextShape == previousShape) || ((nextShape == null) && (previousShape == this))) {
					/*
					 * This block if we detect backtracking. At least one of the two shapes must be
					 * a tet.
					 */
				...
				if (inside && (currentShape == null)) { // Inside-out crossing
					...
					while (inside) {
						...
						/*
						 * Test that we're now outside (previousShape is the tet we're exiting)
						 */
						inside = previousShape.contains(currentpos);
						/*
						 * If we're now outside we're finished. Otherwise we move it a bit more
						 */
					}
			...
			/*
			 * Arrive here when there are no more boundaries between currentpos and pos0.
			 */

			if (u > 1) {
				/*
				 * pos0 is unambiguously nearer than the next boundary. Inside is true or false
				 * as dictated by the current value stored in that flag.
				 */
				if (inside)
			...
			if (pos1 != null) { // tiebreak available
				if (inside) {
					...
					/*
					 * If the boundary we hit is an internal one, between two tets within the
					 * MeshShape, then there will be a nextTet.
					 */
				...
				/*
				 * Here we were outside, with tiebreak available. This boundary must be a
				 * surface of the MeshShape. If we are now inside, it will be because we are
				 * inside the Tetrahedron that we have hit here.
				 */
				currentShape = currentShape.nextTet();
			...
			/*
			 * Arrive here if there's no tiebreak, boundary counts as inside. However, I
			 * once encountered a situation where the currentShape's contains() method was
			 * not consistent with this determination. This was due to round-off error. To
			 * make sure the pos is unequivocally inside, we move it slightly towards the
			 * center.
			 */
			if (inside) {
				containingShape = (Tetrahedron) currentShape;
		...
		public double[] getFirstNormal(double[] pos0, double[] pos1) {
			...
			/*
			 * This is a brute-force implementation. It just checks all triangles on the
			 * surface of the mesh. If N is the number of mesh elements, this is an O(N^2/3)
			 * process. It should be possible to design a more efficient--albeit more
			 * complicated--O(N^1/3) algorithm, but at present the payoff for the added
			 * effort seems likely to be small. We won't use MeshShape.getFirstNormal() from
			 * within the mesh since when we begin inside we are always within one of the
			 * mesh's tetrahedral subregions. Only when we are outside will this routine be
			 * necessary--But my expectation is that we will mesh all the volume we care
			 * about. Ouside the mesh will only be a field-free evacuated region between the
			 * mesh and the chamber. In this region the mean free path is infinite, so the
			 * electron ordinarily only takes a single step. Under the circumstances, it
			 * doesn't pay to get too fancy with this algorithm.
			 */
		...
		/**
		 * Returns the tetrahedron across the boundary at the last boundary intersection
		 * returned by getFirstNormal or getFirstIntersection.
		 *
		 * @return
		 */
		@Override
	...
	static final public class Tetrahedron extends Element implements ConnectedShape {
		...
		/**
		 * Returns the Tetrahedron corresponding to the indexed element of mesh. Throws
		 * EPQFatalException if the indexed element is not a tetrahedron.
		 *
		 * @param mesh
		 * @param index
		 * @return
		...
		/**
		 * Returns true if the indexed element is a tetrahedron and the tetrahedron has
		 * already been instantiated in the cache.
		 *
		 * @param mesh
		 * @param index
		 * @return
		...
		/*
		 * Following is predigested geometrical information. It must be updated by using
		 * updateGeom() after transformations. A point p is in a plane if n.p-b=0. n and
		 * b are stored in the following arrays. The tetrahedron has 4 triangular faces,
		 * indexed 0 to 3. The numbering convention adopted here: Face i is the triangle
		 * with 3 nodes obtained by omitting node i.
		 */
		...
		/**
		 * Constructs the Tetrahedron corresponding to the indexed element of mesh.
		 ...
		 *
		 * @param faceIndex - index of the face shared with the adjacent Tetrahedron
		 * @return
		 */
		 ...
		 * indexed face. Returns 0 if the face is on the mesh boundary (no adjacent
		 * Tetrahedron).
		 *
		 * @param faceIndex - index of the face shared with the adjacent Tetrahedron
		 * @return
		...
		public boolean contains(double[] pos0, double[] pos1) {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (p0dotn == barray[i]) { // p0 is ON the boundary
					...
					final double deltadotn = (delta[0] * narray[i][0]) + (delta[1] * narray[i][1])
							+ (delta[2] * narray[i][2]);
					if (deltadotn > 0.)
						return false;
		...
		/**
		 * Returns the outward pointing normal vector of the face with the supplied
		 * index
		 *
		...
		/**
		 * Let v be a column vector of potentials at the 4 nodes. Then the potential at
		 * x inside the tetrahedron is v0 - E.x where v0 is a constant and E is the
		 * electric field vector. (v0,-E) is computed from geoCoef.v. geoCoef depends
		 * only on the geometry (node coordinates). This private routine computes the
		 * geoCoef matrix when needed.
		 */
		private Matrix geoCoef() {
			...
			coord.setMatrix(0, 3, 1, 3, new Matrix(new double[][] { mesh.getNodeCoordinates(elementsNodeIndices[0]),
					mesh.getNodeCoordinates(elementsNodeIndices[1]), mesh.getNodeCoordinates(elementsNodeIndices[2]),
		...
		/**
		 * Returns the total charge in this volume in units of e. E.g., the electron
		 * charge is -1;
		 *
		...
		/**
		 * Returns the electric field inside this tetrahedron, as determined from the
		 * potentials at its nodes. (Note that no position argument is required because
		 * the FEA does a linear approximation to the potential. Within this
		 * approximation the electric field is constant within each element.)
		 *
		...
		public double[] getFirstNormal(double[] pos0, double[] pos1) {
			...
			// indices.
			result = new double[] { 0., 0., 0., Double.MAX_VALUE }; // Initial value designates no intersection

			final double[] delta = { pos1[0] - pos0[0], pos1[1] - pos0[1], pos1[2] - pos0[2] };
			for (int i = 0; i < 4; i++) {
				/*
				 * Note significance of the sign of the next two variables numerator<0 means
				 * pos0 is inside the current face; numerator>0 means it's outside.
				 * denominator<0 means the line segment and the plane normal point in opposite
				 * directions. I.e., this intersection is an outside->inside transition.
				 * denominator>0 means the opposite.
				 */
				final double numerator = ((pos0[0] * narray[i][0]) + (pos0[1] * narray[i][1])
						+ (pos0[2] * narray[i][2])) - barray[i];
				final double denominator = (delta[0] * narray[i][0]) + (delta[1] * narray[i][1])
						+ (delta[2] * narray[i][2]);
				if (denominator == 0) {
					/*
					 * If the trajectory is parallel to the plane there are no intersections. If it
					 * starts inside it's always inside. If it starts outside it's always outside.
					 * In or Out is determined by the numerator. numerator<0, or =0 with tie break =
					 * true, means it is inside. In this case we continue looping, searching for
					 * intersections with other planes of this shape. Otherwise, we return u>1.
					 */
					if ((numerator < 0) || ((numerator == 0) && mesh.containsTieBreak(narray[i]))) {
						/*
						 * In the next line, if numerator=0 then the second of the above conditions is
						 * what places us within this if block. The trajectory lies wholly within the
						 * plane.
						 */
				...
				if (denominator > 0) { // This is an insidethisplane->outside
					...
					if (u < umax) { // It changes change umax
						/*
						 * If the new umax is < 0 the "inside" is behind our line segment If the new
						 * umax is < umin, this plane's inside and an earlier one are disjoint. In
						 * either case abort and return no intersection
						 */
						if ((u < 0) || (u <= umin)) { /*
														 * If the new umax is < 0 the "inside" is behind our line
														 * segment If the new umax is < umin, this plane's inside and an
														 * earlier one are disjoint. If umax=umin, the trajectory enters
														 * and leaves the shape at the same point, i.e., it is tangent
														 * to the surface. Since our shape is convex, a line can only be
														 * tangent on the OUTside, so this counts as a non-intersection.
														 * In any of these cases, abort and return no intersection.
														 */
							tie = false;
							return result;
						}
					...
					/*
					 * If the new umin is > 1 the "inside" is beyond the end of our line segment. If
					 * it is >umax this plane's inside and an earlier one are disjoint. Return
		...
		/**
		 * Returns the potential at x, linearly determined from the potentials at this
		 * tetrahedron's nodes. This is only reasonably accurate if x is inside the
		 * tetrahedron. If x is outside, this routine will extrapolate; this can give
		 * inaccurate results.
		 *
		...
		public double getPotential(double[] x) {
			final double potential = v0 - (electricField[0] * x[0]) - (electricField[1] * x[1])
					- (electricField[2] * x[2]);
			return potential;
		}

		/**
		 * Returns the index of the most recently intersected face. The tetrahedron has
		 * 4 nodes and 4 triangular faces. The numbering convention: Face i is the
		 * triangle specified by omitting node i.
		 *
		...
		public Tetrahedron nextTet() {
			...
			/*
			 * We end up here rarely, only if the trajectory at the last intersection was
			 * incident exactly onto an edge or node. The next line constructs the list of
			 * candidates in order from most to least probable.
			 */
			final int[] candidates = mesh.extendedAdjacentVolumeArray(myIndex, intersectedFace);
			/*
			 * Because of our strict definition of intersection (the continuation of the
			 * trajectory must go inside the shape if an intersection is to count) we can
			 * intersect at most ONE of these shapes. Therefore, we stop searching as soon
			 * as we find one.
			 */
			...
			for (final int index : candidates) {
				...
				/*
				 * if this is the next tet, our previous trajectory should intersect it earlier
			...
			/*
			 * If we didn't intersect any of them, we must have hit a boundary of the mesh
		...
		/**
		 * Updates cached values that depend on the node positions. This routine is
		 * called after any transformation (e.g., rotate or translate) that alters the
		 * node positions. The updated values including the normal vectors and distances
		 * to the planes of the 4 faces, the matrix of geometry parameters that is used
		 * to compute the interpolation parameters, and the interpolation parameters.
		 */
		@Override
		public void updateGeom() {
			for (int i = 0; i < 4; i++) {
				...
				mesh.planePerp(narray[i], coords0, mesh.getNodeCoordinates(facenodeindices[1]),
						mesh.getNodeCoordinates(facenodeindices[2]));
		 ...
		 * solution. Until it is called, the getPotential() and getEField() methods will
		 * return values that do not reflect the updated values of potentials on the
		 * nodes.
		 */
		public void updatePotentials() {
			final int[] nI = mesh.getNodeIndices(myIndex);
			final Matrix temp = geoCoef().times(new Matrix(new double[] { mesh.getNodePotential(nI[0]),
					mesh.getNodePotential(nI[1]), mesh.getNodePotential(nI[2]), mesh.getNodePotential(nI[3]) }, 4));
			v0 = temp.get(0, 0);
	...
	static public class Triangle extends Element {
		...
		/*
		 * Following is the index of the inside Tetrahedron. The insideTet is the one
		 * that n points away from.
		 */
		private final int insideTet;
		/*
		 * The index of the face of the insideTet that corresponds to this Triangle
		 */
		private final int faceIndex;
		/*
		 * A side-effect of getFirstNormal is to set the following variable. nextTet is
		 * either insideTet or outsideTet, whichever is the one on the other side of the
		 * boundary intersected by the segment from p0 to p1.
		 */
		private int nextTet = -1;
		/*
		 * Following are vectors representing the 3 sides. Numbering convention: Side i
		 * is the side that starts at node i and moves counterclockwise around the
		 * triangle when viewed from the outside. Thus, side 0 is node1-node0, side 1 is
		 * node2-node1, and side 2 is node0-node2.
		 */
		 ...
		 * @param faceIndex - The index of the face of insideTet that corresonds to this
		 *                  triangle.
		 * @param insideTet - the index of the Tetrahedron of which this triangle is a
		 *                  face. (This tetrahedron is inside the plane of the
		 *                  Triangle.)
		 */
		...
		/**
		 * Returns scaled distance to intersection with one of the Tetrahedra of which
		 * this Triangle is a face. This is the definitive test of whether and where a
		 * trajectory hits the triangle. However, it is costly to use, because it
		 * requires instantiating one or two tetrahedra and checking 4 faces for each
		 * one. For this reason it is best used only to adjudicate ambiguous cases.
		 *
		 * @param p0 - trajectory leg start point
		 * @param p1 - trajectory leg end point
		 * @return - u, such that p0 + u(p1-p0) is the intersection. Values of u>1 may
		 *         not be quantitatively correct, and should be taken to mean only "no
		 *         intersection between p0 and p1".
		 */
		private double associatedTetrahedronIntersection(double[] p0, double[] p1) {
			...
			if (outsideTet != -1) {
				...
				/*
				 * The remaining possibility is temp negative, in which case we leave u as is.
				 */
			}
			return u;
		}
		...
		/**
		 * Returns [nx,ny,nz,u], where [nx,ny,nz] is the normalized vector pointing
		 * towards the outside of this Triangle and u is a parameter such that the
		 * intersection of the line segment from p0 to p1 with the Triangle is at
		 * p0+u*(p1-p0). u will only be accurate if it lies between 0 and 1 (i.e., if
		 * the intersection lines within the line segment). If there is no intersection
		 * or if the intersection is past p1, a value of u&gt;1 will be returned.
		 *
		...
		public double[] getFirstNormal(double[] p0, double[] p1) {
			...
			/*
			 * Consider the line containing the segment joining p0 to p1. This line has
			 * equation x = p0 + u*(p1-p0). The plane defined by our triangle has equation
			 * n.x - b = 0. Compute u = (b-n.p)/(n.(p1-p0)).
			 */
			...
			if (denominator == 0.) {
				if ((numerator == 0.) && mesh.containsTieBreak(n))
					/*
					 * The numerator and denominator both = 0 case is a trajectory that hits the
					 * triangle edge-on. It is so improbable it almost never happens accidentally.
					 * It most commonly arises if the user aims the electron gun right at an edge. A
					 * tie break decides whether the plane of the triangle contains or does not
					 * contain the trajectory. The tie breaker must be consistent with the one used
					 * by Tetrahedron.contains(). If the plane does contain the trajectory then
					 * further examination is required to determine whether the triangle does. We
					 * deem the trajectory to hit the triangle if it hits one of the tetrahedra
					 * associated with the triangle. I.e., the NormalShape intersection rules are
			...
			/*
			 * If we start outside the plane (numerator < 0) then the next tet will be the
			...
			/*
			 * We arrive here if we intersect the plane of this triangle between p0 and p1.
			 * It remains to determine whether the intersection point lies within or outside
			 * the triangle.
			 */
		...
		/**
		 * Gets the value of the tetrahedron that adjoins this triangle on the inside.
		 *
		 * @return
		 */
		...
		/**
		 * Gets the index of the Tetrahedron that adjoins this triangle on the outside.
		 *
		 * @return
		 */
		...
		/**
		 * Applies a cross product test to determine if x is inside the indexed edge.
		 * The returned value is positive if x is inside, negative if outside, 0 if on
		 * the edge.
		 *
		 * @param edgeIndex
		 * @param x
	...
	/**
	 * Resets charge densities in all volume elements and potentials at all nodes to
	 * 0, then calls updateAllPotentials().
	 */
	...
	/**
	 * Decrements the charge number by 1. This corresponds, e.g., to an increase of
	 ...
	 * used to save the state of the mesh during or after a simulation. At a later
	 * time, a duplicate of the mesh can be created by once again calling the
	 * constructor and then importing the charge and potentials by using the
	 * importChargeAndPotentials() method. Note that the node and element indexing
	 * used by these methods is the same as that of the mesh file used to generate
	 * this mesh; the import and export methods cannot be used with a different
	 * mesh, even of the same sample.
	 *
	...
	/**
	 * Returns the total charge contained in volume elements tagged with the given
	 * tag. (Performance note: this requires a scan through all elements of the
	 * mesh.) The returned charge is in units of e (magnitude of electron charge).
	 *
	 * @return - the charge number
	 */
	public int getChargeFromTag(long tag) {
		...
		for (int i = 1; i < numElements; i++)
			if (isVolumeType(i) && (basicMesh.getTags(i)[0] == tag)
					&& ((elementCharge = basicMesh.getChargeNumber(i)) != 0))
				totalCharge += elementCharge;
		return totalCharge;
	}

	/**
	 * Given an array of positions inside the mesh, returns an array of containing
	 * the number of elementary charges in the mesh that contains each position. If
	 * a position lies outside the mesh the returned value is 0.
	 *
	 * @param pos - An N x 3 (double[][]) array of positions
	 * @return - double[] An array of corresponding potentials.
	 */
	 ...
	 * ConnectedShape that describes the boundary of a mesh. It is a closed surface
	 * consisting of triangular facets that are faces of tetrahedral members of the
	 * mesh. Faces that are on the surface are members of only a single tetrahedron,
	 * unlike faces in the interior, which have tetrahedra on both sides.
	 *
	 * @return
	 */
	 ...
	 * that shares the supplied nodeIndex.
	 *
	 * @param nodeIndex
	 * @return
	 */
	...
	/**
	 * Increments the charge number in the indicated element by 1. This corresponds,
	 * e.g., to a loss of one electron from this volume element.
	 *
	 * @param index - The index of the element for which charge is to be
	 *              incremented.
	 */
	 ...
	 * revision number. If it is unchanged since the previous call to this method,
	 * nothing is done. If it differs, internal caches are cleared or reinitialized.
	 *
	 * @return - true if the initialization was needed (&amp; performed), false if
	 *         not.
	 */
	 ...
	 * this class, forcing the interior potential and electric field for each to be
	 * recomputed. Note that if the clearElementsCache() has been used to clear the
	 * cache, any remaining instances of volume elements that had been in the cache
	...
	/**
	 * If the indexed mesh element exists (i.e., is already instantiated and cached)
	 * and is a volume-type element, its updatePotentials() method is called.
	 *
	 * @param index
	 */
	 ...
	 * i*deltax + j*deltay. In this equation, p0, deltax, and deltay should each be
	 * understood as 3-component vectors. The indices i and j form a raster with i
	 * varied from 0 to nx-1, j from 0 to ny-1. p0, deltax, and deltay are vectors
	 * describing the first point in the grid, the offset between neighboring points
	 * in the "x" direction, and the offset in the "y" direction. (Because deltax
	 * and deltay are vectors, the x and y directions in the map can correspond to
	 * any desired directions in the 3-D coordinate system of the simulation.) The
	 * output file is a text file with p0, deltax, deltay, nx, and ny forming the
	 * first 5 lines. The remaining lines are two values--charge density and
	 * potential in that order-- per line in raster order, i values (the "x" index)
	 ...
	 * @param deltay- double[] A 3-vector representing the increment between lines
	 *                of points along the "y" direction.
	 ...
	 * @throws IOException
	 */
	public void writeChargeAndPotentialMap(double[] p0, double[] deltax, double[] deltay, int nx, int ny,
