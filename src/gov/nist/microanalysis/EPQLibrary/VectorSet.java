...
public class VectorSet {
	...
	static double computeDose(final ISpectrumData spec) {
		...
		return props.getNumericWithDefault(SpectrumProperties.LiveTime, 60.0)
				* SpectrumUtils.getAverageFaradayCurrent(props, 1.0);
	}
	...
	/**
	 * Defines a vector for a ROI using the specified array of double. This array is
	 * dot producted onto the spectrum.
	 *
	 * @param roi
	 * @param vector
	 */
	...
	/**
	 * Removes all vectors associated with transitions with edge energies above e0
	 * unless there is no other less energetic transition available.
	 *
	 * @param e0
					...
					if (!t)
						if (ee < e0)
							if (xrt.getFamily() == bestXrt.getFamily())
								t = (xrt.getWeight(XRayTransition.NormalizeFamily) > bestXrt
										.getWeight(XRayTransition.NormalizeFamily));
							else
								t = (ee > bestEE);
					if (t) {
						best = v;
	...
	/**
	 * Returns the RegionOfInterest objects for which vectors are available. These
	 * are a subset of the RegionOfInterest objects used to calculate the Vectors.
	 *
