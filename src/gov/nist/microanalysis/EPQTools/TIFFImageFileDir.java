...
public class TIFFImageFileDir {
   ...
   final public static byte[] LITTLE_MAGIC = new byte[]{0x49, 0x49, 0x2A, 0x00};

   final public static byte[] BIG_MAGIC = new byte[]{0x4D, 0x4D, (byte) 0xA2, 0x00};

   // Data sizes in bytes of the various tag data item types (index by ttByte,
   // ttAscii,... ttDouble)
   static int[] kDataSize = {0, 1, 1, 2, 4, 8, 1, 1, 2, 4, 8, 4, 8};
   ...
   static final String[] typeToStr = new String[]{"NULL", "byte", "ASCII", "short", "long", "rational", "signed byte", "undefined", "signed short",
   ...
   static public class Field {
      ...
      public Field(short id, int i) {
         this(id, ttLong, new int[]{i});
      }

      public Field(short id, short i) {
         this(id, ttShort, new int[]{i});
      }
      ...
      private Field(ByteBuffer bb) throws IOException {
         mTagId = bb.getShort();
         mType = bb.getShort();
         mOrder = bb.order();
         assert (mType >= ttByte);
      ...
      public String toString() {
         return Integer.toString(mTagId) + "[" + Integer.toHexString(shortToInt(mTagId)) + "]: " + Integer.toString(mCount) + " of "
               + typeToString(mType);
      }
      ...
      public int[] getAsIntegerArray() throws EPQException {
         final ByteBuffer bb = ByteBuffer.wrap(mData);
         bb.order(mOrder);
      ...
      double[] getAsDoubleArray() throws EPQException {
         final ByteBuffer bb = ByteBuffer.wrap(mData);
         bb.order(mOrder);
   ...
   public TIFFImageFileDir(ByteBuffer bb, int offset) throws IOException {
      bb.position(offset);
   ...
   public static List<TIFFImageFileDir> readIFD(File file) throws IOException, EPQException {
   ...
   public void addBWImage(BufferedImage si) {
      ...
      addField(new Field(X_RESOLUTION, ttRational, new int[]{300, 1}));
      addField(new Field(Y_RESOLUTION, ttRational, new int[]{300, 1}));
