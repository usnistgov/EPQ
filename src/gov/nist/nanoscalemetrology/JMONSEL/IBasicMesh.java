...
public interface IBasicMesh {
    ...
    * #tetIndex through the indexed face. "Extended" means volumes that share
    * only an edge or a node of this face are returned in addition to the volume
    * that shares the entire face. The array is sorted from most shared nodes to
    * fewest shared nodes. I.e., the tet that shares the whole face (3 nodes) is
    * listed first if there is one. Then tets that share an edge (2 nodes) are
    * listed. Finally tets that share only a point (a single node) are listed.
    */
   ...
   /**
    * Returns an N x 2 array that specifies the faces on the boundary of the
    * mesh. N is the number of faces on the boundary. Each face is specified by
    * two integers. The first is the index of the volume element to which the
    * face belongs and the second is the local index of the face within that
    * volume element. E.g., result[i][0] = n and result[i][1] = k means the kth
    * face of volume element #n is one of the boundary faces.
    *
    * @return
    */
   ...
   /**
    * Gets the name of the file that was read to produce this mesh. (This may be
    * a stored mesh file or a geometry file from which the mesh was computed.)
    *
    * @return
    */
    ...
    * element that shares the supplied nodeIndex. These are sorted in order of
    * increasing index.
    *
    * @param nodeIndex
    * @return
    ...
    * getNumberOfElements(). The order that nodes are listed in the returned
    * array is significant. For a tetrahedron, any node may be given first. The
    * remaining 3 are then given in clockwise order as viewed from the first.
    *
    * @param elementIndex
    * @return
    */
   ...
   /**
    * Changes the coordinates of the indexed node to the supplied new
    * coordinates. Note that by moving coordinates it is possible to make
    * nonsense out of a mesh, for example by having positions that are contained
    * in more than one volume element of the altered mesh. It is the
    * responsibility of the calling routine to insure that the cumulative effect
    * of coordinate changes makes sense before finite element analysis (or other
    * operations that require a valid mesh) are attempted. Common
