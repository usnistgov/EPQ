 ...
 * @version 1.0
 */
public class Armstrong1982ParticleMC
   ...
   public boolean initialize(Composition comp, AtomicShell shell, SpectrumProperties props)
      ...
      if(res) {
         ...
         assert mMCShape.contains(Math2.plus(Math2.multiply(1.0e-9, Math2.Z_AXIS), SpectrumUtils.getSamplePosition(props)));
         assert !mMCShape.contains(Math2.plus(Math2.multiply(-1.0e-9, Math2.Z_AXIS), SpectrumUtils.getSamplePosition(props)));
   ...
   public double particleAbsorptionCorrection(XRayTransition xrt)
      ...
      // muArho in CGS
      final double muArho = MassAbsorptionCoefficient.toCmSqrPerGram(mac.compute(mComposition, xrt)) * FromSI.gPerCC(mRho);
      assert muArho > 0.0;
      ...
      final MCIntegrator integrator = new MCIntegrator(mPt0, mPt1) {
         ...
         public double[] function(double[] args) {
            ...
            return new double[] {
               gen * Math.exp(-muArho * g) * h(rhoXyz),
               gen
            };
         }
