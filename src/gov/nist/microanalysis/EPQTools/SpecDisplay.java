...
public class SpecDisplay
   ...
   public class Region
      ...
      public String toString() {
         return "[" + Integer.toString((int) Math.round(mLowEnergy)) + " eV, " + Integer.toString((int) Math.round(mHighEnergy))
               + " eV]";
      }
   ...
   public enum SCALING_MODE {
      NONE("Same scale"), INTEGRAL("Scaled to equal integral"), MAX_PEAK("Scaled to highest peak"), REGION_INTEGRAL(
            "Scaled to region integral"), CONSTANT_FLUX("Scaled to 60 nA\u00B7second"), PER_NA_EV("Scaled to 1 eV\u00B7second");
   ...
   static List<EffectiveLine> buildEffectiveLines(Collection<KLMLine> lines, double tolerance) {
      ...
      dupLines.sort(new Comparator<KLMLine>() {
         ...
         public int compare(KLMLine o1, KLMLine o2) {
            return -Double.compare(o1.mAmplitude, o2.mAmplitude);
         }
      });

      while(dupLines.size() > 0) {
         final KLMLine line = dupLines.remove(0);
         final EffectiveLine el = new EffectiveLine(line, tolerance);
         effs.add(el);
         final List<KLMLine> removed = el.addLines(dupLines);
         dupLines.removeAll(removed);
      }
      return effs;
   }
   ...
   static private class EffectiveLine {
      ...
      public List<KLMLine> addLines(Collection<KLMLine> lines) {
         ...
         do {
            ...
            for(final KLMLine line : lines)
               if(memberOf(line) && (!res.contains(line))) {
                  ++cx;
                  add(line);
                  res.add(line);
               }
      ...
      private void add(KLMLine klm) {
         assert mLines.isEmpty() || memberOf(klm);
         mLines.add(klm);
   ...
   private void drawLines(Graphics2D gr, KLMLine.LabelType lt, Collection<KLMLine> lines, boolean forSvg) {
      ...
      try {
         ...
         try {
            final double tolerance = ToSI.eV((mEMax - mEMin) / 200);
            ...
            while(effs.size() > 0) {
               ...
               EffectiveLine maxLine = (xeffCenter >= mPlotRect.x) && (xeffCenter < mMaxHeight.length + mPlotRect.x) ? eff
                     : null;
   ...
   private Dimension saveAsImage(File outfile, String formatName)
      ...
      final BufferedImage bi = new BufferedImage(cWidth, cHeight, formatName.equalsIgnoreCase("BMP")
   ...
   protected void paintComponent(Graphics g, boolean forSvg) {
      ...
      final int MIN_GRID_SCALE = 5;
      {
         if(mShowHAxisLabels) {
            ...
            dup.drawString(ENERGY_AXIS_LABEL_KEV, (int) (mPlotRect.x
                  + (0.5 * (mPlotRect.width - fm.stringWidth(ENERGY_AXIS_LABEL_KEV)))), canvasHeight - fm.getDescent());
      ...
      switch(mVAxisType) {
         ...
         case SQRT: {
            ...
            if(showVAxis) {
               final int x = fm.getHeight(),
                     y = (int) (mPlotRect.y + (0.5 * (mPlotRect.height + fm.stringWidth(COUNTS_SQUARE_ROOT_AXIS_LABEL))));
               final AffineTransform oldAf = dup.getTransform();
               dup.rotate(-0.5 * Math.PI, x, y);
      ...
      dup.setColor(mBackTextColor);
      {
         ...
         if(!forSvg)
            dup.drawString(Integer.toString(getWidth()) + " \u00D7 " + Integer.toString(getHeight()), l0
                  + fm.charWidth('j'), pp);
      }
         ...
         for(int i = 0; i < mPlotRect.width; ++i) {
            ...
            base_ch += chPerPixel;
            {
               ...
               if((low < vMax) && (high > vMin)) {
                  ...
                  if(low < vMin)
                     low = vMin;
                  gr.drawLine(i + mPlotRect.x
                        + off, (int) ((mPlotRect.y + mPlotRect.height) - (((low - vMin) * mPlotRect.height) / (vMax - vMin)))
                              - off, i + mPlotRect.x + off, highPos - off);
               }
   ...
   private void endDrag(int x) {
      ...
      if((mDragStart != Integer.MIN_VALUE) && (Math.abs(mDragStart - x) >= MIN_DRAG_DELTA)) {
         if(mDragEnd != Integer.MIN_VALUE) {
            ...
            gr.setXORMode(Color.green);
            gr.fillRect(mDragStart < mDragEnd ? mDragStart
                  : mDragEnd, mPlotRect.y, Math.abs(mDragEnd - mDragStart), mPlotRect.height);
         ...
         }
         {
            ...
            final List<ISpectrumData> data = getData();
            for(final ISpectrumData spec : data) {
               final long cx = Math.round(SpectrumUtils.sumCounts(spec, SpectrumUtils.channelForEnergy(spec, min), SpectrumUtils.channelForEnergy(spec, max)
                     + 1));
   ...
   public class JMenuItem_Zoom
       ...
       * 
       * @param factor 0.0 is zoom to region, 1.0 is zoom to all, 2.0 is zoom in
       *           by 2, 0.5 is zoom out by 2.0 etc.
       */
    ...
    * @version 1.0
    */
   public class JMenuItem_Compare
      extends
      JCheckBoxMenuItem {
      private static final long serialVersionUID = 6627853387207913400L;
      ...
      public JMenuItem_Compare(ButtonGroup bg, SCALING_MODE mode) {
         ...
         addActionListener(new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
               ...
               Preferences.userNodeForPackage(SpecDisplay.class).put("Scaling", mMode != SCALING_MODE.REGION_INTEGRAL
                     ? mMode.name()
                     : SCALING_MODE.NONE.name());
            }
         });
      }
   }
   ...
   private class LabelAllLinesAction
      extends
      AbstractAction {
         ...
         mi.addActionListener(new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
               try {
                  ...
                  final SimpleFileFilter png = new SimpleFileFilter(new String[] {
                     "png",
   ...
   public void backgroundCorrectedPeakIntegration() {
      ...
      for(final Region r : mRegions.mList) {
         ...
         final List<ISpectrumData> data = getData();
         for(final ISpectrumData sd : data) {
            final double[] bci = SpectrumUtils.backgroundCorrectedIntegral(sd, r.getLowEnergy(), r.getHighEnergy()
                  - ENERGY_EPS);
   ...
   public void threeROIpeakIntegration() {
      if(mRegions.size() == 3) {
         ...
         for(final ISpectrumData sd : getData()) {
            final double[] tmp = SpectrumUtils.backgroundCorrectedIntegral(sd, onPeak.getLowEnergy(), onPeak.getHighEnergy(), low.getLowEnergy(), low.getHighEnergy(), high.getLowEnergy(), high.getHighEnergy());
            res += "\n" + nf.format(tmp[0]) + "\t" + nf.format(tmp[1]) + "\t" + nf.format(tmp[0] / tmp[1]) + "\t"
                  + sd.toString();
         }
         setTemporaryAnnotation(res);
      } else
         ErrorDialog.createErrorMessage(this, "Three region integration", "Please highlight three and only three regions.", "Three region integration requires you to highlight a low energy\nbackground window, an on-peak window and a high energy background window.");
   }
         ...
         if(data.size() > 0) {
            ...
            if(sd != sd0)
               if(mCurrentScalingMode == SCALING_MODE.INTEGRAL) {
                  ...
                  scale = c > 0.0 ? sd0.getCounts(SpectrumUtils.maxChannel(sd0, MIN_CHANNEL, sd0.getChannelCount())) / c : 1.0;
               } else if(mCurrentScalingMode == SCALING_MODE.REGION_INTEGRAL)
                  if(mScalingRegions != null)
                     for(final Region r : mScalingRegions.mList) {
                        final double c0 = SpectrumUtils.integrate(sd0, r.getLowEnergy(), r.getHighEnergy() - ENERGY_EPS)
                              * sd0.getChannelWidth();
                        if(c0 > 0.0) {
                           final double c = SpectrumUtils.integrate(sd, r.getLowEnergy(), r.getHighEnergy() - ENERGY_EPS)
                                 * sd.getChannelWidth();
