...
public class NormalIntersectionShape implements NormalShape, ITransform {
	...
	/**
	 * Construct a NormalIntersectionShape that corresponds to the intersection of
	...
	public double[] getFirstNormal(double[] pos0, double[] pos1) {
		...
		/*
		 * Logic of the algorithm: This algorithm is adapted from the NormalUnionShape
		 * version. See the notes there for a fuller description. The difference here is
		 * that for an intersection, we are inside the intersection only when depth = 2.
		 * Therefore, instead of depth transitions to and from 0, which were the
		 * relevant ones for the union shape, we must here look for transitions to and
		 * from depth of 2.
		 */
