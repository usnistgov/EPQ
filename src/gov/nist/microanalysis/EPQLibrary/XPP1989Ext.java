...
public class XPP1989Ext extends XPP1991 {
   ...
   static private final BookChapter PAP_IXCOM12 = new BookChapter(
         new Book("Proceedings of the 12th International Congress on X-Ray Optics and Microanalysis", "", 1991, new Author[]{}), "p52-59",
      ...
      if (res) {
         ...
         if (beta > MIN_TILT) {
            ...
            mF = (cosBeta * TiltedBackscatterFactor.compute(mComposition, mShell, mBeamEnergy)
                  * StoppingPower.invToGramPerkeVcmSqr(sp.computeInv(mComposition, mShell, mBeamEnergy))) / icx.computeFamily(mShell, mBeamEnergy);
            ...
            // Calculate the initial slope (p)
            {
               ...
               mLittleA = (pTilt + (mLittleB * ((2.0 * mPhi0) - (mLittleB * mF)))) / ((mLittleB * mF * (2.0 - (mLittleB * rBar))) - mPhi0);
