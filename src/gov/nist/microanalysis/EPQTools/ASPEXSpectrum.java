...
public class ASPEXSpectrum
   ...
   private static String[] mIgnoredList = {
      "dead_percent",
      "type4et",
      "composition",
      "peak_label",
   ...
   public ASPEXSpectrum(File file, boolean withImage)
         throws EPQException {
      this(file);
      if(withImage)
         try {
            ...
            if(!Double.isNaN(mag)) {
               ...
               try (final FileImageInputStream fiis = new FileImageInputStream(file)) {
                  ...
                  if(nImgs > 0) {
                     ...
                     final ScaledImage scImg = new ScaledImage(bi, microFov, (microFov * bi.getHeight())
                           / bi.getWidth(), stgPos, "SE");
                     getProperties().setImageProperty(SpectrumProperties.MicroImage, scImg);
                  }
                  if(nImgs > 1) {
                     final BufferedImage bi = ir.read(1);
                     final ScaledImage scImg = new ScaledImage(bi, macroFov, (macroFov * bi.getHeight())
                           / bi.getWidth(), stgPos, "BSE");
   ...
   public ASPEXSpectrum(File file)
         throws EPQException {
      super();
      {
         try {
      ...
      if(eq != -1)
         try {
            ...
            if(tag.equals("live_time"))
               ...
               if(val.length() > 0)
                  sp.setTextProperty(SpectrumProperties.SpecimenDesc, val + " "
                        + sp.getTextWithDefault(SpectrumProperties.SpecimenDesc, ""));
   ...
   static public String readImageDescription(File file, int imageIndex)
