   ...
   static class BoteSalvatCrossSection extends AbsoluteIonizationCrossSection {
      ...
      public double computeShell(AtomicShell shell, double energy) {
         ...
         if ((iz < Element.elmH) || (iz > Element.elmEs))
            throw new EPQFatalException("Unsupported element in BoteSalvatCrossSection.computeShell: " + shell.getElement().toAbbrev());
      ...
      public double computeShell(AtomicShell shell, double beamE) {
         ...
         if (u > 1.0) {
            ...
            final double f = ((2.0 + i) / (2.0 + t)) * Math2.sqr((1.0 + t) / (1.0 + i))
                  * Math.pow(((i + t) * (2.0 + t) * Math2.sqr(1.0 + i)) / ((t * (2.0 + t) * Math2.sqr(1.0 + i)) + (i * (2.0 + i))), 1.5);
            res = ((shell.getGroundStateOccupancy() * Math2.sqr((PhysicalConstants.BohrRadius * PhysicalConstants.RydbergEnergy) / ee) * f * psi * phi
                  * Math.log(u)) / u);
         }
         return res;
      }
