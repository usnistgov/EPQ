...
public class GetDPspec {
   ...
   private double[] readRes(String resolutionFileName, int numVals)
   ...
   /**
    * Reads FEA results produced by getdp. Getdp results are in two files. One
    * is a preresolution (*.pre) file that provides a descriptive line for each
    * equation in the FEA problem, both those that relate to nodes with unknown
    * (to be solved for) potentials and those with fixed potentials. The other
    * is a resolution (*.res) file that provides the solved values of potentials
    * at the unknown nodes. This form of readResolution() can be used when the
    * two files have extensions .pre and .res and are otherwise the same. The
    * argument is the common stem.
    *
    * @param fileNameStem
    */
   private void readResolution(String fileNameStem)
         throws FileNotFoundException {
      readResolution(fileNameStem + ".pre", fileNameStem + ".res");
   }

   /**
    * Reads finite element analysis results produced by getdp. Getdp results are
    * in two files. One is a .pre preresolution file that provides a descriptive
    * line for each equation in the FEA problem, both those that relate to nodes
    * with unknown (to be solved for) potentials and those with fixed
    * potentials. The other is a .res resolution file that provides the solved
    * values of potentials at the unknown nodes. The inputs are strings
    * providing paths and names for each of these.
    * </p>
    * <p>
    * Note that this method updates the solution values (potentials) for nodes
    * of the mesh, but it does not update cached values (e.g., in any already
    * instantiated Elements of the mesh) that depend on them. The calling
    * routine is responsible for any such updates, as needed.
    *
    * @param preResolutionFileName
    * @param resolutionFileName
    */
   private void readResolution(String preResolutionFileName, String resolutionFileName)
         throws FileNotFoundException {
      ...
      try {
         ...
         /*
          * read lines from pre file and interleave constrained and
          * unconstrained values as it dictates.
          */
         for(int i = 1; i <= numEntities; i++) {
            ...
            switch(basisfun) {
               case 1: // Single node basis functions
                  switch(type) {
                     ...
                     case 2:
                        ...
                        break;
                     default:
                        throw new EPQFatalException("Unexpected dof type = " + Integer.toString(type) + " in "
                              + preResolutionFileName);
                  }
                  break;
               case 2: // grouped nodes basis function
                  switch(type) {
                     case 1: // Unknown potential
                        ...
                        /*
                         * Assign val to all nodes of each element in the
                         * appropriate group
                         */
               ...
               default: /*
                         * There can be others (e.g., associated global
                         * quantities like charge) but there's currently nothing
                         * to be done with them. We just read to the end of the
                         * line.
                         */
                  s.nextLine();
                  break;
      ...
      /*
       * updateAllPotentials(); was a previous strategy, but it may be a net
       * waste of time to update a very large cache of tetrahedra merely on the
       * chance that some of them will be reused before the next update. Current
       * strategy is to delete the cache, forcing it to start over. There is
       * more overhead for each tet that we end up re-using (because we must
       * instantiate it in addition to updating its potentials) but we'll do
       * this to fewer tets.
       */
    ...
    * Normally this is something like "getdp_sparskit sample -pre EleSta_v
    * -cal", possibly with additional command line options. runGetDP is
    * ordinarily called from an IFEArunner, which manages the user's desired FEA
    * options and constructs the appropriate corresponding command line by which
    * to invoke GetDP.
    *
    * @param command
   ...
   public void runGetDP(String command)
      ...
      try {
         ...
         // Handle errors
         final StreamToFile errorStreamHandler = new StreamToFile(child.getErrorStream(), new File(feaFolder, "errOut.txt"));

         // Handle output
         final StreamToFile outputStreamHandler = new StreamToFile(child.getInputStream(), new File(feaFolder, "out.txt"));
   ...
   public void writeFEAfiles()
      ...
      try {
         ...
         throw new EPQFatalException("Exception attempting to copy IntegrationLib.pro to Integration_lib.pro: "
               + err.getMessage());
      }
      ...
      try {
         ...
         throw new EPQFatalException("Exception attempting to copy JacobianLib.pro to Jacobian_lib.pro: " + err.getMessage());
      }
    ...
    * the designated destination folder. For a complete GetDP problem
    * specification we use these files: sample.msh, sample.pro, EleSta_v.pro,
    ...
    * things as constraints or the values of dielectric constant and charge
    * density in various regions. Some of these (constraints and dielectric
    * constants) are typically fixed for a given simulation but may vary from
    * one simulation to another. The charge density changes even within a
    * simulation as charge accumulates or dissipates.
    * </p>
    * <p>
    * Function values and constraints are specified in the samples.pro file,
    * which therefore must be rewritten for each different FEA solution. This
    * file references meshed regions via the corresponding element tag numbers,
    * which generally need to be modified (e.g., when a previously uncharged
    * element becomes charged). This function should be called immediately prior
    * to each FEA solution to write sample.pro and sample.msh files that are
    * appropriate to the current state of the mesh.
    *
    * @throws IOException
    */
   private void writeMesh()
      ...
      /*
       * baseMap will contain information needed for tag assignments. It maps
       * epsr to a Long[3] = {base,minimum tag assigned so far,maximum tag
       * assigned so far.} The algorithm assigns base = n*base1 where n=1 for
       * the first epsr encounted, n=2 for the second, etc. Thus, materials with
       * a given epsr are assigned tags in the range base to base+base1-1. tag =
       * base is reserved for those with q = 0. The others are assigned
       * consecutive integers as they are encountered.
       */
      final HashMap<Double, Long[]> baseMap = new HashMap<Double, Long[]>();
      /*
       * qMap stores the mapping between newly assigned tag value and the
       * associated charge. This is needed when we write the sample.pro file.
       */
      final HashMap<Long, Integer> qMap = new HashMap<Long, Integer>();
      /*
       * epsqToTagMap stores the mapping from epsr and q to the corresponding
       * tag. It gets checked to see if a tag has already been assigned for a
      ...
      try {
         ...
         /*
          * Floating constraints are associated with both surfaces and their
          * contained volumes. The surface tag is in the constraintMap, but the
          * volume tags are not, so we have to make a separate list.
          */
         ...
         for(int i = 1; i <= nElements; i++) {
            ...
            if(!constraintMap.containsKey(tags[0]) && mesh.isVolumeType(i) && !constrainedVolumeTagSet.contains(tags[0])) {
               /*
               ...
               if(baseMap.containsKey(epsr)) {
                  ...
                  /*
                   * This is the first time we've encountered this value of
                   * epsr. initialize its baseMap and epsqToTagMap entries in
                   * addition to assigning the tag.
                   */
                  lastBase += base1;
                  if(nq == 0)
                     tags[0] = lastBase;
            ...
            /*
             * TODO I've represented tags internally as longs, so they are very
             * unlikely to overflow. However, it is not AS unlikely that they
             * will exceed Integer.MAX_VALUE, and it is not at all clear that
             * GetDP, when it imports such values, will be able to read them.
             * (It is possible it uses int instead of long.) At some point, for
             * safety's sake, I should check for an overflow and revise the tag
             * values downward if necessary.
             */
      ...
      try {
         ...
         if(constraintCount[2] > 0) { /*
                                       * Add GlobalCharge terms if we have any
            ...
            for(int i = 0; i < constraintTags.length; i++) {
               ...
               if(c instanceof FloatingConstraint) {
                  ...
                  // cF.computeCharge();
                  sampleOut.write("      { Region " + constraintNames.get(i) + "; Value " + cF.getCharge() + ";}");
                  sampleOut.newLine();
               }
            }
      ...
      try {
         ...
         if(constraintCount[2] == 0) {
            ...
            out.write("        Support Region[{Domain_Ele,DomainC_Ele}]; Entity NodesOf[ All , Not FloatConstrained ]; }");
            out.newLine();
            ...
            out.newLine();
            out.write("      { NameOfCoef GlobalElectricCharge; EntityType GroupsOfNodesOf; NameOfConstraint GlobalElectricCharge; }");
            out.newLine();
            out.write("      { NameOfCoef GlobalElectricPotential; EntityType GroupsOfNodesOf; NameOfConstraint GlobalElectricPotential; }");
         }
         out.newLine();
         ...
         for(int i = 0; i < constraintTags.length; i++) {
            ...
            if(!(c instanceof NeumannConstraint))
               continue;
            out.write("      Galerkin { [ " + Double.toString(-((NeumannConstraint) c).getNormalE()) + " , {v} ]; In "
                  + constraintNames.get(i) + "; ");
            out.newLine();
    ...
    * @throws IOException
    */
   private void writeResource(String resourceName, String destFileName)
         throws IOException {
