...
public class GageSpectrum extends BaseSpectrum {
							...
							if (x == FLAG) { // Marks the end of the pulse
								if (al != null) {
									...
									if (p > pos) {
										pos = p;
										al.actionPerformed(
												new ActionEvent(this, pos, Double.toString(0.1 * pos) + "% complete"));
									}
								}
								// int ts=
	...
	/**
	 * Allows the user to specify an alternative implementation of ProcessRawPulse.
	 * 
	 * @param processor The value to which to set processor.
	 */
