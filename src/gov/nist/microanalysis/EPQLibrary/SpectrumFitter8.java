...
public class SpectrumFitter8 {
	...
	private static final String[] POLYNOMIAL_UNITS = new String[] { "eV", "eV/ch", "eV/ch<sup>2</sup>",
			"eV/ch<sup>3</sup>", "eV/ch<sup>4</sup>", "eV/ch<sup>5</sup>", "eV/ch<sup>6</sup>", "eV/ch<sup>7</sup>",
			"eV/ch<sup>8</sup>", "eV/ch<sup>9</sup>", "eV/ch<sup>10</sup>", };
	private static final String[] POLYNOMIAL_NAMES = new String[] { "Constant", "Linear", "Quadratic", "Cubic",
			"Quartic", "Quintic", "Sextic", "Septic", "Octic", "Nonic", "Decic" };

	/**
	 * <p>
	 * Used to model the resolution of the detector using a standard fano function /
	 * electronic noise model. The function may be used to fit 0 (nothing), 1 (noise
	 * only) or 2 (noise/fano) parameters.
	 * </p>
	 * <p>
	 * Copyright: Pursuant to title 17 Section 105 of the United States Code this
	 * software is not subject to copyright protection and is in the public domain
	...
	static public class FanoNoiseWidth extends FunctionImpl {
		...
		public FanoNoiseWidth(int nDim, double fano, double noise) {
			super();
			mFano = add(new ParameterObject<String>("Fano-Noise[Fano]", new Constraint.Bounded(DEFAULT_FANO, 0.1), fano,
					nDim > 1, "Fano"));
			mNoise = add(new ParameterObject<String>("Fano-Noise[Noise]", new Constraint.Positive(6.0), noise, nDim > 0,
					"Noise"));
		}
		...
		public double derivative(double eV, Map<Parameter, Double> param, Parameter idx) {
			if (isFitParameter(idx)) {
				...
				if (idx.equals(mFano))
					...
					return (noise * ENERGY_PER_EH_PAIR)
							* Math.pow((noise * noise) + ((eV * fano) / ENERGY_PER_EH_PAIR), -0.5);
			}
			return 0.0;
		}
		...
		public UncertainValue2 computeU(double eV, Map<Parameter, UncertainValue2> param) {
			...
			return UncertainValue2.multiply(ENERGY_PER_EH_PAIR,
					UncertainValue2.sqrt(UncertainValue2.add(UncertainValue2.sqr(noise),
							UncertainValue2.divide(UncertainValue2.multiply(eV, fano), ENERGY_PER_EH_PAIR))));
		}
	...
	static public class EnergyScaleFunction extends FunctionImpl implements InvertableFunction {
		...
		public EnergyScaleFunction(double[] coeff, int nFit) {
			...
			for (int i = 0; i < mCoeff.length; ++i) {
				...
				mCoeff[i] = add(new Parameter("pow[ch," + Integer.toString(i) + "]",
						new Constraint.Bounded(def, 100.0 * Math.pow(10.0, -3.0 * i)), def, true));
			}
		...
		public EnergyScaleFunction(double[] coeff, boolean[] fitP) {
			...
			for (int i = 0; i < mCoeff.length; ++i)
				mCoeff[i] = add(new Parameter("pow[ch," + Integer.toString(i) + "]",
						new Constraint.Bounded(coeff[i], 100.0 * Math.pow(10.0, -3.0 * i)),
		...
		public UncertainValue2 inverse(double eV, Map<Parameter, UncertainValue2> param) {
			...
			if (mCoeff.length == 2)
				...
				if (res == null)
					...
					return Math.abs(res[0].doubleValue() - linear.doubleValue()) < Math
							.abs(res[1].doubleValue() - linear.doubleValue()) ? res[0] : res[1];
			} else {
				final double[] coeffs = new double[mCoeff.length];
				for (int i = 0; i < coeffs.length; ++i)
					coeffs[i] = mCoeff[i].getUncertainValue(param).doubleValue();
				...
				try {
					...
					final double bestEst = Math.abs(est[0] - linear.doubleValue()) < Math
							.abs(est[1] - linear.doubleValue()) ? est[0] : est[1];
	...
	static public class AltEnergyScaleFunction extends FunctionImpl implements InvertableFunction {
		...
		@Override
		public double compute(double ch, Map<Parameter, Double> param) {
			return mCoeff[0].getValue(param) + (ch * mCoeff[1].getValue(param))
					+ (mCoeff[2].getValue(param) * Math.sqrt(ch));
		}
		...
		public UncertainValue2 computeU(double ch, Map<Parameter, UncertainValue2> param) {
			return UncertainValue2.add(
					UncertainValue2.add(mCoeff[0].getUncertainValue(param),
							UncertainValue2.multiply(ch, mCoeff[1].getUncertainValue(param))),
	...
	/**
	 * <p>
	 * A lineset represents one or more x-ray transitions. When there is more than
	 * one line, each line can be scaled independently to allow forcing the relative
	 * weights of lines.
	 * </p>
	 * <p>
	 * Copyright: Pursuant to title 17 Section 105 of the United States Code this
	...
	private class Lineset extends FunctionImpl {
		...
		private final class LineData {
			...
			final double compute(double eV, Map<Parameter, Double> param) {
				...
				assert (mIncomplete.getValue(param) >= 0) && (mIncomplete.getValue(param) <= MAX_INCOMPLETE) : Double
						.toString(mIncomplete.getValue(param));
				return (mFitIncompleteCharge && (x < 0.0) ? 1.0 - (mIncomplete.getValue(param) * x) : 1.0)
						* mAmplitude.getValue(param) * mWeight * Math.exp(-0.5 * Math2.sqr(x));
			}
			...
			final UncertainValue2 computeU(double eV, Map<Parameter, UncertainValue2> param) {
				...
				final UncertainValue2 k = (x.doubleValue() < 0.0) && mFitIncompleteCharge
						? UncertainValue2.subtract(UncertainValue2.ONE,
								UncertainValue2.multiply(x, mIncomplete.getUncertainValue(param)))
						: UncertainValue2.ONE;
				return UncertainValue2.multiply(k,
						UncertainValue2.multiply(mAmplitude.getUncertainValue(param), UncertainValue2.multiply(mWeight,
								UncertainValue2.exp(UncertainValue2.multiply(-0.5, UncertainValue2.sqr(x))))));
			}
			...
			final UncertainValue2 integrate(Map<Parameter, UncertainValue2> param) {
				...
				final int chMin = (int) Math
						.round(mScale.inverse(Math.max(0.0, mEnergy - (K * w.doubleValue())), param).doubleValue());
				final int chMax = (int) Math
						.round(mScale.inverse(mEnergy + (K * w.doubleValue()), param).doubleValue());
				UncertainValue2 res = UncertainValue2.ZERO;
		...
		private Lineset(XRayTransitionSet xrts, double scale, Constraint con) throws EPQException {
			...
			mIncomplete = add(new ParameterObject<XRayTransitionSet>("Incomplete[" + xrts.toString() + "]",
					new Constraint.Bounded(0.5 * MAX_INCOMPLETE, MAX_INCOMPLETE), 0.0 * MAX_INCOMPLETE,
					mFitIncompleteCharge, xrts));
				...
				if (idx.equals(mAmplitude))
					...
					if (mResolution.isFitParameter(idx)) {
						...
						for (final LineData ld : mLines) {
							...
							if (mFitIncompleteCharge && (eV < c))
								res += (((xx2 / r) - (1.0 / r)) + (1.0 / ((k * (c - eV)) + r))) * f
										* mResolution.derivative(ld.mEnergy, param, idx);
							else
		...
		public UncertainValue2 computeU(double ch, Map<Parameter, UncertainValue2> param) {
			...
			for (final LineData ld : mLines) {
				final UncertainValue2 xx = UncertainValue2.divide(
						UncertainValue2.subtract(eV, new UncertainValue2(ld.mEnergy)),
						mResolution.computeU(eV.doubleValue(), param));
				res = UncertainValue2.add(res, UncertainValue2.multiply(ld.mWeight,
						UncertainValue2.exp(UncertainValue2.multiply(-0.5, UncertainValue2.multiply(xx, xx)))));
			}
	...
	/**
	 * Creates a MultiLineset build of (mostly) independent transitions. The mostly
	 * comes from the limit that lines which are too close (as determined by de) are
	 * fit as a unit.
	 * 
	 * @param rois     RegionOfInterestSet
	 * @param de       Min separation between distinct lines in eV
	 * @param fraction The fractional variability to permit in the NL fit
	 * @return MultiLineset
	 * @throws EPQException
	 */
	 ...
	 * XRayTransitions are each added separately and fit separately. If de&gt;0.0,
	 * then if two XRayTransition objects differ in energy by less than de then the
	 * XRayTransition objects are placed in the same Lineset and will be fit as a
	 * unit.
	 * 
	 * @param mls      MultiLineset
	 * @param param    Map&lt;Parameter, UncertainValue&gt;
	 * @param de       The energy spacing in eV
	 * @param fraction Change to permit in each fit parameter
	 * @return MultiLineset
	 * @throws EPQException
	 */
	public MultiLineset makeIndependent(MultiLineset mls, Map<Parameter, UncertainValue2> param, double de,
			double fraction) throws EPQException {
		final MultiLineset res = new MultiLineset();
		// Sort the LineData in element, energy order...
		final TreeMap<Lineset.LineData, Lineset> ssld = new TreeMap<Lineset.LineData, Lineset>(
				new Comparator<Lineset.LineData>() {
					@Override
	...
	/**
	 * Constructs a SpectrumFitter8 to calibrated the specified detector using the
	 * specified spectrum collected from the specified composition material.
	 * 
	 * @param det
	 * @param comp
	 * @param spec
	...
	/**
	 * Constructs a SpectrumFitter8 to reuse the energy and resolution calibration
	 * provided by detCal on the ISpectrumData spec which was collected on a
	 * material of Composition comp.
	 * 
	 * @param detCal
	 * @param comp
	 * @param spec
	 ...
	 * @throws EPQException
	 */
	public SpectrumFitter8(SpectrumFitter8 detCal, Composition comp, ISpectrumData spec)
	...
	public RegionOfInterestSet getROIS() {
		if (mROIS == null) {
			final double e0 = mSpectrum.getProperties().getNumericWithDefault(SpectrumProperties.BeamEnergy,
					Double.NaN);
	...
	private static ISpectrumData extractCharacteristicSpectrum(ISpectrumData spec, EDSDetector det, Composition comp,
			RegionOfInterestSet rois, BremsstrahlungAnalytic bs) throws EPQException {
	...
	static private double[] createChannelList(ISpectrumData spec, final RegionOfInterestSet rois) {
		...
		for (final RegionOfInterest roi : rois) {
			final int minCh = Math.max(lld,
					SpectrumUtils.bound(spec, SpectrumUtils.channelForEnergy(spec, FromSI.eV(roi.lowEnergy()))));
			final int maxCh = Math.max(lld,
					SpectrumUtils.bound(spec, SpectrumUtils.channelForEnergy(spec, FromSI.eV(roi.highEnergy()))));
	 ...
	 * the resolution and weights relative to one spectrum, it may be useful to use
	 * the same energy scale and resolution functions to perform a linear scale-only
	 * fit on other spectra. That is what this inner class does. It takes the scale
	 * and resolution fit from the previously calibrated spectrum in SpectrumFitter8
	 * and holds this firm while adjusting the intensity scales to best fit a new
	 * spectrum. The fit weights are then returned.
	 * </p>
	 * <p>
	 * Copyright: Pursuant to title 17 Section 105 of the United States Code this
	 * software is not subject to copyright protection and is in the public domain
	...
	public class HighQualityLinearFit extends LinearLeastSquares {
		...
		public HighQualityLinearFit(ISpectrumData spec, Composition comp, double de)
				throws EPQException, UtilException {
			final double e0 = spec.getProperties().getNumericWithDefault(SpectrumProperties.BeamEnergy, Double.NaN);
			assert !Double.isNaN(e0);
			final DetectorLineshapeModel dlm = mDetector.getDetectorLineshapeModel();
			final double fwhm = dlm.getFWHMatMnKa();
			final RegionOfInterestSet rois = new RegionOfInterestSet(dlm, 1.0e-6, ToSI.eV(0.3 * fwhm),
					ToSI.eV(0.3 * fwhm));
			...
			mHQComposition = comp;
			mHQSpectrum = spec;
			mHQCharSpectrum = extractCharacteristicSpectrum(spec, mDetector, comp, rois,
					new BremsstrahlungAnalytic.DTSAQuadratic());
		...
		public SpectrumFitResult getResult() throws EPQException {
			...
			for (final Lineset ls : mLineset)
				for (final Lineset.LineData ld : ls.mLines) {
					...
					res.addTransition(ld.mTransition, ld.computeU(ld.mEnergy, pm), ld.integrate(pm), gaussianW,
							mScale.inverse(ld.mEnergy, pm));
				}
	...
	/**
	 * Performs a very crude fit as a first estimate of the amplitude of the various
	 * linesets.
	 * 
	 * @param chD
	 * @throws EPQException
	...
	public SpectrumFitResult compute() throws EPQException, UtilException {
		...
		res.setBremsstrahlungModel(mBremsstrahlung, new UncertainValue2(mBremsstrahlung.mA),
				new UncertainValue2(mBremsstrahlung.mB));
	...
	/**
	 * Recomputes the fit but first splits weighted transitions apart so as to fit
	...
	/**
	 * Perform a QC analysis on the specified spectrum and return the results as a
	 * TreeMap&lt;String,UncertainValue&gt;. The ISpectrum
	 * 
	 * @param det  EDSDetector The detector on which the spectrum was collected
	 * @param std  Composition The approximate composition of the material from
	 *             which the spectrum was collected.
	 * @param spec ISpectrumData The spectrum
	 * @param mode QCNormalizeMode
	 * @param al   {@link ActionListener} (May be null)
	 * @return TreeMap&lt;String,UncertainValue&gt;
	 * @throws EPQException
	 * @throws UtilException
	 */
	public static TreeMap<String, UncertainValue2> performQC(EDSDetector det, Composition std, ISpectrumData spec,
			QCNormalizeMode mode, ActionListener al) throws EPQException, UtilException {
	...
	public RegionOfInterestSet getRegionOfInterests(Element elm) {
		...
		final DetectorLineshapeModel dlm = mDetector.getDetectorLineshapeModel();
		final RegionOfInterestSet rois = new RegionOfInterestSet(dlm, MIN_I, ToSI.eV(0.5 * dlm.getFWHMatMnKa()),
				ToSI.eV(0.25 * dlm.getFWHMatMnKa()));
	...
	/**
	 * Returns true if the fitter is configured to fit incomplete charge collection.
	 * 
	 ...
	 * 
	 * @param fitIncompleteCharge The value to which to set fitIncompleteCharge.
	 */
