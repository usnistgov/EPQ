...
public class ParticleSignature {
   ...
   private final Set<Element> mStrip; // Those elements not included in the full
                                      // normalization
   ...
   public ParticleSignature(KRatioSet krs, Collection<Element> strip) {
      mNormalization = null;
      mFullNorm = null;
      mStrip = new TreeSet<Element>();
      if(strip != null)
   ...
   public ParticleSignature(KRatioSet krs) {
      this(krs, null);
   }

   public ParticleSignature(Collection<Element> strip) {
      super();
      mNormalization = UncertainValue2.ONE;
      mFullNorm = UncertainValue2.ONE;
   ...
   public void add(Element elm, UncertainValue2 value) {
      if(!value.isNaN()) {
         if((value.doubleValue() < 0) && ((value.doubleValue() + value.uncertainty()) > 0.0))
            value = new UncertainValue2(0.0, "PS", value.doubleValue() + value.uncertainty());
   ...
   public void add(KRatioSet krs, double e0) {
      for(final Element elm : krs.getElementSet()) {
         UncertainValue2 uv = UncertainValue2.ZERO;
         // Select the most energetic with U>1.333
         double bestEE = Double.NaN;
         for(final XRayTransitionSet xrts : krs.getTransitions(elm)) {
            final double ee = xrts.getWeighiestTransition().getEdgeEnergy();
            if(Double.isNaN(bestEE) || ((bestEE < ee) && (ee < (0.75 * e0)))) {
               bestEE = ee;
               uv = krs.getKRatioU(xrts);
            }
         }
         add(elm, uv);
      }
   ...
   private UncertainValue2 getNorm(Element elm) {
      return mStrip.contains(elm) ? getFullNorm() : getNorm();
   }
