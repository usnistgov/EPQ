...
public class SEmaterial extends Material implements Cloneable {
	 ...
	 * represented here by discrete binding energy/occupancy pairs. In the case of
	 * materials with a continuous band of states, this discrete representation must
	 * be regarded as an approximation, but it may be made as close as we wish by
	 * dividing the band into as many discrete states as we deem necessary. In many
	 * cases it may not be necessary to use very many. If SE yield is the main
	 * concern, it may be adequate, and a good deal more computationally efficient,
	 * to approximate a band by one or a very few states of well-chosen binding
	 * energy(ies). The density of occupied electronic states is here represented by
	 * 3 arrays. These are lists of binding energies, kinetic energies, and
	 * densities of electrons at those binding energies (in electrons/m^3). The
	 * total occupancy of all states should sum to the actual number of target
	 * electrons per unit volume in the material. Binding energies are positive for
	 * bound electrons and are measured with respect to the vacuum. Attempting to
	 * assign a negative binding energy or negative electron density will cause a
	 * runtime EPQFatalException.
	 */
	 ...
	 * levels. For a metal, this is the energy relative to the Fermi level. They are
	 * used by some ScatterMechanisms--e.g., TabulatedInelasticSM, which assumes an
	 * energy transfer greater than one of these represents excitation of a core
	 * electron. This may be redundant with bindingEnergy, above, but coreEnergy
	 * states need not be accompanied by kinetic energies or densities, since
	 * TabulatedInelasticSM doesn't need those. For now I'm implementing this as a
	 * TreeSet. The TreeSet does not allow duplicate energy levels. Such duplicates
	 * do appear in some references from some materials. However, such duplicates
	 * have no effect on any of the algorithms that use the coreEnergy so far. (I
	 * don't think they could, unless they depended upon some additional property of
	 * the state, like a quantum number, but these other properties are not
	 * presently stored either.) Presently there is no effect of storing duplicate
	 * energies except to expend time getting past them to the next unique entry;
	 * thus use of TreeSet is all benefit and no loss. If in the future I do need to
	...
	/**
	 * SEmaterial - Constructs an SE material of specified composition and density,
	 * but with parameters otherwise appropriate for vacuum. For a realistic
	 * material the constructor call must be followed by other methods to set the
	 * work function, plasmon energy, energy of the conduction band bottom, and
	 * density of occupied electronic states.
	 *
	 ...
	 * electronic states.
	 *
	 * @param elms        - Element[] An array of elements that comprise this
	 *                    material
	...
	/**
	 * SEmaterial - Constructs an SE material with the same composition and density
	 * as mat, but with specifically secondary electron properties set as
	...
	public void addBindingEnergy(double bindingEnergy, double density) {
		...
		/*
		 * Use default kinetic energy for this state. The default for the conduction
		 * band is to use the difference between the energy and the conduction band
		 * bottom. For other bands we use kinetic energy = binding energy, a choice
		 * based upon the virial theorem. -- Or should I use binding energy + CBbottom?
		 */
	...
	/**
	 * addBindingEnergy - Adds a binding energy, corresponding kinetic energy, and
	 * density to density of states. The binding energy is the difference, vacuum
	 * energy minus electron's total energy. This number is positive for bound
	 * electrons. The density is the number of electrons per cubic meter of this
	 * material that has this binding energy.
	 *
	 * @param bindingEnergy - binding energy in Joules
	 ...
	 * adding multiple binding energies and densities, because the re-initialization
	 * of the scatter mechanisms is only done once at the end.
	 *
	 * @param bindingEnergy - binding energy in Joules
	 * @param density       - # electrons/meter^3 in this material at this binding
	 *                      energy
	...
	/**
	 * addCoreEnergy - Adds a core energy to the existing list. (Current values are
	 * not cleared.) The core energies are the minimum energies to excite electrons
	 * out of atomic core levels. That is, they are the energy differences between
	 * core level and lowest unoccupied state (Fermi energy For a metal, conduction
	 * band minimum for a non-metal).
	 *
	...
	/**
	 * addCoreEnergy -- Adds all core energies in a list. (Previous values are not
	 * cleared.)
	 *
	...
	/**
	 * Returns a "version" number. This number starts at 0 and is incremented each
	 * time the material is altered in any way (e.g., by changing a parameter or
	 * adding or removing a scatter mechanism). A changed version number alerts a
	 * client to refresh its cache.
	 */
	/*
	 ...
	 * EFermi = -workfunction. To re-reference it to the conduction band bottom we
	 ...
	 * material.
	 *
	 * @return Double[] - A copy of the array of electron densities (electrons/m^3).
	 */
	 ...
	 * method would determine reasonable binding energy/electron density values to
	 * be used for this material. For example, it could set the binding energy equal
	 * to the work function and the electron density to the average number of
	 * lightly bound electrons in atoms of this material. This can be discovered by
	 * using the EPQLibrary's AtomicShell getEnergy() and getGroundStateOccupancy()
	 * methods.
	 */
	...
	/**
	 * removeBindingEnergy - Removes a binding energy, kinetic energy, and density
	 * of states triplet from the existing list.
	 *
	...
	/**
	 * removeCoreEnergy - Removes the specified core energy from the existing list.
	 *
	 ...
	 * existing list. The index corresponds to the core energies in sorted order, as
	 * returned by getCoreEnergyArray().
	 *
	...
	/**
	 * setBindingEnergy -- Changes the energy associated with the state at a given
	 * index to a new value. If default kinetic energy values have not been
	 * overridden, the corresponding kinetic energy is set according to the default
	 * algorithm.
	 *
	 ...
	 * previous values are cleared.) @param coreEnergy - Double[] Energies of core
	 * levels in Joules
	 */
	...
	/**
	 * setCoreEnergy - Sets the core energy list to the supplied list. (Any previous
	 * values are cleared.)
	 *
	...
	/**
	 * energyCBbottom is the energy of the conduction band bottom. The vacuum energy
	 * is defined to be 0, so energyCBbottom should be negative.
	 *
	...
	/**
	 * bandgap is the width of an insulating or semiconducting material's bandgap,
	 * in Joules. bandgap = 0 for conductors.
	 *
	 ...
	 * different angular momentum states) and then a gap. Requiring this gap to be a
	 * minimum size might be more reliable than requiring the absolute energy to be
	 * bigger than a certain size. Alternatively, maybe Nicholas's utilities will
	 * tell me the principle quantum number, and I just want energies for states
	 * with principle quantum numbers different from the valence shell's number.
	 */
	final double coreEnergyCutoff = ToSI.eV(20.);

	/**
	 * setEstimatedCoreEnergy - Resets the list of core energy levels to estimated
	 * values. The estimated values for an element are the atomic shell edge
	 * energies for the shells with nonzero ground state occupancy. Shells with
	 * energies less than a cutoff (specifiable, 20 eV default) are not considered
	 * to be core states, and they are excluded. The edge energies values are
	 * determined by calling EPQLibrary's AtomicShell.getEdgeEnergy(). For a
	 * composite material, the estimate is determined by merging the lists energies
	 * from the constituent elements. Known limitations: (1) This procedure does not
	 * take into account energy shifts that depend upon the state of the material
	 * (e.g, atomic vs. solid). (2) All constituents of composite materials are
	 * included, even those that might be present only in trace amounts. Best
	 * accuracy is therefore achieved by looking up the values in a good reference
	 * and setting them using one of the other methods.
	 */
	...
	/**
	 * setKEtoDefault -- resets kinetic energies according to the default algorithm.
	 * The default is for the kinetic energies associated with states in the
	 * conduction band to be the difference between the electron energy and the
	 * energy of the band bottom. For states below the conduction band the kinetic
	 * energy by default is set equal to the distance of that state below the vacuum
	 * level. This choice is motivated by the virial theorem. It should be a good
	 * value for deep core levels. Its value for shallow states (e.g., the valence
	 * band) is not established. It is probably as good a guess as one can make
	 * absent specific band structure information. If such information is available,
	 * the default should be overridden.
	 */
	void setKEtoDefault() {
	...
	/**
	 * setKineticEnergy -- Changes the kinetic energy associated with the state at a
	 * given index to a new value.
	 *
	 * @param index  - the index of the entry to change
	 * @param energy - the new density in electrons/m^3
	 ...
	 * function is the positive difference in energy between vacuum and the highest
	 * occupied state, whether in the conduction band or, if the conduction band is
	 * empty, the valence band. States (such as trap states within the band gap)
	 * that have insignificant density and so are unlikely to be a source of
	 * secondary electrons may be ignored in making this assignment.
	 *
	 ...
	 * models. By default it is set to infinity (no charge redistribution occurs).
	 *
	 * @param breakdownField - the magnitude of the electric field (volts/meter) for
	 *                       which there is significant conduction.
	 */
