...
public interface IConductionModel {
    ...
    *           propagate charges.
    * @param deltat - double specifying the time for which to propagate charges.
    * @param feaRunner - An IFEArunner that can be used to do finite element
    *           analysis if needed by the algorithm
    * @return - true if FEA is needed (e.g., if any charges were moved and
    *         without subsequent FEA).
    */
   public boolean conductCharges(MeshedRegion meshedRegion, double deltat, IFEArunner feaRunner);

   /**
    * When the scattering code decides to stop following an electron in a
    * nominally insulating region, it calls this routine to determine what
    * region the electron should stop in. The input startRegion is the
    * electron's region when the scattering code is finished with it. The
    * conduction model may, e.g., if the electric field in this region is high
    * enough, decide to put the electron in a different MeshElementRegion, which
    * it returns. It may return null, e.g., if the stopRegion lies outside the
    * mesh.
    *
