...
public class ControlChart
   ...
   public void setRange(Date minDate, Date maxDate, double minY, double maxY) {
      ...
      final long msMinDate = TO_DAY * ((minDate.getTime() - ((2 * (maxDate.getTime() - minDate.getTime())) / 100)) / TO_DAY);
      final long msMaxDate = (TO_DAY * ((maxDate.getTime() + ((2 * (maxDate.getTime() - minDate.getTime())) / 100)) / TO_DAY))
            + TO_DAY;
      mXExtent = new long[] {
         Math.min(msMinDate, msMaxDate),
         Math.max(msMinDate, msMaxDate)
      };
      mXBounds = new Date[] {
         new Date(mXExtent[0]),
         new Date(mXExtent[1])
   ...
   public void autoRange() {
      UncertainValue2 minY = Collections.min(mData.values());
      UncertainValue2 maxY = Collections.max(mData.values());
      if((!Double.isNaN(mLowerLimit)) && ((minY.doubleValue() - minY.uncertainty()) > mLowerLimit))
         minY = new UncertainValue2(mLowerLimit);
      if((!Double.isNaN(mUpperLimit)) && ((maxY.doubleValue() + maxY.uncertainty()) < mUpperLimit))
         maxY = new UncertainValue2(mUpperLimit);
      final double delta = Math.max((maxY.doubleValue() - minY.doubleValue()), 0.1);
      ...
      setRange(minDate, maxDate, minY.doubleValue() - (EXTRA * delta), maxY.doubleValue() + (EXTRA * delta));
   }
   ...
   private Point toCanvas(Date dt, double y) {
      ...
      final double xPos = canvas.getX() + ((canvas.getWidth() * (dt.getTime() - mXExtent[0])) / (mXExtent[1] - mXExtent[0]));
   ...
   public void update() {
      ...
      Rectangle2D.intersect(fm.getStringBounds(xLabel(mXBounds[0]), gr), fm.getStringBounds(xLabel(mXBounds[1]), gr), res);
      ...
      }
      {
         ...
         gr.drawString(mName, (int) Math.round((canvas.x + canvas.width) - (2 * BORDER_WIDTH)
               - rect.getWidth()), (int) Math.round(canvas.y + (2 * BORDER_WIDTH) + rect.getHeight()));
      }
      {
      ...
      if(mDrawStats) {
         ...
         if(!Double.isNaN(avg)) {
            ...
            try {
               ...
               gr.setStroke(new BasicStroke(1.0f, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_MITER, 10.0f, new float[] {
                  12.0f,
                  4.0f
               }, 0.0f));
               gr.drawLine(canvas.x, pt.y, canvas.x + canvas.width, pt.y);
      ...
      for(final Map.Entry<Date, UncertainValue2> me : mData.entrySet()) {
         ...
         gr.drawLine(pt0.x - RADIUS, pt0.y, pt0.x + RADIUS, pt0.y);
         gr.drawLine(pt1.x - RADIUS, pt1.y, pt0.x + RADIUS, pt1.y);
      }
   }
}
