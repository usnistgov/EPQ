 ...
 * @version 1.0
 */
abstract public class AbsorptionCorrection extends AlgorithmClass {

	private static final AlgorithmClass[] mAllImplementations = { AbsorptionCorrection.Philibert,
			AbsorptionCorrection.PhilibertHeinrich, AbsorptionCorrection.PhilibertHeinrichFull };
	...
	protected double computeChi(XRayTransition xrt, Composition comp, double takeOffAngle) throws EPQException {
	...
	public static class PhilibertAbsorption extends AbsorptionCorrection {
		PhilibertAbsorption() {
		...
		public double compute(Composition comp, XRayTransition xrt, double e0, double takeOffAngle)
			...
			final double sigma = 2.54e5
					/ (Math.pow(FromSI.keV(e0), 1.5) - Math.pow(FromSI.keV(xrt.getEdgeEnergy()), 1.5));
			double h = 0;
			...
			// return 1.0 / ((1.0 + (chi / sigma)) * (1.0 + ((h * (chi / sigma)) / (1.0 +
			// h))));
		}
	}
	 ...
	 * claims to implement (as per the XM17330/27330 documentation dated Dec-2001)
	 */
	static public PhilibertHeinrichAbsorption PhilibertHeinrich = new PhilibertHeinrichAbsorption();

	static public class PhilibertHeinrichAbsorption extends AbsorptionCorrection {
		...
		public double compute(Composition comp, XRayTransition xrt, double e0, double takeOffAngle)
			...
			final double sigma = 4.5e5
					/ (Math.pow(FromSI.keV(e0), 1.65) - Math.pow(FromSI.keV(xrt.getEdgeEnergy()), 1.65));
			double h = 0;
			for (final Element el : comp.getElementSet()) {
		...
		/**
		 * Estimates the uncertainty in the absorption correction due to uncertainty in
		 * chi.
		 * 
		 * @param comp
		 * @param xrt
		...
		public double uncertainty(Composition comp, XRayTransition xrt, double e0, double takeOffAngle)
			...
			final double df_dchi = -((1.0 + h) * sigma * sigma * (sigma + (2.0 * h * spc)))
					/ Math2.sqr(spc * (sigma + (h * spc)));
			return Math.abs(df_dchi * dChi);
		}
	};

	public static class PhilibertHeinrichFullAbsorption extends AbsorptionCorrection {
