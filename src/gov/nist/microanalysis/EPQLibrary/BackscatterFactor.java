 ...
 * @version 1.0
 */
abstract public class BackscatterFactor extends AlgorithmClass {
	 ...
	 * item is an implements the BackscatterFactor class.
	 * 
	 * @return List
	 */
	@Override
	...
	/**
	 * Pouchou1991 - The backscatter correction algorithm described in Pouchou and
	 * Pichoir's chapter in the book "Electron Probe Quantitation", Heinrich &amp;
	 * Newbury editors (1991). (Produces very similar results to CITZAF)
	 */
	static public class PouchouBackscatterFactor extends BackscatterFactor {
	...
	/**
	 * Myklebust1991 - The backscatter correction algorithm described in Myklebust
	 * &amp; Newbury's chapter in the book "Electron Probe Quantitation", Heinrich
	 * &amp; Newbury editors (1991).
	 */

	static public class Myklebust91BackscatterFactor extends BackscatterFactor {
		Myklebust91BackscatterFactor() {
			super("Myklebust & Newbury 1991", new LitReference.BookChapter(LitReference.ElectronProbeQuant,
					new LitReference.Author[] { LitReference.RMyklebust, LitReference.DNewbury }));
		}
	 ...
	 * Heinrich's Electron Beam X-ray Microanalysis, 1981. (Note: this algorithm is
	 * only hinted at in Pub. 298. Is the Heinrich's reference wrong?) (Produces
	 * similar numbers as CITZAF.)
	 */
	static public class DuncumbBackscatterFactor extends BackscatterFactor {
		DuncumbBackscatterFactor() {
			super("Duncumb & Reed 1981",
					new LitReference.BookChapter(LitReference.QuantitativeElectronProbeMicroanalysis,
							new LitReference.Author[] { LitReference.PDuncumb, LitReference.SReed }));
		}
		...
		public double compute(Composition comp, AtomicShell shell, double e0) {
			...
			for (final Element el : comp.getElementSet()) {
				...
				final double rZ = 1.0
						+ (((((((((((2.962e-2 * w) - 8.619e-2) * w) + 9.213e-2) * w) - 5.137e-2) * w) + 2.162e-2) * w)
								- 0.581e-2) * z)
						+ (((((((((((-17.676e-4 * w) + 46.540e-4) * w) - 47.744e-4) * w) + 28.791e-4) * w) - 8.298e-4)
								* w) - 1.609e-4) * z * z)
						+ (// The sign of the 110.700 term is different in CITZAF
							// (which
							// is right!!!!!)
						((((((((((41.792e-6 * w) + 110.700e-6) * w) + 120.050e-6) * w) - 75.733e-6) * w) + 19.184e-6)
								* w) + 5.4000e-6) * z * z * z)
						+ (((((((((((-42.445e-8 * w) + 117.750e-8) * w) - 136.060e-8) * w) + 88.128e-8) * w)
								- 21.645e-8) * w) - 5.725e-8) * z * z * z * z)
						+ (((((((((((15.851e-10 * w) - 46.079e-10) * w) + 55.694e-10) * w) - 36.510e-10) * w)
								+ 8.947e-10) * w) + 2.095e-10) * z * z * z * z * z);
	 ...
	 */
	@Deprecated
	public static class YakowitzBackscatterFactor extends BackscatterFactor {
		...
		public String caveat(Composition comp, AtomicShell shell, double e0) {
			...
			if (u > 10)
				return CaveatBase.format(this,
						"This algorithm works very poorly at overvoltages above 10. (u=" + df.format(u) + ")");
			if (u > 5)
				return CaveatBase.format(this,
						"This algorithm has problems with large overvoltages. (u=" + df.format(u) + ")");
	 ...
	 * application. J. de Physique 45, suppliment 2, 1984, C2-41 (Produces similar
	 * numbers as CITZAF.)
	 */
	static public class Myklebust84BackscatterFactor extends BackscatterFactor {
		Myklebust84BackscatterFactor() {
			super("Myklebust/FRAME 1984",
					new LitReference.CrudeReference("J. de Physique 45, suppliment 2, 1984, C2-41"));
		}
		...
		public double compute(Composition comp, AtomicShell shell, double e0) {
			...
			for (final Element el : comp.getElementSet()) {
				final double z = el.getAtomicNumber();
				final double rZ = (1.0 - (0.0081512 * z)) + (3.613e-5 * z * z) + (0.009582 * z * Math.exp(-u))
						+ (0.00114 * e0);
				rAvg += comp.weightFraction(el, true) * rZ;
			}
			return rAvg;
	 ...
	 */
	public static class LoveBackscatterFactor extends BackscatterFactor {
