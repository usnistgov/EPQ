...
abstract public class ElectronRange
   extends
   AlgorithmClass {
    ...
    * material.
    * 
    * @param comp The material's composition
    * @param shell The shell defining an edge energy
    * @param e0 The incident beam energy
   ...
   public static class PouchouElectronRange
      extends
      ElectronRange {
   ...
   public static class LoveElectronRange
      extends
      ElectronRange {
      LoveElectronRange() {
   ...
   public static class KanayaAndOkayamaElectronRange
      extends
      ElectronRange {
      ...
      private double compute(Element elm, double e0) {
         return toKilogramPerMeterSqr((1.0e-4 * 0.0276 * elm.getAtomicWeight() * Math.pow(FromSI.keV(e0), 1.67))
               / (Math.pow(elm.getAtomicNumber(), 0.89)));
      }
