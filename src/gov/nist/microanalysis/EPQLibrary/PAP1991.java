 ...
 * @version 1.0
 */
public class PAP1991 extends CorrectionAlgorithm.PhiRhoZAlgorithm {
	...
	/**
	 * The mean atomic number as defined by Pouchou and Pichoir in the PAP &amp; XPP
	...
	/**
	 * The exponent (m) in the expression for the ionization cross section used in
	 * PAP.
	 * 
	...
	public boolean initialize(Composition comp, AtomicShell shell, SpectrumProperties props) throws EPQException {
		...
		if (res) {
			...
			/*
			 * After puzzling about this for !many! hour I've come to the conclusion that
			 * P&P eqn 13 is wrong. It should read F = (R/S) / QlA(E0). This agrees with the
			 * XMAQNT C library and better yet seems to work.
			 */
	...
	public double computeZACorrection(XRayTransition xrt) throws EPQException {
		...
		final double f1 = (mA1 / chi)
				* ((((((mRc - mRm) * (mRx - mRc - (2.0 / chi))) - (2.0 / chi2)) * Math.exp(-chi * mRc))
						- ((mRc - mRm) * mRx)) + (mRm * (mRc - (2.0 / chi))) + (2.0 / chi2));
		final double f2 = (mA2 / chi)
				* (((((mRx - mRc) * (mRx - mRc - (2.0 / chi))) + (2.0 / chi2)) * Math.exp(-chi * mRc))
