...
public class CzyzewskiMottCrossSection {
   ...
   private static final double kEnergy[] = {
      ToSI.eV(20),
      ToSI.eV(50),
      ToSI.eV(75),
      ToSI.eV(100),
   ...
   private static final double kAngle[] = {
      Math.toRadians(0.0),
      Math.toRadians(1),
      Math.toRadians(2),
      Math.toRadians(3),
   ...
   private void loadTables(int atomicNo) {
      final String name = "CzyzewskiXSec/" + (atomicNo < 10 ? "0" + Integer.toString(atomicNo) : Integer.toString(atomicNo))
            + ".dat";
      try {
         try (final InputStreamReader isr = new InputStreamReader(CzyzewskiMottCrossSection.class.getResourceAsStream(name), "US-ASCII")) {
            try (final BufferedReader br = new BufferedReader(isr)) {
               ...
               final NumberFormat nf = NumberFormat.getInstance(Locale.US); // To
               // ensure
               // that
               // numbers are
               // parsed
   ...
   public double totalCrossSection(double energy) {
      ...
      if(ei >= 0)
         ...
         ei = -(ei + 1);
         return ToSI.sqrAngstrom(mValues[ei - 1][kTotalIndex] - ((energy - kEnergy[ei - 1]) / (kEnergy[ei] - kEnergy[ei - 1]))
