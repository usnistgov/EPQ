...
public class SpectrumFitter8 {
   ...
   private static final String[] SQRT_UNITS = new String[] {
      "eV",
      "eV/ch",
      "eV/ch<sup>1/2</sup>"
   };
   private static final String[] SQRT_NAMES = new String[] {
      "Zero",
      "Width",
      "Root"
   };
   private static final String[] POLYNOMIAL_UNITS = new String[] {
      "eV",
      "eV/ch",
      "eV/ch<sup>2</sup>",
      "eV/ch<sup>3</sup>",
   ...
   private static final String[] POLYNOMIAL_NAMES = new String[] {
      "Constant",
      "Linear",
      "Quadratic",
      "Cubic",
    ...
    * @version 1.0
    */
   static public class FanoNoiseWidth
      extends
      FunctionImpl {
      ...
      public FanoNoiseWidth duplicate(SpectrumFitter8 sf8)
      ...
      public UncertainValue2 computeU(double eV, Map<Parameter, UncertainValue2> param) {
         ...
         return UncertainValue2.multiply(ENERGY_PER_EH_PAIR, UncertainValue2.sqrt(UncertainValue2.add(UncertainValue2.sqr(noise), UncertainValue2.divide(UncertainValue2.multiply(eV, fano), ENERGY_PER_EH_PAIR))));
      }
   ...
   static public class EnergyScaleFunction
      extends
      FunctionImpl
      implements
      InvertableFunction {
      ...
      public EnergyScaleFunction(double[] coeff, boolean[] fitP) {
         ...
         for(int i = 0; i < mCoeff.length; ++i)
            mCoeff[i] = add(new Parameter("pow[ch," + Integer.toString(i) + "]", new Constraint.Bounded(coeff[i], 100.0
                  * Math.pow(10.0, -3.0 * i)), i < coeff.length ? coeff[i] : 0.0, i < fitP.length ? fitP[i] : false));
      }

      public EnergyScaleFunction duplicate(SpectrumFitter8 sf8)
            throws EPQException, UtilException {
         final Map<Parameter, UncertainValue2> param = sf8.getBestFitParameters();
      ...
      public UncertainValue2 inverse(double eV, Map<Parameter, UncertainValue2> param) {
         ...
         if(mCoeff.length == 2)
            ...
            if(res == null)
               ...
               return Math.abs(res[0].doubleValue() - linear.doubleValue()) < Math.abs(res[1].doubleValue()
                     - linear.doubleValue()) ? res[0] : res[1];
         } else {
            final double[] coeffs = new double[mCoeff.length];
            for(int i = 0; i < coeffs.length; ++i)
               coeffs[i] = mCoeff[i].getUncertainValue(param).doubleValue();
            ...
            try {
               ...
               final double bestEst = Math.abs(est[0] - linear.doubleValue()) < Math.abs(est[1] - linear.doubleValue()) ? est[0]
                     : est[1];
   ...
   static public class AltEnergyScaleFunction
      extends
      FunctionImpl
      implements
      InvertableFunction {
      ...
      public AltEnergyScaleFunction duplicate(SpectrumFitter8 sf8)
            throws EPQException, UtilException {
         final Map<Parameter, UncertainValue2> param = sf8.getBestFitParameters();
         final double[] coeffs = new double[mCoeff.length];
         for(int i = 0; i < coeffs.length; ++i)
      ...
      @Override
      public UncertainValue2 computeU(double ch, Map<Parameter, UncertainValue2> param) {
         return UncertainValue2.add(UncertainValue2.add(mCoeff[0].getUncertainValue(param), UncertainValue2.multiply(ch, mCoeff[1].getUncertainValue(param))), UncertainValue2.multiply(mCoeff[2].getUncertainValue(param), new UncertainValue2(Math.sqrt(ch))));
      }
      ...
      public UncertainValue2 inverse(double eV, Map<Parameter, UncertainValue2> param) {
         ...
         if(res == null)
            ...
            return Math.abs(computeU(res[0].doubleValue(), param).doubleValue()
                  - eV) < Math.abs(computeU(res[1].doubleValue(), param).doubleValue() - eV) ? res[0] : res[1];
         }
      }
   ...
   private class Lineset
      ...
      private final class LineData {
         ...
         LineData(XRayTransition xrt)
               throws EPQException {
            this(xrt, xrt.getWeight(XRayTransition.NormalizeFamily));
         }

         LineData(XRayTransition xrt, double weight)
               throws EPQException {
            mTransition = xrt;
         ...
         final double compute(double eV, Map<Parameter, Double> param) {
            ...
            assert (mIncomplete.getValue(param) >= 0)
                  && (mIncomplete.getValue(param) <= MAX_INCOMPLETE) : Double.toString(mIncomplete.getValue(param));
         ...
         final UncertainValue2 computeU(double eV, Map<Parameter, UncertainValue2> param) {
            ...
            final UncertainValue2 k = (x.doubleValue() < 0.0) && mFitIncompleteCharge
                  ...
                  : UncertainValue2.ONE;
            return UncertainValue2.multiply(k, UncertainValue2.multiply(mAmplitude.getUncertainValue(param), UncertainValue2.multiply(mWeight, UncertainValue2.exp(UncertainValue2.multiply(-0.5, UncertainValue2.sqr(x))))));
         }
         ...
         final UncertainValue2 integrate(Map<Parameter, UncertainValue2> param) {
            ...
            final int chMin = (int) Math.round(mScale.inverse(Math.max(0.0, mEnergy
                  - (K * w.doubleValue())), param).doubleValue());
      ...
      private Lineset(XRayTransitionSet xrts, double scale, Constraint con)
            throws EPQException {
         ...
         mIncomplete = add(new ParameterObject<XRayTransitionSet>("Incomplete[" + xrts.toString()
               + "]", new Constraint.Bounded(0.5 * MAX_INCOMPLETE, MAX_INCOMPLETE), 0.0
                     * MAX_INCOMPLETE, mFitIncompleteCharge, xrts));
      ...
      private Lineset(XRayTransitionSet xrts, double scale)
            throws EPQException {
         this(xrts, scale, new Constraint.Positive(scale));
      }

      private Lineset(XRayTransition xrt, double scale)
            throws EPQException {
         this(new XRayTransitionSet(xrt), scale);
      }

      private Lineset(XRayTransition xrt, double scale, Constraint con)
            throws EPQException {
         this(new XRayTransitionSet(xrt), scale, con);
      }

      public void addLine(XRayTransition xrt)
            throws EPQException {
      ...
      public UncertainValue2 computeU(double ch, Map<Parameter, UncertainValue2> param) {
         ...
         for(final LineData ld : mLines) {
            final UncertainValue2 xx = UncertainValue2.divide(UncertainValue2.subtract(eV, new UncertainValue2(ld.mEnergy)), mResolution.computeU(eV.doubleValue(), param));
            res = UncertainValue2.add(res, UncertainValue2.multiply(ld.mWeight, UncertainValue2.exp(UncertainValue2.multiply(-0.5, UncertainValue2.multiply(xx, xx)))));
         }
   ...
   public MultiLineset buildWeighted(RegionOfInterestSet rois)
         throws EPQException {
    ...
    * @throws EPQException
    */
   public MultiLineset buildIndependent(RegionOfInterestSet rois, double de, double fraction)
         throws EPQException {
      final MultiLineset mls = new MultiLineset();
      for(final RegionOfInterest roi : rois)
         for(final Element elm : roi.getElementSet()) {
    ...
    * @throws EPQException
    */
   public MultiLineset makeIndependent(MultiLineset mls, Map<Parameter, UncertainValue2> param, double de, double fraction)
         throws EPQException {
   ...
   public class MultiLineset
      ...
      public String dump(double[] chs, Set<Parameter> param)
            throws EPQException {
   ...
   private static ISpectrumData extractCharacteristicSpectrum(ISpectrumData spec, EDSDetector det, Composition comp, RegionOfInterestSet rois, BremsstrahlungAnalytic bs)
         throws EPQException {
   ...
   public ISpectrumData getCharacteristicSpectrum()
         throws EPQException {
      if(mBackground == null) {
   ...
   private double[] getPeakData(double[] chs)
         throws EPQException {
   ...
   private double[] getPeakSigma(double[] chs)
         throws EPQException {
      final double[] res = new double[chs.length];
      if(true)
         for(int i = 0; i < res.length; ++i) {
   ...
   private class CrudeLinearFit
      extends
      LinearLeastSquares {

      private final Map<Parameter, Double> mParam;

      private CrudeLinearFit(double[] chs, Map<Parameter, Double> param)
            throws EPQException {
      ...
      protected ISpectrumData getFit(double[] chD)
            throws EPQException {
    ...
    * @version 1.0
    */
   public class HighQualityLinearFit
      extends
      LinearLeastSquares {
      ...
      public Map<Parameter, UncertainValue2> getBestFitParameters()
            throws EPQException {
      ...
      public SpectrumFitResult getResult()
            throws EPQException {
         ...
         if(mResolution instanceof FanoNoiseWidth)
            res.setResolution(((FanoNoiseWidth) mResolution).getFano().getUncertainValue(pm), ((FanoNoiseWidth) mResolution).getNoise().getUncertainValue(pm));
    ...
    * @throws EPQException
    */
   private void roughFit(double[] chD)
         throws EPQException {
   ...
   public SpectrumFitResult compute()
         throws EPQException, UtilException {
    ...
    * @throws EPQException
    */
   public SpectrumFitResult recompute(double de, double fraction)
         throws EPQException {
      assert mMultiLines != null;
   ...
   public UncertainValue2 getChannel(double ev)
         throws EPQException, UtilException {
   ...
   public ISpectrumData getElementSpectrum(Element elm) {
      ...
      fit.getProperties().clear(new PropertyId[] {
    ...
    * @throws UtilException
    */
   public static TreeMap<String, UncertainValue2> performQC(EDSDetector det, Composition std, ISpectrumData spec, QCNormalizeMode mode, ActionListener al)
         throws EPQException, UtilException {
      ...
      for(final Element elm : std.getElementSet())
         for(final RegionOfInterestSet.RegionOfInterest roi : sf8.getRegionOfInterests(elm)) {
            ...
            res.put(name.substring(0, name.indexOf("[")
                  - 1), UncertainValue2.divide(sfr.getIntegratedIntensity(roi.getXRayTransitionSet(elm)), norm));
         }
      return res;
   }
   ...
   public RegionOfInterestSet getRegionOfInterests(Element elm) {
      ...
      final DetectorLineshapeModel dlm = mDetector.getDetectorLineshapeModel();
      final RegionOfInterestSet rois = new RegionOfInterestSet(dlm, MIN_I, ToSI.eV(0.5 * dlm.getFWHMatMnKa()), ToSI.eV(0.25
            * dlm.getFWHMatMnKa()));
