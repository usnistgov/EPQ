...
public class NormalCylindricalShape extends CylindricalShape implements NormalShape {
   ...
   /**
    * Constructs a NormalCylindricalShape with the specified radius. One end of
    * the cylinder is at end0 (a 3-element array giving the center coordinates).
    * The other end is at end1.
    *
    * @param end0
    *           - 3 coordinates of one end of the cylinder
    * @param end1
    *           - 3 coordinates of the other end
    * @param radius
   ...
   public boolean contains(double[] pos0, double[] pos1) {
      final double[] p0c = {pos0[0] - end0[0], pos0[1] - end0[1], pos0[2] - end0[2]}; // pos0
                                                                                      // -
                                                                                      // end0
      final double p0cdotn = (p0c[0] * normalizedaxis[0]) + (p0c[1] * normalizedaxis[1]) + (p0c[2] * normalizedaxis[2]);
      ...
      if (on_end0) {
         ...
         final double deltadotnormalizedaxis = (delta[0] * normalizedaxis[0]) + (delta[1] * normalizedaxis[1]) + (delta[2] * normalizedaxis[2]);
         ...
         final double[] delta = {pos1[0] - pos0[0], pos1[1] - pos0[1], pos1[2] - pos0[2]}; // Vector
                                                                                           // pointing
                                                                                           // in
                                                                                           // direction
                                                                                           // of
         // trajectory
         final double[] nv = {p0c[0] - (p0cdotn * normalizedaxis[0]), p0c[1] - (p0cdotn * normalizedaxis[1]), p0c[2] - (p0cdotn * normalizedaxis[2])}; // An
                                                                                                                                                       // outward
         // pointing vector
         // at pos0
   ...
   public double[] getFirstNormal(double[] pos0, double[] pos1) {
      ...
      final double[] p0c = {pos0[0] - end0[0], pos0[1] - end0[1], pos0[2] - end0[2]}; // pos0
                                                                                      ...
                                                                                      // end0
      final double p0cSquared = (p0c[0] * p0c[0]) + (p0c[1] * p0c[1]) + (p0c[2] * p0c[2]);
      final double[] delta = {pos1[0] - pos0[0], pos1[1] - pos0[1], pos1[2] - pos0[2]}; // pos1
                                                                                        // -
                                                                                        // pos0
      final double p0cdotn = (p0c[0] * normalizedaxis[0]) + (p0c[1] * normalizedaxis[1]) + (p0c[2] * normalizedaxis[2]);
      final double deltadotn = (delta[0] * normalizedaxis[0]) + (delta[1] * normalizedaxis[1]) + (delta[2] * normalizedaxis[2]);
      ...
      if (deltadotn != 0.) {
         ...
         if ((u2 > 0.) && (u2 <= 1.) && (u2 < nv[3]) && (((p0cSquared + (2 * u2 * p0cdotdelta) + (u2 * u2 * deltaSquared) + mLen2)
      ...
      if (!(((u1 < 0) && (u2 < 0)) || ((u1 > 1) && (u2 > 1)))) {
         ...
         // pos0
         final double r1Squared = (r0Squared + (2. * (p0cdotdelta - (p0cdotn * deltadotn))) + deltaSquared) - deltadotnSquared; // radius
         // of
         // pos1
      ...
      switch (intersectionnumber) {
         ...
         case 4 :
            ...
            final double normalvmag = Math.sqrt((normalv[0] * normalv[0]) + (normalv[1] * normalv[1]) + (normalv[2] * normalv[2]));
