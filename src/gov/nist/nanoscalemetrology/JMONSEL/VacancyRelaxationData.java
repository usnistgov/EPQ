...
/**
 * <p>
 * Simple auxiliary class intended as a "container" for all necessary information 
 * regarding Auger transitions for a vacancy in a given subshell 
 * (K,L1,L2,L3,M1,M2,...).
 * </p>
 * <p>
 ...
 * vacancy-relaxation "cascade" is needed (e.g. at higher primary 
 * energies, where deeper inner-shell ionizations are accessible).
 * </p>
 * 
 ...
 * branching ratios). The provided binding energies should be those of
 * Williams, in order for the Auger simulation (as currently implemented)
 * to be consistent. At a later stage it would be better to have the user
 * simply enter what shells are ionizable and read the binding energies from
 * a unique and coherent database. This was not done so in the interest of 
 * keeping modifications to a minimum for testing purposes. 
 * </p>
 * <p>
...
public class VacancyRelaxationData {
   ...
   /* Radiative-transition probability for each subshell (fluorescence instead 
    * of Auger yield). */
   ...
   /* Upon instantiation of this class, read the Auger transition probabilities
    * and energies from the database (for the required atomic number). */
   ...
   public void readIonizationEnergiesFromDatabase(int z0) {
      ...
      /* I maintain the subshell labeling of the database (starting at 1):
       * K (1), L1 (2), L2 (3), L3 (4), M1 (5), ...
       * 
       * In order to prevent indexing mistakes (java indexes from 0, the 
       * database from 1) and to have a consistent indexing convention with the
       * database, I add a 0-th element that will never be accessed. */
      bindingEnergies.add(0.0);
      
      /* Get ionization energies from atomconf.tab. */
      try{         
         InputStream is = this.getClass().getResourceAsStream("atomconf.tab");
   ...
   /**
    * <p>
    * Not all inner-shell ionizations decay via the emission of Augers. This 
    * method calculates the probability that the vacancy decays via a 
    * radiative process (emission of fluorescence photons). We currently do 
    * not follow fluorescence processes in this class, only their frequency 
    * of occurrence so as not to mistakenly assume that all inner-shell ionizations
    * decay via the emission of an Auger electron 
    * (this would yield an excess of Augers).
    * </p>
    * <p>
    ...
    * @param z0 Atomic number.
    */
   public void determineRadiativeProbability(int z0) {
      String line;
      radiativeTransitionProbability = new ArrayList<Double>();
      
      /* In order to maintain the subshell indexing of the Penelope files,
       * K (1), L1 (2), L2 (3), L3 (4), M1 (5), ..., I add a fake 0-th 
       * element which is never accessed.
       */
      ...
      for (int i=1; i<=maxSubshell; i++) {
         ...
         try {
            ...
            InputStream is = this.getClass().getResourceAsStream("pdrelax.p11");
            BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(is));
         ...
         System.out.println("Subshell, radprob: "+i+" "+radprob);
         if (radprob>1.0) {System.out.println("RADIATIVE PROB>1."); System.exit(1);}
         if (radprob<0.0) {System.out.println("RADIATIVE PROB<0."); System.exit(1);}
   ...
   public void readAugerProbabilities(int z0) {
      ...
      /* Determine probabilities and energies for the various Auger-emission 
       * processes given a vacancy in subshell i.
       */
      for (int i=1; i<=maxSubshell; i++) {
         ...
         for (int j=0; j<augersFromSubshell.probability.size(); j++) {
            ...
            if (j>0) augersFromSubshell.probability.set(j,
                  augersFromSubshell.probability.get(j-1)
                  +augersFromSubshell.probability.get(j));
         }
   ...
   /** 
    * Given an energy loss and a binding energy, this method determines which shell the 
    * binding energy corresponds to (e.g. L) and which of the subshells are ionizable
    * given the energy loss  (e.g. L2 and L3). Then, it decides whether the vacancy 
    * decayed via the emission of a fluorescence photon (returning a negative value) or 
    * an Auger electron (returning the energy of the Auger electron).
    */
   public double augerEnergy(double eBind, double eLoss) {
      double augerEne;
      ArrayList<Integer> possibleSubshellVacancies = new ArrayList<Integer>();           
      
      //System.out.println("\n\nEbind, Eloss: "+FromSI.eV(eBind)+" "+FromSI.eV(eLoss));
      ...
      if      (i==1)           {ilow=1;  ihigh=1;} 
      else if (i>=2  && i<=4)  {ilow=2;  ihigh=4;}
      else if (i>=5  && i<=9)  {ilow=5;  ihigh=9;}
      else if (i>=10 && i<=16) {ilow=10; ihigh=16;}
      else if (i>=17 && i<=23) {ilow=17; ihigh=23;}
      else if (i>=24 && i<=27) {ilow=24; ihigh=27;}
      else if (i==29)          {ilow=29; ihigh=29;}
      else {
         System.out.println("AUGER LOGIC ERROR. STOPPING."); System.exit(1);
      }
       ...
       * relaxation data in pderelax.p11. I therefore chose to loop until valid data are found.
       */
      int nsteps=0;
      int ntrans0=0;
      int vacancySubshell;
      ...
      //System.out.println("Sampled subshell, radprob: "+vacancySubshell+" "+radiativeTransitionProbability.get(vacancySubshell));
      
      // Sample radiative vs non-radiative probability. If radiative, return a negative Auger energy.
      // This won't be pursued further in the calling method under TabulatedInelasticSM.
      ...
      // Sample Auger energy.
      //System.out.println("Accumulated probs: "+augerProbability.get(vacancySubshell).probability.toString());
      ...
      for (j=0; j<ntrans; j++) {
         if (zeta<augerProbability.get(vacancySubshell).probability.get(j)) break;
      }
      if (j>=ntrans) j=ntrans-1;
      ...
      if (augerEne<0.0)   {System.out.println("Auger ene<0."); System.out.println(""+1/0);System.exit(1);}
      if (augerEne>eLoss) {System.out.println("Auger ene > eLoss.");System.out.println(""+1/0); System.exit(1);}
      //if (augerEne>eBind) {System.out.println("Auger ene > eBind.");System.out.println(""+1/0); System.exit(1);}
      
      return augerEne;
   }   
   
   /**
    * Given a label string ("K","L1","L2", etc.), this returns the associated index.
    * 
