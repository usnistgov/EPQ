...
public class ThresholdConductionModel implements IConductionModel {
	...
	private double faceCurrent(MeshedRegion thisMeshedRegion, int tet1ID, int faceIndex) {
		...
		/*
		 * These tets are useful for geometrical information, but should not be used for
		 * electrical info (e.g., potentials) until I call updatePotentials. In the
		 * meantime, it is more efficient to directly compute the potential at the
		 * center by averaging that at the nodes.
		 */
		 ...
		 * currentFromField() field^p where p is the appropriate power. The result is
		 * proportional to current density (current per unit area). This is multiplied
		 * by the area of the face through which the current is flowing and corrected by
		 * a cosine of the angle between the face and direction of current flow.
		 */
	...
	/**
	 * During charge flow iterations a new finite element analysis is performed to
	 * update fields at intervals of 1 or more iterations, as set by this method's
	 * argument. When FEA is not performed a faster but more approximate method is
	 * used.
	 *
	 * @param feaInterval
	 */
