...
public class PHAMLMeshFEARunner implements IAdaptiveMesh, IFEArunner {
	...
	interface PHAML extends com.sun.jna.Library {
		...
		void extendedAdjacentVolumeArray(int handle, int tetIndex, int faceIndex, int[] adjacentVolumeArray,
		...
		void meshConstructor(IntByReference handle, String meshFileName, int len, String outputPath, int pathlen,
				boolean doGraphics, IntByReference errcode);

		void meshDestructor(int handle, IntByReference errcode);

		void meshDestructorAll(IntByReference errcode);

		void runFEA(int handle, boolean verbose, double tolerance, int max_tet, double min_tet_size, double refinetol,
				int solver, int errind, IntByReference errcode);
		...
		void setConstraints(int handle, int numConstraint, int[] tags, int[] types, double[] constants,
				int[] int_constants, IntByReference errcode);
		...
		void setPointCharges(int handle, int numCharge, double[] xCharge, double[] yCharge, double[] zCharge,
				IntByReference errcode);
	...
	/*
	 * The following constants determine the solver to use, error estimator used for
	 * refinement, and stopping condition. Refinement stops when the estimated L2
	 * error (root mean square error in potential) divided by the potential is less
	 * than TOLERANCE, or when the number of tetrahedra exceeds MAXTET, whichever
	 * comes first. Tets smaller than min_tet_size are not refined unless necessary
	 * for compatibility.
	 */
	...
	/**
	 * Constructs a PHAMLMeshFEARunner. The supplied meshFileName should conform to
	 * the GMSH format. It may be either a GMSH .geo or GMSH .msh file. The former
	 * is preferred because it contains boundary information that can be used if
	 * mesh elements are refined along a curved boundary.
	 *
		...
		if (err < 0) {
			...
			case 3:
				...
				throw new EPQFatalException(
						"tetIndex is not a tetrahedron element in extendedNumberAdjacentVolumeArray.");
			case 4:
				PHAML.lib.meshDestructorAll(errcode);
	...
	public int getAdjacentVolumeIndexPHAMLdirect(int elementIndex, int faceIndex) {
		...
		if (err < 0) {
			...
			case 1:
				PHAML.lib.meshDestructorAll(errcode);
				throw new EPQFatalException(
						"Element index (" + elementIndex + ") out of range in getAdjacentVolumeIndex.");
			case 2:
				PHAML.lib.meshDestructorAll(errcode);
				throw new EPQFatalException(
						"Element " + elementIndex + " is not a volume element in getAdjacentVolumeIndex.");
			case 3:
	...
	/**
	 * Returns the number of charges in the indexed mesh element from PHAML's table.
	 *
	 * @param index
	 * @return
	 */
	 ...
	 * implementations that alter the mesh (e.g., classes that use adaptive meshes)
	 * increment the revision number each time the mesh is changed. This
	...
	public int[] getNodeAdjacentVolumes(int nodeIndex) {
		...
		if (err < 0) {
			PHAML.lib.meshDestructorAll(errcode);
			throw new EPQFatalException(
					"PHAML error code = " + err + " in getNumberNodeAdjacentVolumes in getNodeAdjacentVolumes.");
		} else
			switch (err) {
			case 0:
				break;
			case 1:
				PHAML.lib.meshDestructorAll(errcode);
				throw new EPQFatalException(
						"Node index out of range in getNumberNodeAdjacentVolumes in getNodeAdjacentVolumes.");
			case 2:
				throw new EPQFatalException(
						"Invalid handle in getNumberNodeAdjacentVolumes in getNodeAdjacentVolumes.");
			default:
				PHAML.lib.meshDestructorAll(errcode);
				throw new EPQFatalException(
						"Unknown nonzero error code in getNumberNodeAdjacentVolumes in getNodeAdjacentVolumes.");
			}
		...
		case 2:
			PHAML.lib.meshDestructorAll(errcode);
			throw new EPQFatalException(
					"Node index does not agree with last call to getNumberNodeAdjacentVolumes in getNodeAdjacentVolumes");
		case 3:
			PHAML.lib.meshDestructorAll(errcode);
			throw new EPQFatalException(
					"Allocation failed: adjacencies are returned but not sorted in getNodeAdjacentVolumes");
		case 4:
	...
	/**
	 * Gets the number of nodes in the mesh directly from PHAML, as opposed to using
	 * the locally cached copy, as does getNumberOfNodes().
	 *
	 * @return
	 */
	...
	/**
	 * Returns the value of saveMeshType. A value of 1 means mesh files are stored
	 * in GMSH format. A value of 2 means they are stored in PHAML format.
	 * 
	 * @return
	 */
	 ...
	 * construct a cache of which elements are adjacent to which through each face.
	 * Caching on the Java side of interface speeds execution considerably. This
	 * routine needs to be called in the constructor to initialize the cache and
	 * again to renew it each time the mesh is refined.
	 */
	private void initializeAdjacentVolumesTable() {
		final IntByReference errcode = new IntByReference(0);
		final int nElem = getNumberOfElementsPHAMLdirect();
		/*
		 * Tetrahedra have 4 faces, and we presently have no elements with more than
		 * this, so PHAML returns a N x 4 array.
		 */
	...
	private void initializeAllCache() {
		/*
		 * TODO: In cases where PHAML generates its own mesh from a .geo file the mesh
		 * elements (and maybe other things) are not getting properly copied to the
		 * JMONSEL side of the interface.
		 */
	...
	/**
	 * initializeChargeNumberTable calls PHAML's getChargeNumberTable to construct a
	 * cache of charges associated with the elements. Caching on the Java side of
	 * interface is meant to speed execution. This routine needs to be called in the
	 * constructor to initialize the cache and again to renew it each time the mesh
	 * is refined.
	 */
	...
	/**
	 * initializeElementTypeTable calls PHAML's getElementTypeTable to construct a
	 * cache of types associated with the elements. Caching on the Java side of
	 * interface is meant to speed execution. This routine needs to be called in the
	 * constructor to initialize the cache and again to renew it each time the mesh
	 * is refined.
	 */
	 ...
	 * Java side of interface is meant to speed execution. This routine needs to be
	 * called in the constructor to initialize the cache and again to renew it each
	...
	/**
	 * initializeNodeIndicesTable calls PHAML's getNodeIndicesTable to construct a
	 * cache of node indices associated with the elements. It also caches the number
	 * of nodes associated with each element. Caching on the Java side of interface
	...
	private void initializeNodeIndicesTable() {
		...
		nElem = getNumberOfElementsPHAMLdirect();
		/*
		 * Tetrahedra have 4 nodes, and we presently have no elements with more than
		 * this, so PHAML returns indices in an N x 4 array.
		 */
	...
	/**
	 * initializeNodePotentialTable calls PHAML's getNodePotentialTable to construct
	 * a cache of potentials associated with the nodes. Caching on the Java side of
	 * interface is meant to speed execution. This routine needs to be called in the
	 * constructor to initialize the cache and again to renew it each time the mesh
	 * is refined.
	 */
	...
	/**
	 * initializeTagsTable calls PHAML's getTagsTable to construct a cache of node
	 * indices associated with the elements. It also caches the number of nodes
	 * associated with each element. Caching on the Java side of interface is meant
	 * to speed execution. This routine needs to be called in the constructor to
	 * initialize the cache and again to renew it each time the mesh is refined.
	 */
	...
	/**
	 * Returns true if the element is a volume type (e.g., a tetrahedron) and false
	 * if not (lines, triangles, etc.) The determination is made by relying on
	 * information in JMONSEL's cache of volume types.
	 *
	 * @param elementIndex
	 * @return
	...
	/**
	 * Asks PHAML whether the indexed element is a volume type (e.g., a tetrahedron)
	 * or a nonvolume type (lines, triangles, etc.)
	 *
	 * @param elementIndex
	 * @return
	 */
	...
	public void runFEA(MeshedRegion meshReg) {
		...
		case 8:
			/*
			 * These are informational return codes, indicating that a tolerance was not
			 * met. Code 7 means termination was due to reaching max_tet volume elements.
			 * Code 8 means termination was due to an apparent stalled refinement. I have
			 * nothing good to do with this message, so I currently just ignore it.
			 */
			break;
		case 9:
			/*
			 * TODO: This code means "could not refresh output file". I'm not sure what this
			 * means so I don't know whether to ignore it or throw an exception.
			 */
			break;
		default:
			PHAML.lib.meshDestructorAll(errcode);
	...
	/**
	 * If doAdapt is true, then subsequent calls to runFEA adapt the mesh and solve
	 * for the potential. If doAdapt is false, then subsequent calls to runFEA only
	 * solve for the potential. The default is true.
	 *
	 * @param doAdapt
	 */
	@Override
	...
	/**
	 * Sets the charge in element[index] = n directly in PHAML and updates the local
	 * cache too.
	 *
	 * @param index
	 * @param n
	...
	/**
	 * The maximum number of tetrahedra to allow in the mesh. During refinement, the
	 * number of tetrahedra in the mesh may increase up to approximately this limit
	 * if needed to meet the error tolerance. Once maxtet tetrahedra are in the
	 * mesh, then low error tetrahedra are de-refined and an approximately equal
	 * number of high error tets are refined. Refinement and de-refinement occur
	 * with groups of tetrahedra, so this limit is approximate. Default is 1
	 * million.
	 *
	 * @param maxtet
	 */
	 ...
	 * unless it is necessary for compatibility (e.g., with an adjacent element that
	 * is refined).
	 *
	 ...
	 * saveMeshType = 2 it saves in PHAML format. The PHAML mesh file is larger, but
	 * it retains information about refinement history that allows PHAML to unrefine
	 * previously refined tets. This may be important if an adaptive calculation is
	 * saved and later resumed
	 * 
	...
	/**
	 * The integer argument determines the solver/preconditioner combination used by
	 * PHAML, as follows:
	 * </p>
	 * <p>
	 * 1 GMRES/ILU (default)
	 ...
	 * method, ILU = Incomplete LU factorization, SOR = Successive over-relaxation,
	 ...
	 * runFEA is invoked. Verbose or not verbose (the default) output to out.txt is
	 * set by calling setVerbose with the argument respectively true or false.
	 *
	 * @param verbose
	 */
