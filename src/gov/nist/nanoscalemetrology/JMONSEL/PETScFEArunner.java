...
public class PETScFEArunner implements IFEArunner {
	...
	/*
	 * Following are the PETSc defaults (not necessarily the same as OUR defaults).
	 * If our desired value differs from PETSc default, we must specify it on the
	 * command line.
	 */
	...
	public void runFEA(MeshedRegion meshReg) {
		...
		/*
		 * For options with a PETSc default, if the assigned value = default we can skip
	...
	/**
	 * Sets the value assigned to ksp_monitor. If true (the default) residual norms
	 * after each iteration are printed to the out.txt file in the temporary
	 * feaFolder. If false, they are not printed.
	 *
	...
	/**
	 * Sets the value assigned to log_summary. When true, a performance summary is
	 * included in the out.txt file in feaFolder. Default is false.
	 *
	 ...
	 * iterations at which GMRES, FGMRES and LGMRES restart. Default is 30. Ignored
	 * with non GMRES solvers.
	 *
	 ...
	 *
	 * @param ksp_richardson_scale The value to which to set ksp_richardson_scale.
	 */
	...
	/**
	 * Sets the value assigned to ksp_right_pc. If true, a right preconditioner is
	 * used. If false (default), preconditioning is from the left.
	 *
