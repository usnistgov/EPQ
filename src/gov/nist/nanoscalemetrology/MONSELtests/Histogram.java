...
public class Histogram {
	...
	public class BinName implements Comparable<BinName> {
		...
		public String toString() {
			return "["
					+ (mBin < 0 ? "-inf"
							: MessageFormat.format(mFormat, new Object[] { Double.valueOf(minValue(mBin)) }))
					+ "-" + (mBin >= binCount() ? "inf"
							: MessageFormat.format(mFormat, new Object[] { Double.valueOf(maxValue(mBin)) }))
					+ ")";
		}
	 ...
	 * </p>
	 * <p>
	 * Bins are numbered such that 0..(nBins-1) correspond to the full range. Bin[0]
	 * -&gt; [min,min+delta) where delta = (max-min)/nBins, Bin[nBins-1] -&gt;
	...
	/**
	 * Constructs a Histogram object to represent bins between min (inclusive) and
	 * max (excluded)
	 *
	 * @param min
	 * @param max
	...
	/**
	 * Constructs a Histogram object to bin into the arbitrary bins defined by lower
	 * ends defind by binMins. The max end of the top bin is defined by max.
	 *
	 * @param binMins
	 * @param max
	 */
	 ...
	 * format is used to format numbers using MessageFormat.format(...). An common
	 * usage might be <code>binName(i,"{0,number,#.##}")</code>.
	 *
	 * @param bin    int
	 * @param format String
	 * @return String
