...
public class CompositionFromKRatios extends AlgorithmClass {
	...
	abstract static public class UnmeasuredElementRule {
		...
		/**
		 * Takes a Composition and adds the correct proportion of the unmeasured element
		 * based on the rule implemented.
		 *
		 * @param comp
		 * @return The original composition object adjusted for the unmeasured element
		 */
	 ...
	 * implementation does not do any sanity checking. Use with extreme caution. If
	 * the element interferes with another there will be problems if the element
	 * isn't fit.
	 * </p>
	 * <p>
	...
	static public class ElementByFiat extends UnmeasuredElementRule {
		...
		@Override
		public String toHTML() {
			return "<b>Element-by-fiat:</b> " + getElement().toAbbrev() + " at "
					+ mQuantity.format(new DecimalFormat("0.000")) + " mass fraction.";
		}
	}

	/**
	 * Implements the algorithm for computing an element by stoichiometry. The user
	 * creates an instance of ElementByStoichiometry for a specific element with a
	 * specified valence. Then the user specifies a series of other elements which
	 * are presumably in the material and a valence (of opposite sign than the
	 * constructor element) for each of these elements.
	 *
	 * @author nicholas
	 */
	...
	/**
	 * Compute O by stoichiometry. Compare this to measured O. If there is excess O,
	...
	public static class WatersOfCrystallization extends OxygenByStoichiometry {
		...
		public Composition compute(final Composition comp) {
			...
			final UncertainValue2 Oexcess = UncertainValue2.subtract(
					comp.weightFractionU(Element.O, false).reduced("M[O]"),
					oByStoic.weightFractionU(Element.O, false).reduced("S[O]"));
			if (Oexcess.doubleValue() > 0) {
				final UncertainValue2 massFracH = UncertainValue2
						.multiply((2.0 * Element.H.getAtomicWeight()) / Element.O.getAtomicWeight(), Oexcess);
		...
		private double getKstd(final XRayTransition xrt) {
			if (!mZAFFactors.containsKey(xrt))
				try {
					...
					final ConductiveCoating cc = (ConductiveCoating) mProperties
							.getObjectWithDefault(SpectrumProperties.ConductiveCoating, null);
					final double toa = SpectrumUtils.getTakeOffAngle(mProperties);
					final double trStd = cc != null ? cc.computeTransmission(toa, xrt) : 1.0;
					mZAFFactors.put(xrt, //
							ca.computeZAFCorrection(xrt) * mComposition.weightFraction(xrt.getElement(), false)
									* trStd);
	 ...
	 * element based on the counting statistics specified in the KRatioSet argument
	...
	/**
	 * Constructs a ComputeZAFCorrection for the specified beam energy and take-off
	 * angle.
	 */
	...
	/**
	 * isReady - Is this object ready to compute the Composition from the specified
	 * KRatioSet?
	 *
	 * @param krs
	 ...
	 *
	 * @param xrts     - The XRayTransitionSet for which to define a reference
	 * @param ref      - The Composition of the reference
	 * @param refProps SpectrumProperties for the reference containing at least
	...
	public void addExtraStandard(final XRayTransitionSet xrts, final Composition ref,
			final SpectrumProperties refProps) {
		if (mStandardData.containsKey(xrts))
			mStandardData.remove(xrts);
	...
	/**
	 * <p>
	 * Returns a list of the XRayTransitionSet objects which were actually used to
	 * compute the Composition in the previous call to <code>compute(...)</code>.
	 * This is a subset of the transitions in the KRatioSet argument to
	 * <code>compute(...)</code> with one and only one XRayTransitionSet per
	 * element.
	 * </p>
	 *
	...
	private double score(final KRatioSet krs, final XRayTransitionSet xrts) {
		try {
			/*
			 * Consider Fe Ka vs Fe Kb. I(Ka)~10 I(Kb), E(Ka)=6.4, E(Kb)=7.1, S(Ka)/S(Kb) =
			 * (10*6.4^2)/(7.1^2) ~ 8
			 */
	...
	public Composition iterate(final KRatioSet krs, final SpectrumProperties unkProps, final Composition initialGuess)
			throws EPQException {
		...
		try {
			...
			for (int iteration = 0; iteration < maxIter; ++iteration) {
				...
				for (final XRayTransitionSet xrts : krs.getTransitions()) {
					...
					final double zafCorr = prev.weightFraction(xrts.getElement(), false) > 0.0
							? compute(xrts, prev, unkProps)
							: 1.0;
				...
				if (mIterationCount >= maxIter) {
					...
					msg.append("Best delta = " + Double.toString(bestDelta.doubleValue()) + " = "
							+ Double.toString(bestDelta.doubleValue() / mEpsilon) + " × tolerance\n");
...
/*			try {
				XRayTransition xrt = xrts.getWeighiestTransition();
				final CorrectionAlgorithm ca = getCorrectionAlgorithm();
				ca.initialize(new Material(xrts.getElement(), 1.0), xrt.getDestination(), sd.mProperties);
				final double zafp = ca.computeZAFCorrection(xrt);
				ca.initialize(sd.mComposition, xrt.getDestination(), sd.mProperties);
				final double zafs = ca.computeZAFCorrection(xrt);
				wf *= zafs / zafp;
	...
	private KRatioSet pickOptimized(final Composition firstC, final KRatioSet measured,
			final SpectrumProperties unkProps) {
				...
				if (sxrts.size() > 1) {
					...
					for (final XRayTransitionSet xrts : sorted) {
						...
						if (k.doubleValue() > 0.0) {
							final UncertainValue2 c = UncertainValue2.multiply(massFractionU(xrts, firstC, unkProps),
									k.fractionalUncertaintyU());
	 ...
	 * compute is called it is necessary to specify a reference for every Element in
	 * the KRatioSet using addReference(...). If a reference is missing this
	 ...
	 * <code>KRatioSet.add(XRayTransitionSet, value, error)</code>). The method will
	 * throw an exception if the error estimate is missing. If no redundant k-ratios
	 * are provided then error estimates are not required.
	 * </p>
	 *
	 * @param measured
	 * @param unkProps SpectrumProperties for the unknown containing at least
	 *                 SpectrumProperties.BeamEnergy and
	 *                 SpectrumProperties.TakeOffAngle
	...
	public Composition compute(final KRatioSet measured, final SpectrumProperties unkProps) throws EPQException {
		...
		for (final XRayTransitionSet xrts : mBestKRS.getTransitions())
			result.addElement(xrts.getElement(), UncertainValue2.multiply(massFractionU(xrts, best, unkProps),
					mBestKRS.getKRatioU(xrts).fractionalUncertaintyU()));
	...
	/**
	 * Returns the maximum number of iterations to refine the compositional estimate
	 *
	...
	/**
	 * Sets the maximum number of iterations to refine the compositional estimate.
	 *
	 * @param maxIterations The value to which to set maxIterations.
	 */
	...
	/**
	 * Returns the best estimate of the composition as computed in the last call to
	 * compute. (null if compute hasn't been called or failed before getting to the
	 * iteration loop.) This method returns the best estimate Composition regardless
	 * of whether compute converged.
	 *
	...
	/**
	 * Add an UnmeasuredElementRule to the list. This method ensures that there is
	 * only one UnmeasuredElementRule per element. The rules evaluate in the order
	 * in which they are added so it is possible to add one element by stoichiometry
	 * and another by difference from 100.0%. UnmeasuredElementRules with
	 * getElement().equals(Element.None) are special it that more than one can exist
	 * at a time.
	 *
	 * @param uer
	 */
	 ...
	 * @param unkProps The SpectrumProperties for the material containing at least
	 *                 SpectrumProperties.BeamEnergy and
		...
		if (stdData.mValidate) {
			...
			if (Math.abs(SpectrumUtils.getTakeOffAngle(refProps) - toa) > Math.toRadians(1.0))
				throw new EPQFatalException(
						"The take-off angle for the standard and unknown must match to within a degree.");
		}
	 ...
	 *         uncertainties
	 */
	public UncertainValue2 massFractionU(final XRayTransitionSet xrts, final Composition unkComp,
			final SpectrumProperties unkProps) {
		assert (unkProps.isDefined(SpectrumProperties.BeamEnergy));
		final SampleShape ss = unkProps.getSampleShapeWithDefault(SpectrumProperties.SampleShape, null);
		assert (ss == null) || getCorrectionAlgorithm().supports(ss.getClass());
		...
		if (stdData.mValidate) {
			...
			if (Math.abs(stdProps.getNumericWithDefault(SpectrumProperties.BeamEnergy, -1.0)
					- unkProps.getNumericWithDefault(SpectrumProperties.BeamEnergy, -2.0)) > 0.01)
				throw new EPQFatalException("The beam energy for the standard and unknown must match.");
			if (Math.abs(SpectrumUtils.getTakeOffAngle(stdProps) - SpectrumUtils.getTakeOffAngle(unkProps)) > Math
					.toRadians(1.0))
				throw new EPQFatalException(
						"The take-off angle for the standard and unknown must match to within a degree.");
		}
		...
		if (cUnk.doubleValue() > 0.0) {
			...
			for (final XRayTransition xrt : xrts) {
				...
				if ((w / norm) >= mMinWeight)
					try {
						sum = UncertainValue2.add(sum, UncertainValue2.multiply(w,
								CorrectionAlgorithm.XPP.kratio(stdData.mComposition, unkComp, xrt, unkProps)));
						sumW += w;
					} catch (final EPQException e) {
						e.printStackTrace();
					}
