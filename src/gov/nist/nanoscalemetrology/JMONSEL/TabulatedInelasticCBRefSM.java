...
public class TabulatedInelasticCBRefSM extends ScatterMechanism {
	 ...
	 * occupied state relative to the bottom of the conduction band. This makes it
	 * equal to what we usually think of as the Fermi energy for a metal and equal
	 * to -bandgap for an insulator or semiconductor. In my scattering table notes,
	 * on the other hand, Fermi energy was the distance from bottom to highest
	 * occupied state in whatever band was responsible for scattering. This makes no
	 * change for metals, but it changes the value when bandgap != 0.
	 * offsetFermiEnergy (next line) corresponds to my Mathematica notebook
	 * definition.
	 */
	...
	/*
	 * E0 method. E0 is the energy available to ionize an inner shell. My original
	 * method based on the description in Ding & Shimizu's SCANNING article was to
	 * assume that deltaE, i.e., all energy transferred in an inelastic event, is
	 * available for ionization. Ding et al later modified this procedure. By the
	 * plasmon dispersion (I use Penn's form) deltaE can be broken down into a q=0
	 * part and a q != 0 part. The q = 0 part is E0. When E0fromDispersion = false I
	 * used deltaE. When it is true I use Ding's later method.
	 */
	...
	/**
	 * Constructs a TabulatedInelasticCBRefSM for the specified material. This form
	 * of the constructor has an additional argument, energyOffset, allowing this
	 * parameter to be set to a value other than its default value of 0. This
	 * parameter is still needed in the present model, despite that this model does
	 * not use it to estimate the PE's kinetic energy, because Eoffset affects the
	 * SE kinetic energy after energy transfer.
	 * </p>
	 * <p>
	...
	public Electron scatter(Electron pe) {
		 ...
		 * below the energyGap as such undershoot and correct it.
		 */
		...
		case 3:
			...
			if (be > 0.) { // core level excitation
				...
				if ((Eqmin <= Eq) && (Eq <= Eqmax)) { // single-electron
					...
					final double[] newdir = updateDirection(thetaSE, phiSE, energytheta[1],
							2. * Math.PI * Math2.rgen.nextDouble());
	 ...
	 * used by the Electron class to deflect an electron, except that it accepts the
	 * initial angles in addition to the deflections and it returns the final
	 * angles.
	 *
	 ...
	 * with the secondary electron excitation channel. The default method inherited
	 * from Ding & Shimizu's SCANNING paper is to simply chose the highest binding
	 * energy that is lower than deltaE. If the user has specified branching ratios
	 * to associate with the various binding energies, the alternative method
	 * chooses a binding energy at random with probability consistent with the
	 * supplied ratios.
	 */
	...
	private double pickBE(double Eq, double deltaE) {
		int i;
		/*
		 * Detect and return immediately in the most common case (deltaE too small for
		 * inner shell excitation)
		 */
		if ((coreEnergies.length > 0) && (deltaE <= coreEnergies[0]))
			return 0.;
		/*
		 * Arrive here if there is enough energy in principle to free an inner shell
		 * electron. In this case we must compute E0 from the dispersion equation, given
		 * Eq and deltaE.
		 */
		...
		if (defaultRatios)
			...
			/*
			 * Above binary search returns a positive index in the rare case when r is
			 * exactly in the table. In such a case the energy we want is
			 * coreEnergies[index]. Usually, though, there is no exact match for r. In such
			 * case binarySearch returns index=-insertionPoint-1, so insertionPoint (the
			 * index of the first table value that is larger than r) is -(index+1). The
			 * energy we want though, is the next smaller one than this, i.e., the one at
			 * -(index+1)-1 = -index-2. If this remains less than 0, it means ALL table
			 * entries were greater than r, in which case the energy we want is 0.
			 */
	...
	private double computeE0fromDispersion(double Eq, double deltaE) {
		 ...
		 * Y:\proj\linewidth\jvillar \develop\NewMONSEL\Physics\DielectricDevelopment\
	...
	public double scatterRate(Electron pe) {
		 ...
		 * with a energyGap. In this case the actual scatter rate is 0.
		 */
	...
	public void setMaterial(Material mat) {
		...
		/*
		 * My source for binding energies provides them relative to vacuum for a few
		 * nobel and binary gases that we're unlikely to use, relative to the Fermi
		 * level for metals, and relative to the top of the valence band for insulators
		 * and semiconductors. bEref gives us our offsets to bottom of conduction band.
		 */
		if (bandgap > 0.)
			bEref = -bandgap;
		else
	 ...
	 * with an excitation. If deltaE is the energy lost by the primary electron in a
	 * scattering event, the secondary electron's final energy is equal to its
	 * initial energy plus deltaE, but what is its initial energy? Was it originally
	 * in the valence band, or in a deeper bound state? Only initial states with
	 * binding energies less than deltaE contribute a channel for energy loss.
	 * Typically the energy loss function (ELF) has a discontinuous increase when
	 * deltE is equal to its energy. TabulatedInelasticSM assumes the excitation
	 * channel that does not involve one of these bound electrons is unaffected by
	 * the presence of the new excitation channel. Thus, the ratio, r, of the ELF
	 * value from the left to the ELF value from the right represents the
	 * probability that the new channel is not involved, and 1-r is the probability
	 * that it is. These r ratios can easily be determined from the ELF vs. energy
	 * curve. ratios (the argument of this method) is an array of these ratios. The
	 * first ratio in the array is associated with the lowest nonzero core energy
	 * (i.e., the first non-valence band bound state). The length of the array must
	 * be equal to the length of the material's coreEnergies array.
	 * </p>
	 * <p>
	 * The default behavior, if this method is not called or if it is called with no
	 * argument, is to assume all entries are 0. That is, the largest eligible
	...
	public void setBranchingRatios(double[] ratios) {
		...
		for (int i = 1; i < cElen; i++) {
			...
			for (int j = 1; j <= i; j++)
				cumulativeBranchingProbabilities[i][j] = cumulativeBranchingProbabilities[i][j - 1]
						+ probabilities[i][j];
		}
	}

	/**
	 * This method was added to deal with LiF and similar materials. The distinction
	 * between energyGap and bandgap is this: JMONSEL understands the bandgap to be
	 * the distance between the top of the valence band and the bottom of the
	 * conduction band. The energyGap is the value of the smallest allowed deltaE in
	 ...
	 * gap, scattering into which is included in the scattering tables. In LiF, for
	 * instance, there are excitons just below the bottom of the conduction band.
	 * These were included in the energy loss function from which the scattering
	 * tables were derived in order to include their effect on the stopping power.
	 * </p>
	 * <p>
	 * The rule is, energyGap should be equal to the value used to compute the
	 * scattering tables, while bandgap, which is used to determine the position of
	 * the 0 of kinetic energy for free electrons in the crystal, should be the
	 * distance between the bands. If these are the same (usual case) this method
	 * need not be called. If they differ, use this method to distinguish them.
	 *
	 * @param energyGap
	 * @return
	 */
	...
	/**
	 * The scatterRate or IIMFP (inverse inelastic mean free path) derived from the
	 * associated table is multiplied by the factor provided via this routine
	 * (default = 1). I'm not sure if I'm going to keep this. I added it as an ad
	 * hoc way to simulate the effect of porosity in a material. The multiplier can
	 * be set equal to the fill fraction, i.e., the fraction of the space actually
	 * occupied by the material, in order to simulate the effect of many tiny pores
	 * in the material.
	 *
	 * @param rateMult
	 */
	...
	/**
	 * Sets the value assigned to E0fromDispersion. If E0fromDispersion = false (the
	 * default) JMONSEL continutes to use its original method (Ding & Shimizu's
	 * SCANNING method) for determining the energy available to ionize an inner
	 * shell. This method assumes the shell may be ionized whenever deltaE (the
	 * energy transferred to the SE in an inelastic event) is greater than the
	 * ionization energy. If E0fromDispersion = true, it uses Ding et al's later
	 * method, in which 0-momentum part (E0) of the energy is computed from the
	 * plasmon dispersion for an event which transfers deltaE. Inner shell
	 * ionization can only happen if E0 > ionization energy. This is more
