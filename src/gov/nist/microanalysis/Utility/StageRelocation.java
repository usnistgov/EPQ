...
public class StageRelocation {
    ...
    * 
    * @param pts
    *           The original and relocated points
    * @param mirror
    *           Should one axis be mirrored?
    * @throws EPQException
    */
   public StageRelocation(List<RelocatedPoint> pts, boolean mirror) throws EPQException {
      this();
      optimize(pts, mirror);
   }
   ...
   public double[] apply(double[] pt) {
      return new double[]{(((pt[0] + mX0rigin) * Math.cos(mRotation)) - ((pt[1] + mYOrigin) * Math.sin(mRotation))) * mXScale,
   ...
   public double[] inverse(double[] pt) {
      return new double[]{-mX0rigin + ((Math.cos(mRotation) * pt[0]) / mXScale) + ((Math.sin(mRotation) * pt[1]) / mYScale),
   ...
   static public class RelocatedPoint {
       ...
       * @param relocatedPt
       *           The same point in the translated coordinate system.
       */
   ...
   private void optimize(List<RelocatedPoint> pts, boolean mirror) throws EPQException {
      mMirrored = mirror;
      switch (pts.size()) {
         case 0 :
            break;
         ...
         default : {
            ...
            if (!mirror) {
               mX0rigin = (((-Math2.sqr(x1p) * x2) + (y1p * (((x2p * y1) - (x2 * y1p) - (x2p * y2)) + (x2 * y2p)))
                     + (x1p * (((x1 + x2) * x2p) + (-dy * y2p)))) - (x1 * ((Math2.sqr(x2p) - (y1p * y2p)) + Math2.sqr(y2p)))) / dp2;
               mYOrigin = -((((Math2.sqr(x2p) * y1) + (dx * x2p * y1p) + (Math2.sqr(x1p) * y2) + (dyp * y1p * y2)) - (dyp * y1 * y2p)
               ...
               mX0rigin = -(((((Math2.sqr(x1p) * x2) + (((dyp * x2) + (dy * x2p)) * y1p)) - (x1p * ((x1 * x2p) + (x2 * x2p) + (dy * y2p))))
            ...
            if ((distance(x1, y1, x1p, y1p) > (0.001 * Math.abs(x2p - x1p))) || (distance(x2, y2, x2p, y2p) > (0.001 * Math.abs(x2p - x1p))))
            ...
            try {
               if (pts.size() > 2) {
                  ...
                  class OptimizeFit extends Simplex {
                     ...
                     public double function(double[] v) {
                        ...
                        for (int i = 0; i < mPoints.size(); ++i) {
                           ...
                           final double xp = (((pt.mPoint1[0] + x0) * Math.cos(rot)) - ((pt.mPoint1[1] + y0) * Math.sin(rot))) * xSc;
                           final double yp = (((pt.mPoint1[1] + y0) * Math.cos(rot)) + ((pt.mPoint1[0] + x0) * Math.sin(rot))) * ySc;
