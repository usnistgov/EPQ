      ...
      try {
         ...
         try {
               ...
               writeln(pw, "TITLE", spec.toString());
            {
               ...
               writeln(pw, "DATE", (cal.get(Calendar.DAY_OF_MONTH) < 10 ? "0" : "")
                     + Integer.toString(cal.get(Calendar.DAY_OF_MONTH)) + "-" + months[cal.get(Calendar.MONTH)] + "-"
            ...
            if(sp.isDefined(SpectrumProperties.FaradayBegin)) {
               double pc = sp.getNumericProperty(SpectrumProperties.FaradayBegin);
               pc += sp.getNumericWithDefault(SpectrumProperties.FaradayEnd, pc);
            ...
            if(mode == Mode.COMPATIBLE) {
               ...
               if(sp.isDefined(SpectrumProperties.MultiSpectruMetric))
                  writeln(pw, "#MULTISPEC", Double.toString(sp.getNumericWithDefault(SpectrumProperties.MultiSpectruMetric, -1.0)));
            ...
            writeln(pw, "SPECTRUM", "");
            for(int n = 0; n < nCh; ++n) {
               pw.print((n < spec.getChannelCount() ? nf.format(spec.getCounts(n)) : "0") + ", ");
               if(((n % nCols) == (nCols - 1)) || (n == (nCh - 1)))
                  pw.println();
            }
