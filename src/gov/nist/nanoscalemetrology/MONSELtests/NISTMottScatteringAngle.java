...
public class NISTMottScatteringAngle
    ...
    * tabulated range computations use BrowningEmpiricalCrossSection, except
    * that the total cross section is multiplied by the ratio of the Mott value
    * at 50 eV to the Browning value there. (This in effect extrapolates below
    * 50 eV by using the form of Browning's energy dependence, but with the
    * total cross section at 50 eV adjusted to avoid a discontinuity between
    * Browning's approximation and the actual tabulated Mott value.)
    * </p>
    * <p>
    * This test version differs from similar alternatives as follows:
    * </p>
    * <p>
    * This has the same interpolation method as
    * EPQLibrary.NISTMottScatteringAngle but adds the low energy behavior.
    * </p>
    * <p>
    * Compared to JMONSEL.NISTMottRS, this one has the same low energy behavior
    * but does not implement cubic interpolation.
    * </p>
    * <p>
    * This version is in MONSELtests because it is expected to exist
    * temporarily, only for comparing performance (speed and accuracy) to the
    * improved JMONSEL.NISTMottRS version.
    * </p>
    *
    * @param elm Element
   ...
   final public double totalCrossSection(double energy) {
      ...
      if(energy < MIN_NISTMOTT) {
         ...
         if(j == 1)
            ...
            return value(DL50 + (58.0 * PARAM), DL50 + (59.0 * PARAM), DL50
                  + (60.0 * PARAM), mSpwem[SPWEM_LEN - 3], mSpwem[SPWEM_LEN - 2], mSpwem[SPWEM_LEN - 1], logE) * scale;
         else {
