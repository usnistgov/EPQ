...
public class ChargingListener implements ActionListener {
	 ...
	 * connection, qdot[i] is its time derivative, deltaV[i] is the potential drop
	 * across the ith connection, tau(deltaV[i]) is the time constant associated
	 * with this connection, and p is the reciprocal of a capacitance associated
	 * with the connection.
	 * </p>
	 * <p>
	 * deltaV is determined from a matrix equation of the form deltaV = Vprev +
	 * coefficientsChargesFromBeam.Q + coefficientsChargesFromCurrent.q where Vprev
	 * is the voltage across the connections at last FEA, Q are new charges on the
	 * floating regions since the last FEA and q is the amount of charge in the
	 * connections since the last FEA. The quantities are all vectors or matrices
	 * and "." means matrix multiplication. q is a vector of length equal to the
	 * number of connections (the dimension of the problem). Q is a vector of length
	 * equal to the number of floating regions involved in the connections. Since
	 * connections may involve 1 or 2 floating regions Q will have length greater
	 * than or equal to q's length.
	 * </p>
	 * <p>
	 ...
	 * the matrices, are determined by FEA by the constructor. The voltages at the
	 * last FEA are likewise determined internally. After construction, the only
	 * remaining parameter that the calling routine must supply to set up the
	 * equations before solving is the Q vector--the average charges on floating
	 * regions not due to charge flows in the connections (e.g., charges directly
	 * from the beam) that have appeared in the interval since the previous FEA. The
	 * calling routine supplies this vector by calling setV0().
	...
	private class ConnectionCurrentsODE implements FirstOrderDifferentialEquations {
		...
		/*
		 * The equation for how the voltages across the connections depend upon charges
		 * is in the form Vconnections = Vprev + coefficientsChargesFromBeam.Q +
		 * coefficientsChargesFromCurrent.q where Vprev is the voltage across the
		 * connections at last FEA, Q are new charges on the floating regions since the
		 * last FEA and q is the amount of charge in the connections since the last FEA.
		 * The quantities are all vectors or matrices and "." means matrix
		 * multiplication. q is a vector of length equal to the number of connections
		 * (the dimension of the problem). Q is a vector of length equal to the number
		 * of floating regions involved in the connections. Since connections may
		 * involve 1 or 2 floating regions Q will have length greater than or equal to
		 * q's length.
		 */
		...
		/*
		 * Following will hold the FloatingConstraints referenced in our connection
		 * list. Its a set to avoid duplicates. It's a LinkedHashSet because this is the
		 * only standard set implementation that preserves insertion order.
		 */
		private final LinkedHashSet<FloatingConstraint> floatSet = new LinkedHashSet<FloatingConstraint>();

		/**
		 * Constructs a ConnectionCurrentsODE with equilibration from the connections
		 * defined for the supplied MeshedRegion.
		 *
		 * @param r - the meshed region associated with this ConnectionCurrentsODE.
		 */
		...
		/**
		 * <p>
		 * Constructs with or without equilibration a ConnectionCurrentsODE from the
		 * connections defined for the supplied MeshedRegion.
		 * </p>
		 * <p>
		 * Equilibration refers to flowing charges in the connections so as to make the
		 * starting voltage drops on all connections equal to 0, the steady state
		 * condition. This is appropriate if we are simulating a case in which the
		 * sample has had a time to equilibrate that is large compared to the relaxation
		 * times on all the connections.
		 * </p>
		 *
		 * @param r           - the meshed region associated with this
		 *                    ConnectionCurrentsODE.
		 * @param equilibrate - if true, enough charge is flowed in each connection to
		 *                    make its initial voltage drop = 0. Otherwise, the charge
		 *                    is left as given and the connections may start in
		...
		public ConnectionCurrentsODE(MeshedRegion r, boolean equilibrate) {
			...
			/*
			 * Add FloatingConstraints to our floatSet and construct source and drain
			 * matrices. The matrix dimensions are initialized to the maximum possible size.
			 * We'll truncate later, after we know what the true size is.
			 */
			...
			for (int connectionIndex = 0; connectionIndex < dim; connectionIndex++) {
				...
				if (source instanceof FloatingConstraint) {
					...
					if (floatSet.add(src)) {
						/*
						 * src was not already in our set. Its index is just the current
						 * constraintIndex.
						 */
				...
				if (drain instanceof FloatingConstraint) {
					...
					/*
					 * In my notes drainMatrix always appears with - sign. Here, in contrast to the
					 * notes, I include the sign in the definition so I can add it instead of
					 * subtract it.
					 */
					if (floatSet.add(drn)) {
						/*
						 * drn was not already in our set. Its index is just the current
						 * constraintIndex.
						 */
			...
			/*
			 * Load coefficientsPotential with final potentials (after unit charge flows)
			 */
			...
			feaRunner.runFEA(r);
			/*
			 * Subtract initial potentials from final ones to obtain changes caused by unit
			 * charge changes
			 */
			for (int j = 0; j < floatCount; j++) {
			...
			/*
			 * We're now in a position to calculate the matrices we need to define the
			 * equations
			 */
		...
		/*
		 * In the following, t is the time, q the charge, and qdot the derivative of
		 * charge.
		 */
		@Override
		public void computeDerivatives(double t, double[] q, double[] qdot) {
			...
			for (int i = 0; i < dim; i++) {
				/*
				 * In the following lines, deltaV is the voltage drop across connection i.
				 * recipC is effectively 1/capacitance for this connection. Tau/capacitance is
				 * resistance of the connection. -deltaV/resistance is the current. The minus
				 * sign is because current flows so as to reduce deltaV to 0.
				 */
		...
		/**
		 * Solves the differential equation to determine how much charge flows in each
		 * connection in the interval deltat, then updates the connections to move this
		 * much charge.
		 *
		...
		public void integrate(double deltat) throws MathException, MathException {
			/*
			 * The following lines set these parameters for the integrator: Min step size =
			 * 1.e-5*deltat, Max step size = deltat, Abs tolerance = 1.e-5, Rel tolerance =
			 * 1.e-5, Max Evaluations = 10000.
			 */
			/*
			 * FYI: In tests with dim = 5 tolerance of 1.e-15 for rel and abs required ~250
			 * calls to computeDerivatives in order to converge. With tolerances 1.e-5 it
			 * requires ~35. DormandPrince54Integrator required about double the calls to
			 * computeDerivatives without noticeable improvement in accuracy. Note that the
			 * number of calls to computeDerivatives depends on deltat relative to the
			 * relaxation time. As tau gets shorter, the number of calls must go up because
			 * q has more curvature. With tolerances 1.e-5 and tau a factor of 8 smaller
			 * than above, the number of calls was ~150.
			 */
		...
		/**
		 * Sets the offset voltages (the constant term) that will be used for the ODE
		 * equations. The offset voltage is a vector, V0, that is equal to the voltage
		 * drops across the connections at the last FEA plus an additional drop that
		 * depends linearly on charges added to the regions subject to
		 * FloatingConstraint in the intervening interval. The user supplies an array of
		 * these charges (in units of e). This method computes and includes the effect
		 * of those charges on the potentials.
		 *
		 * @param beamQ - an array of new charges on the regions subject to
		 *              FloatingConstraint. I.e., beamQ[i] is the additional charge on
		 *              the region corresponding to the i'th constraint in the list that
		 *              can be obtained by calling getFloatSet().
		 */
		public void setV0(double[] beamQ) {
			final int numFloats = floatSet.size();
			if (beamQ.length != numFloats)
				throw new EPQFatalException(
						"beamQ (argument of setV0) was length " + beamQ.length + " but must be length " + numFloats);
		...
		/**
		 * Computes the voltage across the connections with additional charge q moved
	 ...
	 * without conduction. * @param mcss - The MonteCarloSS instance to which this
	 ...
	 *
	 * @param mcss                      - The MonteCarloSS instance to which this
	 *                                  listener is attached.
	 * @param solnInterval              - The number of trajectories between FEA
	 *                                  solutions.
	 * @param feaRunner                 - An IFEArunner that provides a method to
	 *                                  run FEA and (usually) methods to specify FEA
	 *                                  parameters.
	 * @param feaInitializationRequired - true if the first FEA should be run at n =
	 *                                  0, false if the first one should be run at n
	 *                                  = solnInterval.
	 */
	public ChargingListener(MonteCarloSS mcss, int solnInterval, IFEArunner feaRunner,
			boolean feaInitializationRequired) {
	 ...
	 *                                  0, false if the first one should be run at n
	 *                                  = solnInterval.
	 * @param conductionModel           - an IConductionModel if there is to be
	 *                                  conduction, null if not.
	 */

	public ChargingListener(MonteCarloSS mcss, int solnInterval, IFEArunner feaRunner,
			boolean feaInitializationRequired, IConductionModel conductionModel) {
		mMonte = mcss;
	 ...
	 * delays the choice of solver until the first time actionPerformed is called.
	 * At that point the default depends upon the constraints, as follows:<br>
	 * If there are any floating regions we use PETScFEAfunner(feaFolder) with its
	 * defaults.<br>
	 ...
	 * options that is good under all conditions. The above defaults have been found
	 * by experience to work well under most circumstances. However, if the solver
	 * does not perform well on your particular problem, it may be worthwhile to
	 * experiment.
	 * </p>
	 *
	 * @param mcss                      - The MonteCarloSS instance to which this
	 ...
	 * @param solnInterval              - The number of trajectories between FEA
	 *                                  solutions.
	 * @param feaFolder                 - The name of the scratch folder the default
	 *                                  feaRunner should use for communication with
	 *                                  the solver.
	 * @param feaInitializationRequired - true if the first FEA should be run at n =
	 *                                  0, false if the first one should be run at n
	 *                                  = solnInterval.
	...
	public void actionPerformed(ActionEvent event) {
		 ...
		 * electron. We don't do anything on these events anyway, so if the electron is
		 * null, we just return.
		 */
		if (el == null)
			return;
		...
		/*
		 * Store the electron's MeshElementRegion in region. If the electron is not in
		 * the mesh, region = null.
		 */
				...
				if (r.getConnectionList().length > 0)
					...
					if (logCP) {
						...
						try {
							...
							if (meshRefined) {
								/*
								 * It's an adaptive mesh and it has changed. We need to log the new mesh too, in
								 * order to be able to interpret the charge and potentials file we have just
								 * saved.
								 */
		...
		if (eventID == MonteCarloSS.TrajectoryStartEvent) {
			if (chargesAccumulate)
				trajCount++;
			/*
			 * If the user hasn't defined maxCascade, initialize it to permit 1 eV/electron
			 */
			if (maxCascade == 0)
					...
					if (logCP) {
						...
						try {
							...
							if (meshRefined) {
								 ...
								 * saved.
								 */
								final String meshFileName = cpDestFolder + File.separator + "mesh" + logCPcount
										+ ".msh";
								((IAdaptiveMesh) r.getMesh().getBasicMesh()).saveMesh(meshFileName);
							}
						} catch (final Exception e) {
		...
		/*
		 * Check whether the electron is in a MeshElementRegion. If not, no action is
		 * performed. Unmeshed volumes are assumed to be field free.
		 */
		if (region == null)
		...
		switch (eventID) {
		/*
		 * When an electron comes to rest (TrajectoryEnd or EndSecondary) in a region
		 * where the potential is not constrained, decrement the total charge in its
		 * region by e.
		 */
		...
		case MonteCarloSS.EndSecondaryEvent:
			if (chargesAccumulate) {
				...
				if ((region != null) && !region.isConstrained()) {
					/*
					 * Note that the logic here relies on the fact, currently true, that the only
					 * constraint that can be applied to a region (i.e., a volume) is the Dirichlet
					 * type. For Dirichlet constraint, there is no charge build-up because whatever
					 * supplies the fixed potential is a source or sink of charge. This is not
					 * necessarily true for other constraints. E.g., a floating region has no source
					 * or sink of charge. At present, floating constraints are associated only with
					 * the *surfaces* of floating regions, not with volume elements--so this code is
					 * correct. But we have to be careful if that changes in the future.
					 */
		...
		/*
		 * When a SE starts in a region with unconstrained potential, increment that
		 * region's total charge by e.
		 */
		...
		/*
		 * Each step ends with a scatter or nonscatter event. At these, make corrections
		 * to account for E-fields due to charging.
		 */
	...
	/**
	 * Utility to correct the electron's last step in accordance with the fields in
	 * its shape. The correction used is Euler's, wherein only terms up to linear in
	 * the step size are retained. This approximation corrects only the electron's
	 * energy and velocity at each step. Its present position is not altered
	 * (although of course the change in velocity alters its future positions).
	 *
	...
	private void correctTrajectory(Electron el, Tetrahedron shape) {
		 ...
		 * compared to the potential energy difference across its region it's possible
		 * it will do some strange things. For example, it's possible within the
		 * approximation for the electron's final kinetic energy to be negative. This
		 * should only be possible when the electron's energy is small so that it should
		 * be dropped. For it to be correctly dropped it may be necessary for various
		 * scattering routines to be tolerant of a negative energy.
		 */
		final double kE = el.getEnergy();
		final double stepLen = el.stepLength();
		/*
		 * Tiny steps can cause numerical problems in the next line, but corrections are
		 * correspondingly tiny. We can just skip them.
		 */
		if (stepLen < 1.e-11)
			return;
		final double scaledT;
		/*
		 * For kE <= 0. scaleT = 0. This means the new electron direction is determined
		 * solely by the electric field lines.
		 */
		if (kE > 0.)
		...
		/*
		 * In rare cases electrons can get trapped in a potential energy minimum in
		 * vacuum. (There's probably a way out, but we don't have time to wait for it to
		 * be found.) Such traps are characterized by finalE < 0 (they contain turning
		 * points), large electron step count, and the material is vacuum. In such cases
		 * we give the electron a little boost.
		 */
	...
	private void doFEAwithConnectionFlows(MeshedRegion r, ConnectionCurrentsODE connectionODE, double deltat) {
		if (connectionODE != null) {
			...
			for (final FloatingConstraint f : fset) {
				...
				f.computeCharge();
				/*
				 * New charge was 0 at previous FEA and is f.getCharge()-qold now. The divide by
				 * 2 represents an average of excess charge during this interval. This is crude,
				 * but since we don't have details of when the charge arrived, it's the best we
				 * can do.
				 */
	 ...
	 * bookkeeping is turned on. Changes of the total charge in mesh volume elements
	 * are tracked. At the next finite element analysis, the potentials and fields
	 * will reflect the new charge distribution. When chargesAccumulate = false,
	 * electrons trajectories are affected by whatever the fields are, but changes
	 * to the charge distribution are ignored. The counter that records the number
	 * of trajectories since the last FEA is not incremented when chargesAccumlate
	 * is false, and no new FEA is run. Setting chargesAccumulate = false can be
	 * useful when it is desirable to run a number of trajectories without changing
	 * the condition of the mesh, for example to generate a trajectory plot or
	 * obtain good statistics for the yield, energy spectrum, etc.
	 *
	...
	/**
	 * A utility to do some cleanup chores we must do every time we refine the mesh.
	 * It checks whether the mesh was refined. If it was, it reinitializes it. Then,
	 * if the electron is in the mesh it either updates the potentials only (if the
	 * mesh wasn't refined) or else updates the electron's entire current region (if
	 ...
	 * default, chargeMultiplier = 1. and each trapped electron is counted as a full
	 * electron. Under some circumstances it is desirable to count each trapped
	 * charge as less than a full charge. For example, to determine the average
	 * potential produced by depositing one electron into a pixel, we could deposit
	 * 1 electron, do an FEA, repeat 1000 times, and average the results.
	 * Alternatively, we could run 1000 electrons, treat each charge as having
	 * 1/1000 its usual value, and do a single FEA. To do the latter, set
	 * chargeMultiplier = 1/1000. Electrons retain their usual charge when moving in
	 * fields. The multiplier only affects how the sources of the field are dealt
	 * with.
	 *
	...
	/**
	 * Turns on logging. When logging is on, a file called cp[n].dat is written to
	 * the destination folder designated by the input string. (The folder is created
	 * if necessary.) Each time a file is written, n is incremented, so the files
	 * are cp1.dat, cp2.dat, etc. A second optional argument can specify that the
	 * count starts from some other positive integer value. (This can be useful to
	 * avoid overwriting earlier log files when resuming an interrupted simulation.)
	 * The log file is generated using the Mesh class's exportChargeAndPotentials
	 * method. It is a record of nonzero charges contained in volume elements of the
	 * mesh and the potentials on nodes of the mesh. It can be subsequently imported
	 * with the Mesh class's importChargeAndPotentials method to recreate the state
	 * of the mesh that is stored in the log file.
	 *
	 * @param destFolder
	 */
